...
class SqlActionGroupRepository:
    ...
    @classmethod
    def _from_row(cls, row: ActionGroupRow) -> ActionGroupRecord:
        payload = json.loads(str(row.members_json))
        if not isinstance(payload, list):
            raise TypeError("action group members_json must be a list")
        members = tuple(
            ActionGroupMember(
                ...
                client_order_id=(
                    None if item.get("client_order_id") is None else str(item["client_order_id"])
                ),
        ...
        return ActionGroupRecord(
            action_group_id=UUID(str(row.action_group_id)),
    ...
    def get(self, action_group_id: UUID) -> ActionGroupRecord | None:
        with self._session_factory() as session:
            row = session.scalar(
                select(ActionGroupRow).where(ActionGroupRow.action_group_id == str(action_group_id))
            )
...
class SqlExecutionLedger(InMemoryExecutionLedger):
    ...
    @staticmethod
    def _identity(order: ExecutionOrder) -> tuple[str, str, str, str, str]:
        ...
        position_side = str(getattr(getattr(intent, "position_side", None), "value", "")).strip()
    ...
    @staticmethod
    def _correlation_id(order: ExecutionOrder) -> str:
        intent = order.intent
        group = getattr(intent, "action_group_id", None)
        return str(group or intent.intent_id)
    ...
    def record(
        ...
        *,
        ...
        typed_order = cast(ExecutionOrder, order)
        exchange, account_id, symbol, position_side, client_order_id = self._identity(typed_order)
        ...
        correlation_id = self._correlation_id(typed_order)
        lifecycle = typed_order.lifecycle
        ...
        with self._session_factory.begin() as session:
            ...
            SqlExecutionStore(self._session_factory)._put_in_session(session, typed_order)
            ...
            if existing_intent is None:
                session.add(
                    OrderIntentRow(
                        ...
                        approved_at=_naive_utc(typed_order.created_at),
                    )
                )
            else:
                expected_key = (
                ...
                existing_intent.status = getattr(lifecycle.status, "value", str(lifecycle.status))
            ...
            if fill is not None:
                ...
                if fill.order_side is None or fill.action is None:
                    raise ValueError("execution FillEvent requires order_side and action")
                ...
                if existing is None:
                    session.add(
                        FillEventRow(
                            ...
                            exchange_order_id=(
                                getattr(lifecycle, "exchange_order_id", None) or client_order_id
                            ),
            ...
            session.add(
                AuditEvent(
                    ...
                    occurred_at=_naive_utc(lifecycle.updated_at),
                )
            )
            ...
            if outbox is not None:
                ...
                session.add(
                    EventOutbox(
                        ...
                        status=("PENDING" if outbox.published_at is None else "PUBLISHED"),
                        attempts=outbox.attempts,
                        published_at=(
                            None if outbox.published_at is None else _naive_utc(outbox.published_at)
                        ),
                    )
                )
    ...
    def mark_published(
        ...
        *,
        ...
        with self._session_factory.begin() as session:
            row = session.scalar(select(EventOutbox).where(EventOutbox.event_id == str(event_id)))
            if row is None:
...
def _metadata_from_json(raw: str) -> Mapping[str, Any]:
    ...
    if not isinstance(decoded, Mapping):
        raise TypeError("execution metadata_json must decode to a mapping")
    return decoded
...
class SqlExecutionStore:
    ...
    def _apply(row: ExecutionOrderStateRow, order: ExecutionOrder) -> None:
        ...
        row.lifecycle_average_price = (
            None if lifecycle.average_price is None else canonical_decimal(lifecycle.average_price)
        )
    ...
    def list_orders(
        ...
        *,
        ...
        position_side: ExecutionPositionSide | None = None,
        ...
        stmt = select(ExecutionOrderStateRow)
        if account_id is not None:
            stmt = stmt.where(ExecutionOrderStateRow.account_id == account_id)
        if symbol is not None:
            stmt = stmt.where(ExecutionOrderStateRow.symbol == symbol)
        if position_side is not None:
            stmt = stmt.where(ExecutionOrderStateRow.position_side == position_side.value)
        if statuses is not None:
            stmt = stmt.where(
                ExecutionOrderStateRow.lifecycle_status.in_(
                    tuple(state.value for state in statuses)
                )
            )
        if action_group_id is not None:
            stmt = stmt.where(ExecutionOrderStateRow.action_group_id == str(action_group_id))
        if not include_terminal:
            stmt = stmt.where(
                ExecutionOrderStateRow.lifecycle_status.notin_(
                    tuple(state.value for state in TERMINAL_STATES)
                )
            )
        if newest_first:
            stmt = stmt.order_by(
                ExecutionOrderStateRow.created_at.desc(),
                ExecutionOrderStateRow.id.desc(),
            )
        else:
            stmt = stmt.order_by(
                ExecutionOrderStateRow.created_at,
                ExecutionOrderStateRow.id,
            )
...
class SqlExecutionIdempotencyStore:
    ...
    def __init__(
        ...
        *,
        ...
        self._owner_id = owner_id or (f"{socket.gethostname()}:{os.getpid()}:{uuid4().hex[:8]}")
    ...
    def complete(self, key: str, value: ExecutionResult) -> None:
        ...
        with self._session_factory.begin() as session:
            ...
            if row.lease_expires_at is None or row.lease_expires_at <= now:
                raise TimeoutError("idempotency reservation lease expired before completion")
