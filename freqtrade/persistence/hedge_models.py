...
class ExecutionOrderStateRow(HedgeModelBase):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("idempotency_key", name="uq_hedge_execution_order_idempotency_key"),
        Index(
...
class ExecutionBudgetReservationRow(HedgeModelBase):
    ...
    __table_args__ = (
        CheckConstraint(
            "state IN ('PREPARED','CONFIRMED','RELEASED')",
            name="ck_hedge_r5_budget_reservation_state",
        ),
...
class ExecutionIncomeEventRow(HedgeModelBase):
    ...
    __table_args__ = (
        UniqueConstraint(
            "exchange", "account_id", "external_event_id", name="uq_hedge_r5_income_event"
        ),
...
def create_hedge_tables(engine: Engine) -> None:
    ...
    HedgeModelBase.metadata.create_all(
        engine, tables=[cast(Table, model.__table__) for model in HEDGE_MODEL_CLASSES]
    )
