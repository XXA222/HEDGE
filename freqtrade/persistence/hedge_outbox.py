        ...
        with self.session_factory.begin() as session:
            result = session.execute(
                ...
                .values(
                    ...
                    available_at=(now if exhausted else now + self._retry_delay(envelope.attempts)),
                    lock_token=None,
                    locked_at=None,
    ...
    def release_stale_leases(self) -> int:
        ...
        with self.session_factory.begin() as session:
            ...
            for row in rows:
                ...
                row.available_at = now if exhausted else now + self._retry_delay(row.attempts)
                row.lock_token = None
                row.locked_at = None
...
def dead_letter_rows(session: Session) -> Sequence[EventOutbox]:
    return session.scalars(
        select(EventOutbox).where(EventOutbox.status == "DEAD").order_by(EventOutbox.id)
    ).all()
