...
def _normalize_position_facts(
    facts: Iterable[PositionFact],
) -> dict[tuple[str, str], PositionFact]:
    normalized: dict[tuple[str, str], PositionFact] = {}
    for fact in facts:
...
def _normalize_account_position_facts(
...
def _normalize_account_order_facts(
    orders: Iterable[OrderFact],
) -> dict[tuple[str, str, str], OrderFact]:
    ...
    for fact in orders:
        ...
        order_facts[key] = OrderFact(
            ...
            average_price=(
                canonical_decimal(fact.average_price) if fact.average_price is not None else None
            ),
...
def _record_account_mode_and_unmanaged_diffs(
    *,
    exchange: str,
    account_id: str,
    mode: AccountModeFact | None,
    expected_position_mode: str,
    expected_margin_mode: str,
    expected_leverage: str | None,
    unmanaged_positions: Iterable[str],
    unmanaged_orders: Iterable[str],
    add_diff: Callable[..., None],
    ...
    return unmanaged_position_keys, unmanaged_order_keys


def _reconcile_position_entity(
    repository: HedgeLedgerRepository,
    *,
    run_id: str,
    account_id: str,
    exchange: str,
    ...
    side: str,
    local: Any,
    remote: PositionFact | None,
    policy: ReconciliationPolicy,
    observed_at: datetime | None,
) -> tuple[int, bool]:
    entity_key = f"{exchange}|{account_id}|{symbol}|{side}"
    local_quantity = decimal_value(local.quantity) if local else Decimal(0)
    exchange_quantity = decimal_value(remote.quantity) if remote else Decimal(0)
    local_price = decimal_value(local.entry_price) if local else Decimal(0)
    exchange_price = decimal_value(remote.entry_price) if remote else Decimal(0)
    ...
    if max(local_quantity, exchange_quantity) > 0 and price_drift > policy.price_tolerance:
        fields.append(("entry_price", local_price, exchange_price, price_drift))
    for field_name, local_value, exchange_value, _drift in fields:
        repository.add_reconciliation_diff(
            ...
            resolution="AUTO_REPAIRED" if policy.auto_repair_positions else "UNRESOLVED",
            repair_action=(
                "APPEND_EXCHANGE_POSITION_FACT" if policy.auto_repair_positions else None
            ),
        )
    repaired = bool(fields and policy.auto_repair_positions)
    if repaired:
        ...
        repository.append_position_snapshot(
            snapshot_key=f"reconciliation:{run_id}:{symbol}:{side}:{sequence_number or 0}",
            ...
            realized_pnl=(remote.realized_pnl if remote else local.realized_pnl if local else "0"),
            source="REST",
            source_event_time=event_time,
            sequence_number=sequence_number,
            raw_payload={
                "reconciliation_run_id": run_id,
                "exchange_position_missing": remote is None,
            },
        )
    return len(fields), repaired


def _reconcile_account_position_entity(
    repository: HedgeLedgerRepository,
    *,
    run_id: str,
    account_id: str,
    ...
    """Compare one account position while preserving the account audit contract."""
    ...
    if max(local_qty, remote_qty) > 0 and abs(local_price - remote_price) > policy.price_tolerance:
        changed = True
        add_diff(
            entity_type="POSITION",
            entity_key=entity_key,
    ...
    if not (changed and policy.auto_repair_positions):
        return 0
    event_time = (remote.event_time if remote else None) or observed_at
    if event_time is None:
        raise ValueError("Position auto repair requires event time")
    repository.append_position_snapshot(
        snapshot_key=f"reconciliation:{run_id}:{symbol}:{side}",
        account_id=account_id,
        exchange=exchange,
        symbol=symbol,
        position_side=side,
        ...
        source_event_time=event_time,
        sequence_number=remote.sequence_number if remote else None,
        raw_payload={"reconciliation_run_id": run_id},
    )
    return 1
...
class HedgeReconciler:
    ...
    def reconcile_positions(
        ...
        *,
        ...
        normalized_facts = _normalize_position_facts(facts)
        ...
        for symbol, side in all_keys:
            ...
            exchange_quantity = decimal_value(exchange_row.quantity) if exchange_row else Decimal(0)
            ...
            diff_count, was_repaired = _reconcile_position_entity(
                self.repository,
                run_id=run.run_id,
                account_id=account_id,
                exchange=exchange,
    ...
    def reconcile_account_facts(
        ...
        *,
        ...
        policy = policy or ReconciliationPolicy()
        position_facts = _normalize_account_position_facts(positions)
        order_facts = _normalize_account_order_facts(orders)
        ...
        for symbol, side in sorted(set(local_positions) | set(position_facts)):
            ...
            repaired += _reconcile_account_position_entity(
                self.repository,
                run_id=run.run_id,
                account_id=account_id,
                exchange=exchange,
        ...
        for key in sorted(set(local_orders) | set(order_facts)):
            order_local = local_orders.get(key)
            order_remote = order_facts.get(key)
            symbol, side, order_id = key
            entity_key = f"{exchange}|{account_id}|{symbol}|{side}|{order_id}"
            comparisons = {
                "status": (
                    normalize_order_status(order_local.status) if order_local else None,
                    order_remote.status if order_remote else None,
                ),
                "executed_quantity": (
                    order_local.executed_quantity if order_local else None,
                    order_remote.executed_quantity if order_remote else None,
                ),
                "remaining_quantity": (
                    order_local.remaining_quantity if order_local else None,
                    canonical_decimal(
                        max(
                            decimal_value(order_remote.original_quantity)
                            - decimal_value(order_remote.executed_quantity),
                            Decimal(0),
                        )
                    )
                    if order_remote
            ...
            for field_name, (local_value, remote_value) in comparisons.items():
                ...
                add_diff(
                    ...
                    repair_action=(
                        ...
                        if policy.auto_repair_orders and order_remote is not None
                        else None
                    ),
                )
            if order_changed and policy.auto_repair_orders and order_remote is not None:
                event_time = order_remote.event_time or observed_at
                if event_time is None:
                ...
                self.repository.append_order_snapshot(
                    ...
                    client_order_id=order_remote.client_order_id,
                    side=order_remote.side,
                    action=order_remote.action,
                    status=order_remote.status,
                    original_quantity=order_remote.original_quantity,
                    executed_quantity=order_remote.executed_quantity,
                    cumulative_quote=order_remote.cumulative_quote,
                    average_price=order_remote.average_price,
                    source="REST",
                    source_event_time=event_time,
                    sequence_number=order_remote.sequence_number,
                    raw_payload={"reconciliation_run_id": run.run_id},
                )
                repaired += 1

        unmanaged_position_keys, unmanaged_order_keys = _record_account_mode_and_unmanaged_diffs(
            exchange=exchange,
            account_id=account_id,
            mode=mode,
