...
from sqlalchemy import Engine, Table, bindparam, inspect, text
...
def _psutil_pid_is_alive(pid: int) -> bool | None:
    try:
        import psutil
    ...
    try:
        ...
        # An existing process that cannot be inspected must retain the lock.
        return True


def _stdlib_pid_is_alive(pid: int) -> bool:
    try:
        os.kill(pid, 0)
...
def _pid_is_alive(pid: int) -> bool:
    ...
    if isinstance(pid, bool) or pid <= 0:
        return False
    psutil_result = _psutil_pid_is_alive(pid)
    return _stdlib_pid_is_alive(pid) if psutil_result is None else psutil_result
...
@contextmanager
def _postgresql_migration_lock(engine: Engine, timeout_seconds: float):
    ...
    try:
        while not acquired:
            ...
            if time.monotonic() >= deadline:
                raise HedgeMigrationError("Timed out waiting for PostgreSQL H3 migration lock")
...
@contextmanager
def _memory_migration_lock_context(engine: Engine, timeout_seconds: float):
...
@contextmanager
def _sqlite_migration_lock(engine: Engine, timeout_seconds: float):
    database = _sqlite_database_path(engine)
    if database is None:
        with _memory_migration_lock_context(engine, timeout_seconds):
            yield
        return
...
def _migration_lock(engine: Engine, *, timeout_seconds: float = 60.0):
    ...
    if engine.dialect.name == "postgresql":
        with _postgresql_migration_lock(engine, timeout_seconds):
            yield
        return
    with _sqlite_migration_lock(engine, timeout_seconds):
        yield
...
def _ensure_migration_table(engine: Engine) -> None:
    ...
    _table(SchemaMigrationRecord).create(engine, checkfirst=True)
...
def _backup_sqlite(engine: Engine, backup_directory: Path | None) -> str:
    ...
    if not integrity or integrity[0] != "ok":
        ...
        raise HedgeMigrationError(f"SQLite pre-migration backup verification failed: {integrity!r}")
...
def _postgresql_backup_tables(connection: Any, source_schema: str) -> list[str]:
    ...
    selected = {
        table for table in available if table in {"trades", "orders"} or table.startswith("hedge_")
    }
...
def _postgresql_restore_table_order(
    ...
    if len(ordered) != len(selected):
        ...
        raise HedgeMigrationError(
            f"PostgreSQL restore dependency cycle cannot be restored safely: {cyclic}"
        )
    return ordered
    ...
    for table in tables:
        ...
        for column in inspector.get_columns(table, schema=target_schema):
            ...
            maximum = connection.execute(
                text(f"SELECT MAX({quoted_column}) FROM {quoted_target_schema}.{quoted_table}")
            ).scalar_one_or_none()
            if maximum is None:
                connection.execute(
...
def _load_postgresql_backup_manifest(
    connection: Any,
    schema: str,
    quoted_schema: str,
) -> tuple[list[Any], str]:
    if schema not in set(inspect(connection).get_schema_names()):
    ...
    return list(manifest_rows), target_schema


def _validate_postgresql_backup_columns(
    connection: Any,
    schema: str,
    target_schema: str,
    ...
    for row in manifest_rows:
        ...
        if table not in available_public:
            raise HedgeMigrationError(f"Restore target table is missing: {target_schema}.{table}")
        public_columns = [
            column["name"]
            for column in inspect(connection).get_columns(table, schema=target_schema)
        ]
        backup_columns = [
            column["name"] for column in inspect(connection).get_columns(table, schema=schema)
        ]
...
def _restore_postgresql_backup_rows(
    connection: Any,
    engine: Engine,
    schema: str,
    quoted_schema: str,
    ...
    truncate_targets = ", ".join(
        f"{quoted_target_schema}.{_quote(engine, table)}" for table in reversed(restore_order)
    )
    try:
    ...
    for table in restore_order:
        ...
        actual = int(
            connection.execute(
                text(f"SELECT COUNT(*) FROM {quoted_target_schema}.{quoted_table}")
            ).scalar_one()
        )
...
def restore_postgresql_backup(
    ...
    *,
    ...
    quoted_schema = _quote(engine, schema)
    with engine.begin() as connection:
        manifest_rows, target_schema = _load_postgresql_backup_manifest(
            connection, schema, quoted_schema
        )
        _validate_postgresql_backup_columns(connection, schema, target_schema, manifest_rows)
        restored = _restore_postgresql_backup_rows(
            connection, engine, schema, quoted_schema, target_schema, manifest_rows
        )
...
def _step_backfill_core(engine: Engine) -> dict[str, Any]:
    ...
    with engine.begin() as connection:
        connection.execute(
            text("UPDATE trades SET account_id='default' WHERE account_id IS NULL OR account_id=''")
        )
        connection.execute(
            text(
        ...
        connection.execute(
            text(
                "UPDATE trades SET hedge_version=0 WHERE hedge_version IS NULL OR hedge_version < 0"
            )
        )
        connection.execute(
            text(
        ...
        if orders_exists:
            ...
            if "ft_is_open" in order_columns:
                submit_state_sql = (
                    ...
                    "WHEN ft_is_open=" + true_literal + " THEN 'ACKNOWLEDGED' ELSE 'TERMINAL' END "
                ...
                # safe to resubmit.
                submit_state_sql = (
                    "UPDATE orders SET submit_state='TERMINAL' WHERE submit_state IS NULL"
                )
...
def _collect_conflicts(engine: Engine) -> dict[str, Any]:
    ...
    if _table_exists(engine, "trades"):
        ...
        with engine.connect() as connection:
            ...
            if invalid_rows:
                conflicts["invalid_active_trade_identity"] = [dict(row) for row in invalid_rows]
            rows = (
                connection.execute(
                    text(
...
def _step_fix_fill_identity_scope(engine: Engine) -> dict[str, Any]:
    ...
    if not _table_exists(engine, table):
        ...
        _table(FillEvent).create(engine, checkfirst=True)
    ...
    with engine.begin() as connection:
        ...
        if old_table not in tables:
            connection.execute(
                text(f"ALTER TABLE {_quote(engine, table)} RENAME TO {_quote(engine, old_table)}")
            )
        elif table in tables:
            connection.execute(text(f"DROP TABLE {_quote(engine, table)}"))

        index_rows = (
            connection.exec_driver_sql(f"PRAGMA index_list({_quote(engine, old_table)})")
            .mappings()
            .all()
        )
        ...
        _table(FillEvent).create(connection, checkfirst=False)
        fill_columns = [column.name for column in _table(FillEvent).columns]
        quoted_columns = ", ".join(_quote(engine, column) for column in fill_columns)
        connection.execute(
            text(
...
def _step_verify_v1_1(engine: Engine) -> dict[str, Any]:
    ...
    invalid_fill_identity = _table_exists(engine, FILL_TABLE) and not _has_unique_columns(
        engine,
        FILL_TABLE,
...
def _step_verify_hardened_identity(engine: Engine) -> dict[str, Any]:
    ...
    if _table_exists(engine, "orders") and "idempotency_key" in _column_names(engine, "orders"):
        with engine.connect() as connection:
            empty_keys = int(
                connection.execute(
    ...
    if _table_exists(engine, "trades"):
        if engine.dialect.name == "sqlite":
            ...
            if set(trigger_sql) != required:
                drift["missing_trade_identity_triggers"] = sorted(required - set(trigger_sql))
...
def _step_verify_full_function(engine: Engine) -> dict[str, Any]:
    ...
    if missing or invalid_indexes:
        raise HedgeMigrationError(
            _canonical_json({"missing_columns": missing, "invalid_indexes": invalid_indexes})
        )
    return {"schema_version": H3_SCHEMA_VERSION, "release": H3_RELEASE}
...
def _backfill_order_intents(engine: Engine, connection: Any) -> int | None:
    if not _table_exists(engine, "hedge_order_intents"):
        return None
    result = connection.execute(
        text(
...
def _backfill_order_snapshots(engine: Engine, connection: Any) -> int | None:
    if not _table_exists(engine, "hedge_order_snapshots"):
        return None
    ...
    for row in rows:
        ...
        connection.execute(
            ...
            ),
            {
                "symbol": symbol,
                "fact_key": fact_key,
                "remaining": format(max(original - executed, Decimal(0)), "f"),
                "correlation_id": row["correlation_id"] or row["intent_id"] or row["snapshot_key"],
                "id": row["id"],
            },
        )
    return len(rows)
...
def _backfill_fills(engine: Engine, connection: Any) -> int | None:
    if not _table_exists(engine, "hedge_fill_events"):
        return None
    from freqtrade.persistence.hedge_contracts import canonical_symbol

    rows = (
        connection.execute(text("SELECT id, symbol, intent_id, event_id FROM hedge_fill_events"))
        .mappings()
        .all()
    )
    for row in rows:
...
def _backfill_position_snapshots(engine: Engine, connection: Any) -> int | None:
    if not _table_exists(engine, "hedge_position_snapshots"):
        return None
    from freqtrade.persistence.hedge_contracts import canonical_symbol, stable_fact_key
...
def _backfill_risk_snapshots(engine: Engine, connection: Any) -> int | None:
    if not _table_exists(engine, "hedge_account_risk_snapshots"):
        return None
    from freqtrade.persistence.hedge_contracts import stable_fact_key
    ...
    for row in rows:
        ...
        fact_key = stable_fact_key(
            "risk", row["exchange"], row["account_id"], row["source"], row["source_event_time"]
        )
        connection.execute(
            text(
                "UPDATE hedge_account_risk_snapshots SET fact_key=:fact_key, "
                "equity=COALESCE(NULLIF(equity,'0'), margin_balance), "
                "gross_long_notional=:gross_long, gross_short_notional=:gross_short, "
            ...
            {
                "fact_key": fact_key,
                "gross_long": format(max((gross + net) / Decimal(2), Decimal(0)), "f"),
                "gross_short": format(max((gross - net) / Decimal(2), Decimal(0)), "f"),
                "id": row["id"],
            },
        )
    return len(rows)
...
def _backfill_account_events(engine: Engine, connection: Any) -> int | None:
    if not _table_exists(engine, "hedge_account_events"):
        return None
    from freqtrade.persistence.hedge_contracts import canonical_symbol, stable_fact_key
    ...
    for row in rows:
        ...
        connection.execute(
            text(
                ...
                "symbol=CASE WHEN symbol IS NULL THEN NULL ELSE :symbol END WHERE id=:id"
            ),
...
def _backfill_strategy_states(engine: Engine, connection: Any) -> int | None:
    if not _table_exists(engine, "hedge_strategy_side_states"):
        return None
    result = connection.execute(
        text(
...
def _backfill_outbox(engine: Engine, connection: Any) -> int | None:
    if not _table_exists(engine, "hedge_event_outbox"):
        return None
    rows = (
        connection.execute(
            text(
...
def _step_backfill_v2_contracts(engine: Engine) -> dict[str, Any]:
    handlers = (
        ("order_intents", _backfill_order_intents),
        ("order_snapshots", _backfill_order_snapshots),
        ("fills", _backfill_fills),
        ("position_snapshots", _backfill_position_snapshots),
        ("risk_snapshots", _backfill_risk_snapshots),
        ("account_events", _backfill_account_events),
        ("strategy_states", _backfill_strategy_states),
        ("outbox", _backfill_outbox),
    )
    changed: dict[str, int] = {}
    with engine.begin() as connection:
        for name, handler in handlers:
            result = handler(engine, connection)
...
def _step_rebuild_strategy_identity(engine: Engine) -> dict[str, Any]:
    ...
    with engine.begin() as connection:
        ...
        for row in connection.exec_driver_sql(f"PRAGMA index_list({old_table})").mappings():
            ...
            if not name.startswith("sqlite_autoindex_"):
                connection.execute(text(f"DROP INDEX IF EXISTS {_quote(engine, name)}"))
        _table(StrategySideState).create(connection, checkfirst=False)
        new_columns = [column.name for column in _table(StrategySideState).columns]
...
def _step_scope_managed_trade_slots(engine: Engine) -> dict[str, Any]:
    ...
    with engine.begin() as connection:
        ...
        connection.execute(
            text(
                "UPDATE trades SET open_slot_key=NULL WHERE is_open="
                + ("0" if engine.dialect.name == "sqlite" else "FALSE")
            )
        )
...
def _step_create_stage_a_durable_paper_tables(engine: Engine) -> dict[str, Any]:
    ...
    _table(PaperRuntimeCheckpointRow).create(engine, checkfirst=True)
    _table(ActionGroupRow).create(engine, checkfirst=True)
    return {
        "tables": (
...
def _step_verify_stage_a_durable_paper_tables(engine: Engine) -> dict[str, Any]:
    ...
    checkpoint_uniques = {
        ...
        for item in inspect(engine).get_unique_constraints("hedge_paper_runtime_checkpoints")
    }
...
def _step_create_risk_approval_commits(engine: Engine) -> dict[str, Any]:
    ...
    _table(RiskApprovalCommitRow).create(engine, checkfirst=True)
...
def _step_verify_risk_approval_commits(engine: Engine) -> dict[str, Any]:
    ...
    if missing:
        raise HedgeMigrationError(f"Durable risk approval commit columns are missing: {missing}")
    unique_sets = {
        tuple(sorted(item.get("column_names") or ()))
...
def _step_create_control_operations(engine: Engine) -> dict[str, Any]:
    ...
    _table(ControlOperationRow).create(engine, checkfirst=True)
...
def _step_verify_control_operations(engine: Engine) -> dict[str, Any]:
    ...
    missing = sorted(required - _column_names(engine, table))
    if missing:
        raise HedgeMigrationError(f"Durable control operation columns are missing: {missing}")
    unique_sets = {
        tuple(sorted(item.get("column_names") or ()))
        for item in inspect(engine).get_unique_constraints(table)
    }
...
def _step_add_control_operation_leases(engine: Engine) -> dict[str, Any]:
    ...
    added = {
        "lease_owner": _add_column_if_missing(engine, table, "lease_owner", "VARCHAR(128)"),
        "lease_expires_at": _add_column_if_missing(engine, table, "lease_expires_at", "DATETIME"),
    }
...
def _step_verify_control_operation_leases(engine: Engine) -> dict[str, Any]:
    ...
    missing = sorted(required - _column_names(engine, table))
    if missing:
        raise HedgeMigrationError(f"Durable control operation lease columns are missing: {missing}")
    return {"table": table, "verified": True}
...
def _step_create_r2_execution_authority(engine: Engine) -> dict[str, Any]:
    ...
    _table(ExecutionOrderStateRow).create(engine, checkfirst=True)
    _table(ExecutionIdempotencyRow).create(engine, checkfirst=True)
    return {
        "tables": (
...
def _step_verify_r2_execution_authority(engine: Engine) -> dict[str, Any]:
    ...
    order_uniques = {
        tuple(sorted(item.get("column_names") or ()))
        for item in inspect(engine).get_unique_constraints("hedge_execution_order_states")
    }
    ...
    idempotency_foreign_keys = inspect(engine).get_foreign_keys("hedge_execution_idempotency")
    if not any(
...
def _step_create_execution_authority_tables(engine: Engine) -> dict[str, Any]:
    ...
    for model in tables:
        _table(model).create(engine, checkfirst=True)
