...
class HedgePersistenceService:
    ...
    def transition_order_intent(self, **values: Any):
        return self.transaction(lambda repository: repository.transition_order_intent(**values))
    ...
    def record_position_snapshot(self, **values: Any):
        return self.transaction(lambda repository: repository.append_position_snapshot(**values))
    ...
    def update_strategy_side_state(self, **values: Any):
        return self.transaction(lambda repository: repository.upsert_strategy_side_state(**values))

    def rebuild_current_positions(self, **values: Any) -> LedgerRecovery:
        return self.transaction(lambda repository: repository.rebuild_current_projections(**values))
    ...
    def update_core_position_state(self, **values: Any):
        return self.transaction(lambda repository: repository.upsert_core_position_state(**values))
