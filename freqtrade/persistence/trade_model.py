...
from freqtrade.enums import ExitType, PositionSide, TradingMode
...
from freqtrade.hedge.compatibility import (
    ...
    normalize_compat_position_side,
)
...
class Order(ModelBase):
    ...
    # Hedge-T persistence identity. Defaults preserve one-way databases.
    position_side: Mapped[str] = mapped_column(
        String(10),
        nullable=False,
        default=PositionSide.BOTH.value,
        server_default=PositionSide.BOTH.value,
        index=True,
    )
    position_action: Mapped[str | None] = mapped_column(String(16), nullable=True, index=True)
    action_group_id: Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)

    action: Mapped[str | None] = mapped_column(String(32), nullable=True)
    client_order_id: Mapped[str | None] = mapped_column(String(255), nullable=True)
    idempotency_key: Mapped[str | None] = mapped_column(String(255), nullable=True, index=True)
    submit_state: Mapped[str | None] = mapped_column(String(32), nullable=True)
    ...
    def to_json(self, entry_side: str, minified: bool = False) -> dict[str, Any]:
        ...
        position_side = normalize_compat_position_side(self.position_side)
        explicit_hedge_order = is_explicit_hedge_order(self)
        if explicit_hedge_order:
            resp.update(
                {
...
class LocalTrade:
    ...
    # Side-aware open-trade index. The legacy pair index remains available.
    bt_trades_open_pps: ClassVar[dict[tuple[str, str], list["LocalTrade"]]] = defaultdict(list)
    ...
    is_short: bool = False
    position_side: str = PositionSide.BOTH.value
    leverage: float = 1.0
    ...
    open_slot_key: str | None = None
    hedge_version: int = 0

    def refresh_hedge_identity(self) -> None:
        """Normalize optional Hedge identity without changing legacy trade semantics.

        Ordinary Freqtrade trades remain ``BOTH`` and do not reserve a Hedge slot.
        Only callers that explicitly construct a LONG/SHORT trade are treated as
        Hedge-managed. ``open_slot_key`` is the sole database uniqueness marker.
        """

        self.account_id = str(getattr(self, "account_id", None) or "default").strip() or "default"
        raw_side = getattr(self, "position_side", PositionSide.BOTH.value)
        side = str(getattr(raw_side, "value", raw_side) or PositionSide.BOTH.value).upper()
        if side not in {"LONG", "SHORT", "BOTH"}:
            side = PositionSide.BOTH.value

        managed = side in {"LONG", "SHORT"}
        self.position_side = side
        self.hedge_version = 2 if managed else 0
        self.open_slot_key = (
            f"{self.account_id}|{self.pair}|{side}"
            if managed and bool(getattr(self, "is_open", False)) and getattr(self, "pair", None)
            else None
        )

    @property
    def position_key(self) -> tuple[str, str]:
        """Stable local identity for one-way and hedge-mode positions."""
        return (self.pair, self.position_side or PositionSide.BOTH.value)
    ...
    def __repr__(self):
        ...
        position_side = self.position_side or PositionSide.BOTH.value
        side_repr = (
            f"position_side={position_side}, " if position_side != PositionSide.BOTH.value else ""
        )
        return (
    ...
    def to_json(self, minified: bool = False) -> dict[str, Any]:
        ...
        position_side = normalize_compat_position_side(self.position_side)
        if is_explicit_hedge_trade(self):
            result.update(
                {
                    "account_id": self.account_id,
                    "position_side": position_side,
                    "open_slot_key": self.open_slot_key,
                    "hedge_version": self.hedge_version,
                }
            )
        return result
    ...
    def reset_trades() -> None:
        ...
        LocalTrade.bt_trades_open_pps.clear()
    ...
    def close_bt_trade(trade):
        ...
        LocalTrade.bt_trades_open_pps[trade.position_key].remove(trade)
        if not LocalTrade.bt_trades_open_pps[trade.position_key]:
            del LocalTrade.bt_trades_open_pps[trade.position_key]
    ...
    def add_bt_trade(trade):
        if trade.is_open:
            ...
            LocalTrade.bt_trades_open_pps[trade.position_key].append(trade)
            LocalTrade.bt_open_open_trade_count += 1
        else:
            LocalTrade.bt_trades.append(trade)
    ...
    @classmethod
    def get_trades_proxy_by_position(
        cls,
        *,
        pair: str | None = None,
        ...
        # Query trades while preserving pair + position-side identity.
        trades = cls.get_trades_proxy(
            pair=pair,
            is_open=is_open,
            open_date=open_date,
        ...
        normalized_side = (
            ...
            else str(position_side).upper()
        )
        return [
            trade
            for trade in trades
            if getattr(trade, "position_side", PositionSide.BOTH.value) == normalized_side
        ]

    @classmethod
    def get_open_trades_for_pair_side(
        cls,
        pair: str,
        ...
        if not cls.use_db:
            return list(LocalTrade.bt_trades_open_pps.get((pair, normalized_side), []))
        return cls.get_trades_proxy_by_position(
            pair=pair,
    ...
    @classmethod
    def get_open_trade_for_pair_side(
        cls,
        pair: str,
        position_side: PositionSide | str,
    ) -> "LocalTrade | None":
        trades = cls.get_open_trades_for_pair_side(pair, position_side)
        if len(trades) > 1:
            raise OperationalException(
                "More than one open trade exists for the same pair and position side."
            )
        return trades[0] if trades else None

    @classmethod
    def assert_single_open_trade_per_side(cls, pair: str) -> None:
        for side in (
            PositionSide.LONG,
            PositionSide.SHORT,
            PositionSide.BOTH,
        ):
            if len(cls.get_open_trades_for_pair_side(pair, side)) > 1:
                raise OperationalException(
                    f"Duplicate open trade for {pair} and position side {side.value}."
                )
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        trade = cls(
            ...
            position_side=data.get("position_side", PositionSide.BOTH.value),
            ...
            open_slot_key=data.get("open_slot_key"),
            hedge_version=data.get("hedge_version", 1),
        )
        for order in data["orders"]:
            order_obj = Order(
                ...
                position_side=order.get(
                    "position_side", data.get("position_side", PositionSide.BOTH.value)
                ),
                position_action=order.get("position_action"),
                action_group_id=order.get("action_group_id"),
                action=order.get("action"),
                client_order_id=order.get("client_order_id"),
                idempotency_key=order.get("idempotency_key"),
                submit_state=order.get("submit_state"),
            )
...
class Trade(ModelBase, LocalTrade):
    ...
    use_db: bool = True
    # Keep the side-aware backtest index visible on both LocalTrade and Trade.
    # The upstream object-identity invariant compares vars() on both classes.
    bt_trades_open_pps: ClassVar[dict[tuple[str, str], list["LocalTrade"]]] = (
        LocalTrade.bt_trades_open_pps
    )
    ...
    open_slot_key: Mapped[str | None] = mapped_column(String(512), nullable=True)
    hedge_version: Mapped[int] = mapped_column(Integer, nullable=False, default=0)

    # Keep classmethod parity with LocalTrade.  Upstream validates that every
    # non-property/non-function LocalTrade attribute is explicitly represented
    # on Trade.  These thin delegates retain Trade's database-aware
    # get_trades_proxy implementation while exposing the Hedge side helpers in
    # Trade.__dict__.
    @classmethod
    def get_trades_proxy_by_position(
        cls,
        *,
        ...
        return super().get_trades_proxy_by_position(
            pair=pair,
            position_side=position_side,
            is_open=is_open,
            open_date=open_date,
    ...
    def get_open_trades_for_pair_side(
        ...
        return super().get_open_trades_for_pair_side(pair, position_side)
    ...
    def get_open_trade_for_pair_side(
        ...
        return super().get_open_trade_for_pair_side(pair, position_side)

    @classmethod
    def assert_single_open_trade_per_side(cls, pair: str) -> None:
        super().assert_single_open_trade_per_side(pair)

    def __init__(self, **kwargs):
        from_json = kwargs.pop("__FROM_JSON", None)
        super().__init__(**kwargs)
        # SQLAlchemy column defaults are applied at flush time, not necessarily
        # during direct construction.  Normalize Hedge identity immediately so
        # explicit LONG/SHORT trades have a durable version and active slot key
        # before JSON/RPC serialization or wallet lookup.  Native BOTH trades
        # remain un-managed and preserve the upstream schema.
        self.refresh_hedge_identity()
        if not from_json:
