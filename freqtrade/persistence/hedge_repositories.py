...
def _validate_action_direction(
    ...
    *,
    ...
    increases_position = _signed_quantity(position_side, order_side, Decimal(1)) > 0
...
def _validate_position_snapshot_values(
    *,
    ...
    if entry_price < 0 or (quantity > 0 and entry_price == 0):
...
def _raise_immutable_conflict(label: str, key: str, mismatches: list[str]) -> None:
    raise ValueError(f"{label} conflict for key {key!r}; changed fields: {', '.join(mismatches)}")
...
class HedgeLedgerRepository:
    ...
    def transition_order_intent(
        ...
        *,
        ...
        row = self.session.scalar(select(OrderIntent).where(OrderIntent.intent_id == normalized_id))
        if row is None:
    ...
    def append_order_snapshot(
        ...
        *,
        ...
        expected = {
            ...
            "is_terminal": (
                normalized_status in TERMINAL_ORDER_STATUSES if is_terminal is None else is_terminal
            ),
    ...
    def append_position_snapshot(
        ...
        *,
        ...
        _validate_position_snapshot_values(
            sequence_number=sequence_number,
            quantity=normalized_quantity,
            entry_price=normalized_entry_price,
            leverage=normalized_leverage,
            mark_price=normalized_mark_price,
            notional=normalized_notional,
            liquidation_price=normalized_liquidation,
        )
        ...
        expected = {
            ...
            "notional": (
                canonical_decimal(normalized_notional) if normalized_notional is not None else None
            ),
        ...
        self._enqueue(
            ...
            aggregate_id=_bounded_key("position", exchange, account_id, canonical, normalized_side),
    ...
    def append_account_risk_snapshot(
        ...
        *,
        ...
        gross_total_input = decimal_value(gross_exposure) if gross_exposure is not None else None
        ...
        gross_total = (
            gross_total_input if gross_total_input is not None else gross_long + gross_short
        )
        wallet_value = decimal_value(wallet_balance)
        margin_value = decimal_value(margin_balance)
        equity_value = (
            decimal_value(equity) if equity is not None else max(wallet_value, margin_value)
        )
        numbers = {
            "equity": equity_value,
    ...
    def add_reconciliation_diff(
        ...
        *,
        ...
        row = ReconciliationDiff(
            ...
            resolved_at=(utcnow() if resolution.upper() not in {"UNRESOLVED", "PENDING"} else None),
        )
        self.session.add(row)
    ...
    def upsert_strategy_side_state(
        ...
        *,
        ...
        if row is None:
            raise RuntimeError("strategy side state disappeared during optimistic update")
        compare_revision = row.revision if expected_revision is None else expected_revision
        now = utcnow()
        result = self.session.execute(
        ...
        if cast(CursorResult[Any], result).rowcount != 1:
            ...
            raise ValueError(
                f"Strategy side state revision mismatch: expected {compare_revision}, found {found}"
            )
        self.session.expire_all()
        updated_row = load()
    ...
    def upsert_core_position_state(
        ...
        *,
        ...
        self._enqueue(
            ...
            aggregate_id=_bounded_key("core", exchange, account_id, symbol, position_side),
    ...
    def apply_fill(
        ...
        *,
        ...
        self.append_order_snapshot(
            ...
            exchange_order_id=exchange_order_id,
            client_order_id=(
                client_order_id or (current_order.client_order_id if current_order else None)
            ),
            intent_id=intent_id,
            correlation_id=normalized_correlation,
            side=normalized_order_side,
        ...
        if fill_fee > 0 and fee_currency:
            self.record_account_event(
                event_key=_bounded_key("fill-fee", exchange, account_id, symbol, exchange_trade_id),
                account_id=account_id,
                exchange=exchange,
                event_type="FEE",
                asset=fee_currency,
        ...
        self._enqueue(
            ...
            aggregate_id=event_id,
            event_type=(
                "hedge.fill.projection_blocked" if projection_blocked else "hedge.fill.accepted"
            ),
            payload={
                "event_id": event_id,
                "exchange": exchange,
    ...
    def _duplicate_fill_result(
        ...
        *,
        ...
        return FillApplyResult(
            ...
            action=existing.action or _default_fill_action(existing.position_side, existing.side),
            position_quantity=position.quantity if position else "0",
            position_entry_price=position.entry_price if position else "0",
            cumulative_order_quantity=cumulative_quantity,
            cumulative_order_quote=cumulative_quote,
    ...
    def _rebuild_position_scope(
        ...
        *,
        ...
        base_sequence = fact.sequence_number if fact and fact.sequence_number is not None else -1
        ...
        for row in fills:
            ...
            projected_quantity = quantity + _signed_quantity(position_side, row.side, fill_quantity)
    ...
    def recover_projection(
        ...
        *,
        ...
        for fact in fact_rows:
            position_key = (fact.exchange, fact.symbol, fact.position_side)
            previous_projection = projection.get(position_key)
            fact_sequence = fact.sequence_number if fact.sequence_number is not None else -1
            if previous_projection is None or (
                fact.source_event_time,
                fact_sequence,
                fact.id,
            ) >= (
                previous_projection["base_time"],
                previous_projection["base_sequence"],
                previous_projection["base_id"],
            ):
                projection[position_key] = {
        ...
        for fill in fill_rows:
            fill_key = (fill.exchange, fill.symbol, fill.position_side)
            ...
            if projected < 0:
                entity_key = f"{fill.exchange}|{account_id}|{fill.symbol}|{fill.position_side}"
                raise ProjectionInvariantError(
            ...
            if state["mark_price"] is not None:
                state["notional"] = canonical_decimal(quantity * decimal_value(state["mark_price"]))
    ...
    def rebuild_current_projections(
        ...
        *,
        ...
        for order in recovery.orders:
            ...
            if current_order is not None and all(
                getattr(current_order, attr_name) == value for attr_name, value in values.items()
            ):
                continue
            if current_order is None:
                current_order = CurrentOrderProjection(
                    exchange=order.exchange,
                    account_id=account_id,
                    symbol=order.symbol,
                    position_side=order.position_side,
                ...
                for attr_name, value in values.items():
                    setattr(current_order, attr_name, value)
            current_order.revision += 1
            current_order.updated_at = utcnow()
            changed += 1
        ...
        for stale_order in self.session.scalars(stale_stmt).all():
            stale_key = (
                stale_order.exchange,
                stale_order.symbol,
                stale_order.position_side,
                stale_order.exchange_order_id,
            )
            if stale_key not in recovered_order_keys:
                self.session.delete(stale_order)
                changed += 1
        ...
        if changed:
            self._enqueue(
                ...
                metadata=EventMetadata(
                    correlation_id=_bounded_key("recovery", account_id, canonical_filter or "*")
                ),
            )
        self.session.flush()
        return recovery
    ...
    def _enqueue(
        ...
        *,
        ...
        bounded_aggregate_id = (
            aggregate_id if len(aggregate_id) <= 255 else _bounded_key(aggregate_type, aggregate_id)
        )
        ...
        row = EventOutbox(
            ...
            exchange_time=(_naive_utc(metadata.exchange_time) if metadata.exchange_time else None),
            observed_time=(
                _naive_utc(metadata.observed_time) if metadata.observed_time else utcnow()
            ),
