...
def migrate_hedge_position_columns(engine: Engine) -> tuple[str, ...]:
    """Run only the additive Hedge compatibility-column migration.

    H3 ledger migrations remain outside the native Freqtrade migration hook and
    are started only by the optional Hedge bootstrap path.
    """

    from freqtrade.persistence.hedge_bootstrap import migrate_hedge_compatibility_columns

    return migrate_hedge_compatibility_columns(engine)
...
def migrate_trades_and_orders_table(
    ...
    account_id = (
        "coalesce(account_id, 'default')" if has_column(cols, "account_id") else "'default'"
    )
    position_side = (
        "coalesce(position_side, 'BOTH')" if has_column(cols, "position_side") else "'BOTH'"
    )
    hedge_version = "coalesce(hedge_version, 0)" if has_column(cols, "hedge_version") else "0"
    open_slot_key = (
        "case when coalesce(hedge_version, 0) >= 2 then open_slot_key else null end"
...
def migrate_orders_table(engine, table_back_name: str, cols_order: list):
    ...
    position_side = (
        "coalesce(position_side, 'BOTH')" if has_column(cols_order, "position_side") else "'BOTH'"
    )
...
def check_migrate(engine: Engine, decl_base, previous_tables: list[str]) -> None:
    """Run only Freqtrade's native database migrations.

    Optional Hedge H3 migrations are intentionally excluded from this hook and
    run only after Hedge configuration has passed its feature gate.
    """
