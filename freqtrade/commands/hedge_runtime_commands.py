...
def _validate_paper_runtime_config(config: dict[str, Any]) -> None:  # noqa: C901
    ...
    if pair and whitelist and pair not in whitelist:
        raise OperationalException("hedge managed_pair must be present in exchange.pair_whitelist")
    ...
    if (
        ...
        raise OperationalException("target_leverage must equal hedge.paper.leverage")
    max_open = config.get("max_open_trades")
    if max_open not in (None, 1):
        raise OperationalException("single-pair hedge-paper currently requires max_open_trades=1")
    if not paper.ephemeral:
        ...
        if paper.ohlcv_source is not PaperOhlcvSource.DATAPROVIDER:
            raise OperationalException("durable hedge-paper requires DataProvider OHLCV")
        if paper.funding_source is not PaperFundingSource.EXCHANGE:
            raise OperationalException("durable hedge-paper requires exchange funding events")
        if not paper.account_events_enabled:
            raise OperationalException("durable hedge-paper requires account_events_enabled=true")
...
def start_hedge_research_optimize(args: dict[str, Any]) -> None:
    ...
    try:
        ...
        raise OperationalException(f"invalid Hedge optimization configuration: {exc}") from exc
    ...
    summary = {
        ...
        "parallel_backend": run.parallel_backend,
        "peak_workers": run.peak_workers,
        "resource_samples": list(run.resource_samples),
