...
def _assert_readonly_config(config: Mapping[str, Any]) -> None:
    ...
    if hedge.get("live_trading_enabled", False) is not False:
        raise OperationalException("hedge-readonly-check requires hedge.live_trading_enabled=false")
...
async def _run_preflight(
    ...
    *,
    ...
    try:
        ...
        permission = await runtime.client.preflight_permissions(runtime.config.permission_policy)
        fill_start_time_ms = None
        if include_history:
            fill_start_time_ms = int(
                (runtime.clock.now() - runtime.config.fill_lookback).timestamp() * 1000
            )
...
def start_hedge_readonly_check(args: dict[str, Any]) -> int:
    ...
    if result["status"] != "PASS":
        raise OperationalException(
            "Binance Hedge read-only preflight halted: " + ",".join(result["failure_reasons"])
        )
    return 0
