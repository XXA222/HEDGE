...
def legacy_position_projection(
    ...
    return {symbol: positions[0] for symbol, positions in grouped.items() if len(positions) == 1}
...
class Wallets:
    ...
    def get_free(self, currency: str) -> float:
        ...
        return (
            0.0
            if balance is None
            else _finite_float(balance.free, field_name=f"wallet[{currency}].free")
        )
    ...
    def get_used(self, currency: str) -> float:
        ...
        return (
            0.0
            if balance is None
            else _finite_float(balance.used, field_name=f"wallet[{currency}].used")
        )
    ...
    def get_total(self, currency: str) -> float:
        ...
        return (
            0.0
            if balance is None
            else _finite_float(balance.total, field_name=f"wallet[{currency}].total")
        )
    ...
    def get_position_risk_legs(
        ...
        *,
        ...
        from freqtrade.enums.hedge import PositionSide as HedgePositionSide
        from freqtrade.hedge.risk.portfolio import PositionRiskLeg
        ...
        for (symbol, side), position in self.get_all_positions_by_side().items():
            ...
            legs.append(
                PositionRiskLeg(
                    ...
                    symbol=symbol,
                    position_side=HedgePositionSide(str(side).upper()),
                    quantity=quantity,
                    mark_price=mark_price,
                    leverage=leverage,
                    reported_initial_margin=Decimal(str(position.validated_collateral)),
                    maintenance_margin=Decimal(str(maintenance)),
                    liquidation_price=(None if liquidation is None else Decimal(str(liquidation))),
                )
            )
        return tuple(legs)
    ...
    def _update_live(self) -> None:
        ...
        for position in positions:
            ...
            leverage: float | None = (
                None if raw_leverage is None else _finite_float(raw_leverage, field_name="leverage")
            )
            if not leverage:
            ...
            wallet = PositionWallet(
                ...
                unrealized_pnl=_finite_float(
                    position.get("unrealizedPnl") or 0.0,
                    field_name="unrealizedPnl",
                ),
            )
            self._positions_by_side[position_wallet_key(symbol, position["side"])] = wallet
