...
def authorized_only(command_handler: Callable[..., Coroutine[Any, Any, None]]):
    ...
    async def wrapper(self, *args, **kwargs) -> None:
        ...
        if cchat_id != chat_id:
            ...
            return None
        if (topic_id := self._config["telegram"].get("topic_id")) is not None and str(
            ctopic_id
        ) != topic_id:
    ...
    async def _profit_handler(
        ...
        try:
            if context.args:
                ...
                # Trade timestamps are stored in UTC.  Using the host's local date here
                # drops valid UTC-today trades around midnight in positive-offset zones.
                today_start = datetime.combine(datetime.now(tz=UTC).date(), datetime.min.time())
