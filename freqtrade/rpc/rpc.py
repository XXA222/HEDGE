...
from freqtrade.hedge.safety import assert_native_exchange_write_allowed
...
class RPC:
    ...
    def _assert_native_exchange_write_allowed(self, operation: str) -> None:
        try:
            assert_native_exchange_write_allowed(self._config, operation=operation)
    ...
    def _force_entry_validations(self, pair: str, order_side: SignalDirection):
        self._assert_native_exchange_write_allowed("RPC_FORCE_ENTRY")
    ...
    def _rpc_cancel_open_order(self, trade_id: int):
        self._assert_native_exchange_write_allowed("RPC_CANCEL_OPEN_ORDER")
        if self._freqtrade.state == State.STOPPED:
            raise RPCException("trader is not running")
        with self._freqtrade._exit_lock:
            # Query for trade
