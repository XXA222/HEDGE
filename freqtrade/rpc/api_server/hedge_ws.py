...
class HedgeWsPrincipal:
    ...
    def __post_init__(self) -> None:
        ...
        if accounts is not None:
            normalized = frozenset(value for item in accounts if (value := str(item).strip()))
            if not normalized:
                ...
                if queue.full():
                    try:
                        ...
                        start = (
                            removed.sequence
                            if current is None
                            else min(current[0], removed.sequence)
                        )
                        end = (
                            removed.sequence
                            if current is None
                            else max(current[1], removed.sequence)
                        )
...
def _auth_timeout_seconds(auth_timeout_seconds: float) -> float:
    if (
    ...
    return float(auth_timeout_seconds)


def _after_sequence(websocket: WebSocket) -> int | None:
...
async def _serve_websocket_session(
    websocket: WebSocket,
    hub: HedgeEventHub,
    principal: HedgeWsPrincipal,
) -> None:
    try:
        after_sequence = _after_sequence(websocket)
...
def create_hedge_ws_router(
    ...
    *,
    ...
    auth_timeout = _auth_timeout_seconds(auth_timeout_seconds)
    ...
    async def websocket_endpoint(websocket: WebSocket) -> None:
        ...
        await _serve_websocket_session(websocket, hub, principal)
