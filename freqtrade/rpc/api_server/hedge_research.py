...
def _register_read_routes(
    ...
    *,
    ...
    _register_read_overview_routes(router, service=service, viewer=viewer)
    _register_read_job_routes(router, service=service, viewer=viewer)
    _register_read_experiment_routes(router, service=service, viewer=viewer)
    _register_read_pipeline_routes(router, service=service, viewer=viewer)


def _register_read_overview_routes(
    router: APIRouter,
    *,
    service: HedgeResearchService,
    viewer: Callable[..., HedgePrincipal],
...
def _register_read_job_routes(
    ...
    *,
    ...
    def job(job_id: str, _: HedgePrincipal = Depends(viewer)) -> dict[str, Any]:
        payload = _research_call(service.get, job_id, not_found=(KeyError,))
        payload["runtime"] = _research_call(service.runtime, job_id, not_found=(KeyError,))
        return payload

    @router.get("/jobs/{job_id}/runtime")
    def runtime(job_id: str, _: HedgePrincipal = Depends(viewer)) -> dict[str, Any]:
        return _research_call(service.runtime, job_id, not_found=(KeyError,))
    ...
    def log_tail(
        ...
        return _research_call(service.log_tail, job_id, lines=lines, not_found=(KeyError,))
    ...
    def optimization_replays(
        ...
        _research_call(service.get, job_id, not_found=(KeyError,))
        return _research_call(
            service.optimization_replays,
            job_id,
            not_found=(KeyError,),
            conflict=(ValueError,),
        )
...
def _register_read_experiment_routes(
    router: APIRouter,
    *,
    service: HedgeResearchService,
    viewer: Callable[..., HedgePrincipal],
    ...
    def experiment(
        ...
        return _research_call(
            ...
            experiment_id,
            refresh=refresh,
            not_found=(FileNotFoundError, KeyError, ValueError),
        )
    ...
    def experiment_tensorboard(
        ...
        return _research_call(
            service.experiment_tensorboard,
            experiment_id,
...
def _register_read_pipeline_routes(
    router: APIRouter,
    *,
    service: HedgeResearchService,
    viewer: Callable[..., HedgePrincipal],
    ...
    def pipeline(
        ...
        return _research_call(
            service.pipeline,
            pipeline_id,
            not_found=(KeyError, RuntimeError, ValueError),
        )
...
def _register_pipeline_routes(
    ...
    *,
    ...
    def submit_pipeline(
        ...
        payload["training_kind"] = ResearchKind(str(payload["training_kind"]))
        payload["tags"] = tuple(str(item) for item in payload.get("tags", ()))
        spec = ResearchPipelineSpec(**payload, promotion_policy=policy)
        return _research_call(
            service.submit_pipeline,
            spec,
            auto_start=auto_start,
            conflict=(RuntimeError, TypeError, ValueError),
        )
    ...
    *,
    ...
    def submit(
        ...
        if request.auto_execute:
            try:
                payload["runtime"] = service.execute(str(payload["job_id"]))
    ...
    def replay_best(
        ...
        return _research_call(
            service.replay_best_optimization,
            job_id,
            timerange=request.timerange,
            auto_execute=request.auto_execute,
            name=request.name,
            not_found=(FileNotFoundError, KeyError),
            conflict=(RuntimeError, TypeError, ValueError),
        )
    ...
    def replay_top(
        ...
        return _research_call(
            service.replay_top_optimization,
            job_id,
            limit=request.limit,
            timerange=request.timerange,
            auto_execute=request.auto_execute,
    ...
    def execute(
        ...
        return _research_call(
            ...
            job_id,
            not_found=(KeyError,),
            conflict=(RuntimeError, ValueError),
        )
