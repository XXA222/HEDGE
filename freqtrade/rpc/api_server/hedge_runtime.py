...
from freqtrade.hedge.exchange.base import OrderFact
...
class _ProjectionMetadata(TypedDict):
    source: Literal["EXCHANGE", "PAPER", "LIVE", "SHADOW"]
    source_version: str
    sequence: int
    event_time: datetime | None
    observed_at: datetime
    stale: bool
    operation_mode: Literal["readonly", "paper", "shadow", "live"]


_ReadonlyStatus = Literal[
    "PREPARED",
    "SUBMITTING",
    "ACKNOWLEDGED",
    "PARTIAL",
...
_ReadonlyAction = Literal["OPEN", "INCREASE", "REDUCE", "CLOSE"]
...
class HedgeRuntimeQuery:
    ...
    def _runtime_from_rpc() -> HedgeRuntime:
        ...
        if not isinstance(runtime, HedgeRuntime):
            raise TypeError("Hedge runtime is not attached")
        return runtime
    ...
    def _metadata(self, view) -> _ProjectionMetadata:
        return {
            "source": cast(Literal["EXCHANGE", "PAPER", "LIVE", "SHADOW"], view.source.value),
            ...
            "stale": view.stale,
            "operation_mode": cast(
                Literal["readonly", "paper", "shadow", "live"],
...
class HedgeExecutionRuntimeQuery:
    ...
    @staticmethod
    def _readonly_status(value: str) -> _ReadonlyStatus:
    ...
    @staticmethod
    def _readonly_action(order: OrderFact) -> _ReadonlyAction:
        side = order.side.upper()
        position_side = order.position_side.upper()
        increases = (position_side == "LONG" and side == "BUY") or (
            position_side == "SHORT" and side == "SELL"
        )
        return cast(_ReadonlyAction, "INCREASE" if increases else "REDUCE")
    ...
    def _readonly_order_schema(self, order: OrderFact) -> ExecutionOrderSchema:
        requested = Decimal(str(order.original_quantity))
        filled = Decimal(str(order.cumulative_filled_quantity))
        raw = order.raw
        limit_price = None
        if isinstance(raw, Mapping):
            limit_price = self._positive_decimal(raw.get("price"))
        average = self._positive_decimal(order.average_price)
        observed = order.observed_at
        status = self._readonly_status(str(order.status))
        ...
        return ExecutionOrderSchema(
            ...
            symbol=canonicalize_symbol(str(order.symbol)),
            position_side=cast(Literal["LONG", "SHORT"], str(order.position_side).upper()),
    ...
    def orders(
        ...
        if adapter is not None:
            return adapter.orders(account_id=account_id, symbol=symbol, status=status, limit=limit)
        ...
        if view is not None and view.account_id == account_id:
            for item in view.active_orders:
                ...
                if (
                    ...
                    and canonicalize_symbol(model.symbol) != requested_symbol
                ):
                    continue
    ...
    def pair_summary(self, *, account_id: str, symbol: str) -> PairSummaryResponse:
        ...
        pending_entry = sum(
            (
                ...
                if item.action in {"OPEN", "INCREASE"}
            ),
            Decimal(0),
        )
        pending_reduce = sum(
            (
                item.remaining_quantity
                for item in orders.orders
                if item.action in {"REDUCE", "CLOSE"}
            ),
            Decimal(0),
        )
