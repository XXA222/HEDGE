...
def _validate_json_mapping_value(
...
def _validate_json_sequence_value(
...
def _validate_json_value(
    ...
    *,
    ...
    if isinstance(value, dict):
        return _validate_json_mapping_value(
            value,
            field_name=field_name,
            depth=depth,
            budget=budget,
    ...
    if isinstance(value, (list, tuple)):
        return _validate_json_sequence_value(
            value,
            field_name=field_name,
            depth=depth,
            budget=budget,
...
class RiskStatusSchema(ProjectionMetadataSchema):
    ...
    def _reasons(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        ...
        return tuple(_validate_text(item, field_name="reason", limit=256) for item in value)
...
class ReconciliationStatusSchema(ProjectionMetadataSchema):
    ...
    def _details(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        ...
        return tuple(_validate_text(item, field_name="detail", limit=1024) for item in value)
...
class ReadinessStatusSchema(ProjectionMetadataSchema):
    ...
    def _locks(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        ...
        normalized = tuple(
            _validate_text(item, field_name="unknown leg lock", limit=256) for item in value
        )
    ...
    def _checks(cls, value: object) -> dict[str, bool]:
        ...
        for raw_key, raw_value in value.items():
            ...
            if not isinstance(raw_value, bool):
                # Pydantic v2 turns ValueError into ValidationError; TypeError escapes.
                raise ValueError("readiness check values must be booleans")  # noqa: TRY004
    ...
    def _reasons(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        ...
        return tuple(
            _validate_text(item, field_name="readiness reason", limit=512) for item in value
        )
