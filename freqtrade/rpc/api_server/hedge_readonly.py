...
class HedgeReadonlyQueryPort(Protocol):
    ...
    def readiness(self, *, account_id: str, source: str | None = None) -> ReadinessStatusSchema: ...
    ...
    def events(self, *, account_id: str, limit: int) -> HedgeEventListResponse:
        records = tuple(
            HedgeEventRecordSchema(
                ...
                payload=cast(dict[str, JsonValue], dict(item.payload)),
            )
            for item in self._ledger.audit(limit=limit)
            if str(item.payload.get("account_id", account_id)) == account_id
        )
        return HedgeEventListResponse(events=records, count=len(records), as_of=datetime.now(UTC))
...
def _register_query_routes(
    ...
    authenticated: Callable[..., HedgePrincipal],
) -> None:
...
def _register_execution_routes(
    router: APIRouter,
    execution_query: HedgeExecutionReadonlyQueryPort,
    authenticated: Callable[..., HedgePrincipal],
) -> None:
    ...
    @router.get("/orders/{client_order_id}", response_model=ExecutionOrderSchema)
    def execution_order(
        client_order_id: str,
        _: HedgePrincipal = Depends(authenticated),
    ) -> ExecutionOrderSchema:
    ...
    @router.get("/action-groups/{action_group_id}", response_model=ActionGroupStatusResponse)
    def action_group(
        action_group_id: UUID,
        _: HedgePrincipal = Depends(authenticated),
    ) -> ActionGroupStatusResponse:
        return execution_query.action_group(action_group_id=action_group_id)

    pair_summary = getattr(execution_query, "pair_summary", None)
    if callable(pair_summary):

        @router.get("/pair-summary/{symbol}", response_model=PairSummaryResponse)
        def pair_summary_route(
            symbol: str,
            account_id: str = "default",
            _: HedgePrincipal = Depends(authenticated),
        ) -> PairSummaryResponse:
    ...
    events = getattr(execution_query, "events", None)
    if callable(events):
...
def create_hedge_readonly_router(
    ...
    *,
    ...
    _register_query_routes(router, query, authenticated)
    if execution_query is not None:
        required_execution = ("orders", "order", "action_group")
        if any(not callable(getattr(execution_query, name, None)) for name in required_execution):
            raise TypeError("execution_query does not implement HedgeExecutionReadonlyQueryPort")
        _register_execution_routes(router, execution_query, authenticated)
    return router
...
class HedgeReadonlyCommandDispatcher:
    ...
    def __init__(
        ...
        *,
        ...
        if execution_query is not None:
            ...
            if any(
                not callable(getattr(execution_query, name, None)) for name in execution_required
            ):
    ...
    def _risk(self, command: HedgeReadonlyCommandSchema) -> dict:
        return self._query.risk(account_id=command.account_id).model_dump(mode="json")

    def _reconciliation(self, command: HedgeReadonlyCommandSchema) -> dict:
        return self._query.reconciliation(account_id=command.account_id).model_dump(mode="json")

    def _readiness(self, command: HedgeReadonlyCommandSchema) -> dict:
        return self._query.readiness(account_id=command.account_id).model_dump(mode="json")

    def _user_stream(self, command: HedgeReadonlyCommandSchema) -> dict:
        return self._query.user_stream(account_id=command.account_id).model_dump(mode="json")
    ...
    def _order(self, command: HedgeReadonlyCommandSchema) -> dict:
        ...
        return self._execution_query.order(client_order_id=command.client_order_id).model_dump(
            mode="json"
        )
    ...
    def _action_group(self, command: HedgeReadonlyCommandSchema) -> dict:
        ...
        return self._execution_query.action_group(action_group_id=group_id).model_dump(mode="json")
