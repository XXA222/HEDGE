...
class DashboardStrategySchema(BaseModel):
    ...
    long_score: Decimal = Decimal(0)
    short_score: Decimal = Decimal(0)
...
class DashboardCycleSchema(BaseModel):
    ...
    funding_pnl: Decimal
    fees: Decimal
    ideal_order_count: int
    submit_order_count: int
    cancel_order_count: int
    fill_count: int
    active_order_count: int
    risk_blocked: bool
    diagnostics: tuple[str, ...]
    strategy: DashboardStrategySchema
...
class DashboardOverviewSchema(BaseModel):
    ...
    symbol: str
    mode: str
    readiness_state: Literal["READY", "NOT_READY", "UNAVAILABLE"]
    readiness_reasons: tuple[str, ...] = ()
    control: DashboardControlSchema | None = None
    latest: DashboardCycleSchema | None = None
...
class DashboardCyclesSchema(BaseModel):
    model_config = ConfigDict(frozen=True)
    cycles: tuple[DashboardCycleSchema, ...]
    count: int
