...
def _producer_ws_url(producer: Producer) -> str:
    """Build and validate the producer WebSocket endpoint."""

    host = producer.get("host")
    port = producer.get("port")
    token = producer.get("ws_token")

    if not isinstance(host, str) or not host or host != host.strip():
        raise ValueError("producer host must be a non-empty trimmed string")
    if any(character in host for character in "/?#@"):
        raise ValueError("producer host contains URL delimiter characters")
    if any(character.isspace() or ord(character) < 32 for character in host):
        raise ValueError("producer host contains whitespace or control characters")
    if isinstance(port, bool) or not isinstance(port, int) or not 1 <= port <= 65535:
        raise ValueError("producer port must be an integer between 1 and 65535")
    if not isinstance(token, str):
        raise TypeError("producer ws_token must be a string")

    scheme = "wss" if producer.get("secure", False) else "ws"
    url_host = f"[{host}]" if ":" in host and not host.startswith("[") else host
    return f"{scheme}://{url_host}:{port}/api/v1/message/ws?token={token}"
...
class ExternalMessageConsumer:
    ...
    async def _create_connection(self, producer: Producer, lock: asyncio.Lock):
        ...
        """
        while self._running:
            ws_url = "<invalid producer endpoint>"
            try:
                name = producer["name"]
                ws_url = _producer_ws_url(producer)
