...
def run_smoke_validation() -> ValidationReport:
    ...
    action_count = int(cast(Discrete, env.action_space).n)
    ...
    checks["freqai_bridge_contract"] = bool(
        ...
        and len(bridge.action_mask(neutral_context)) == action_count
    )
    ...
    checks["single_leg_emergency_reduce"] = (
        HedgeActionMasker(cfg)
        .evaluate(HedgeActions.EMERGENCY_REDUCE_BOTH, account=one_leg, mark=100.0)
        .allowed
    )

    unsafe_logits = np.zeros(action_count)
    ...
    checks["nonfinite_inference_fails_closed"] = (
        guarded.executed_action is HedgeActions.HOLD and "NONFINITE_LOGITS" in guarded.reasons
    )
    ...
    actor_critic = HedgeActorCriticNetwork(encoder, action_count=action_count)
    ...
    checks["actor_critic_forward_backward"] = bool(
        logits.shape == (2, action_count) and values.shape == (2,) and torch.isfinite(loss)
    )
