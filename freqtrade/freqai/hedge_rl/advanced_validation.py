...
def run_advanced_validation() -> AdvancedValidationReport:
    ...
    checks["features_drift"] = (
        population_stability_index(features.iloc[:32, 0], features.iloc[:32, 0]) < 1e-12
    )
    ...
    checks["market_gap_detection"] = (
        len(detect_market_gaps(gapped, expected_interval=pd.Timedelta(minutes=1))) == 1
    )
    ...
    checks["execution_liquidity_partial_fill"] = cap.executable_quantity == 20 and math.isclose(
        sum(schedule), 10
    )
    ...
    checks["training_checkpoint_fail_closed"] = (
        compatible and not mismatches and decision.executed_action is HedgeActions.HOLD
    )
