...
def validate_aligned_market_data(
    ...
    *,
    ...
    price_values = (
        prices[list(required)].apply(pd.to_numeric, errors="coerce").to_numpy(dtype=float)
    )
    ...
    if reject_suspicious_feature_names:
        suspicious = [
            str(column) for column in features.columns if _FUTURE_NAME.search(str(column))
        ]
        if suspicious:
