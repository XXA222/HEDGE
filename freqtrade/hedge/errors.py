...
class HedgeError(RuntimeError, ValueError):
    ...
    def __init__(
        self, message: str = "Hedge operation failed.", *, reason_code: str | None = None
    ) -> None:
...
def is_retryable_error(exc: BaseException) -> bool:
    """Return whether an error is explicitly safe to retry.

    The exception hierarchy is authoritative.  The legacy ``retryable``
    attribute remains a compatibility bridge for older Binance transport
    callers until they migrate to ``HedgeRetryableError``.
    """

    if isinstance(exc, HedgeRetryableError):
        return True
    if isinstance(exc, HedgeDefinitiveError):
        return False
    return bool(getattr(exc, "retryable", False))


def is_definitive_error(exc: BaseException) -> bool:
    """Return whether an error is an explicit terminal integration failure."""

    return isinstance(exc, HedgeDefinitiveError)
