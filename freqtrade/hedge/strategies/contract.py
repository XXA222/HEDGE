...
def target_net_quantity_for_directive(
    *,
    ...
    scale = min(directive.confidence, directive.risk_scale)
    side_scale = directive.long_exposure_scale if target >= ZERO else directive.short_exposure_scale
    cap_ratio = base.max_wallet_exposure_long if target >= ZERO else base.max_wallet_exposure_short
