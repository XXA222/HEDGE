...
class RecoveryCheckpointStore:
    ...
    def save_atomic(self, checkpoint: DurableRecoveryCheckpoint) -> Path:
        ...
        # Windows requires a writable file descriptor for fsync.  Keeping the write,
        # flush and fsync in one handle also closes the durability gap between
        # Path.write_text() and reopening the temporary file.
