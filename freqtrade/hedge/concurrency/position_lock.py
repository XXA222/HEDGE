...
def _positive_timeout(value: float, *, field_name: str) -> float:
    if isinstance(value, bool):
        raise ValueError(  # noqa: TRY004 - public contract groups invalid numeric values
...
class PositionLockManager:
    def __init__(
        ...
        *,
        ...
        self._reservations: dict[PositionLockKey, dict[str, _ReduceReservationRecord]] = {}
        self._order = LockOrderTracker()

    def monotonic_time(self) -> float:
        value = self._monotonic_clock()
        if isinstance(value, bool):
            raise ValueError(  # noqa: TRY004 - invalid clock value is a value-domain error
                "monotonic_clock must return a finite nonnegative number."
            )
        result = float(value)
    ...
    def lock(
        ...
        *,
        ...
        normalized_side = (
            ...
            else PositionSide(str(position_side))
        )
        key = PositionLockKey(account_id, symbol, normalized_side, exchange)
        timeout = (
