...
class SqliteDatabaseLeaseStore:
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(busy_timeout_ms, bool) or not isinstance(busy_timeout_ms, int):
            raise ValueError(  # noqa: TRY004 - configuration validation preserves API contract
                "busy_timeout_ms must be an integer."
            )
    ...
    def _row_to_record(row: tuple[object, ...] | None) -> LeaseRecord | None:
        ...
        return LeaseRecord(
            ...
            owner_id=str(row[1]),
            fencing_token=int(str(row[2])),
            acquired_at_ms=int(str(row[3])),
            renewed_at_ms=int(str(row[4])),
            expires_at_ms=int(str(row[5])),
        )
        ...
        *,
        ...
        try:
            ...
            if current is None:
                ...
                same_live_owner = current.owner_id == owner_id and current.is_valid(now_ms=now_ms)
        ...
        *,
        ...
        try:
            ...
            if self._dialect == "sqlite":
                database = str(getattr(self._engine.url, "database", "") or "").strip().lower()
