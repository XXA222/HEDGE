...
class SingleWriterGuard:
        ...
        with self._lock:
            ...
            if lease is None:
                ...
                raise LeaseUnavailable(
                    f"Single-writer lease is held by another process: {self._lease_name}."
                )
            self._lease = lease
            self._lost = False
            return lease
    ...
    def status(self) -> SingleWriterStatus:
        with self._lock:
            ...
            current_lease = cast(LeaseRecord, current)
            self._lease = current_lease
            return SingleWriterStatus(
                True,
                self._owner_id,
                self._lease_name,
    ...
    def assert_valid(self) -> LeaseRecord:
        ...
        with self._lock:
            if not status.valid or self._lease is None:
                raise LeaseLost(f"Single-writer lease is no longer valid: {status.reason_code}.")
            return self._lease
