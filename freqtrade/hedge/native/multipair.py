...
class MultiPairPaperHedgeRuntime:
        ...
        *,
        ...
        with self._lock:
            for assignment in snapshot.assignments:
                if (
                    assignment.state is SymbolAssignmentState.ACTIVE
                    and assignment.pair not in self._applications
                ):
    ...
    def snapshot(self) -> MultiPairAccountSnapshot:
        ...
        equity = sum(pair_equity.values(), ZERO) + max(
            self.initial_balance - allocated_initial, ZERO
        )
        gross = sum(pair_gross.values(), ZERO)
        return MultiPairAccountSnapshot(
            equity=equity,
            available_balance=min(
                available + max(self.initial_balance - allocated_initial, ZERO), equity
            ),
