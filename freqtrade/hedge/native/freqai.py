...
class HedgeSignalEnvelope:
    ...
    def evidence_hash(self) -> str:
        payload = {
            ...
            if self.target_net_ratio is None
            else str(self.target_net_ratio),
            "target_gross_ratio": None
            if self.target_gross_ratio is None
            else str(self.target_gross_ratio),
        ...
        return sha256(
            dumps(payload, sort_keys=True, separators=(",", ":"), default=str).encode()
        ).hexdigest()
...
class HedgeModelManifest:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "compatible_pairs", tuple(str(item).upper() for item in self.compatible_pairs)
        )
        object.__setattr__(self, "metadata", dict(self.metadata))
        ...
        *,
        ...
        if manifest is None:
            ...
            if (
                ...
                and manifest.feature_schema != self.expected_feature_schema
            ):
    ...
    def from_row(
        ...
        *,
        ...
        return HedgeSignalEnvelope(
            ...
            model_version=str(
                self._lookup(row, self.COLUMN_ALIASES["model_version"], default_model_version)
            ),
            feature_schema=str(
                self._lookup(row, self.COLUMN_ALIASES["feature_schema"], default_feature_schema)
            ),
