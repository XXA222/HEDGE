...
class ProducerIdentity:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "allowed_pairs", tuple(str(item).upper() for item in self.allowed_pairs)
        )
...
class ProducerObservation:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.message_id:
            payload = (
                f"{self.identity.producer_id}:{self.signal.evidence_hash()}:"
...
class HedgeProducerConsumerGate:
    ...
    def observe(
        ...
        *,
        ...
        if (
            identity.expected_feature_schema
            and signal.feature_schema != identity.expected_feature_schema
        ):
        ...
        with self._lock:
            if observation.message_id in self._seen_messages:
                return False
            self._seen_messages.add(observation.message_id)
    ...
    def select(
        ...
        *,
        ...
        with self._lock:
            rows = [
                item for (item_pair, _), item in self._observations.items() if item_pair == pair
            ]
        for row in rows:
            reasons: list[str] = []
        ...
        accepted.sort(
            key=lambda item: (
                item.identity.priority,
                -item.received_at.timestamp(),
                item.identity.producer_id,
            )
        )
        conflict = False
        if len(accepted) > 1:
            ...
            for row in accepted[1:]:
                if (
                    ...
                    break
        selected = (
            None if conflict and self.fail_on_conflict else (accepted[0] if accepted else None)
        )
    ...
    def admit(self, intent: NativeOrderIntent) -> AdmissionDecision:
        if intent.reduce_only:
            return AdmissionDecision.allow(
                reason="PRODUCER_GATE_REDUCE_ONLY", reduce_only_exempt=True
            )
        selection = self.select(intent.pair)
        if selection.selected is None:
            reason = (
                "PRODUCER_CONFLICT" if selection.conflict else "PRODUCER_SIGNAL_MISSING_OR_STALE"
            )
            return AdmissionDecision.block(AdmissionCode.PRODUCER_STALE, reason)
        return AdmissionDecision.allow(
            reason=f"PRODUCER_SELECTED:{selection.selected.identity.producer_id}"
        )
    ...
    def status(self) -> dict[str, Any]:
        ...
        return {
            ...
            if selection is None
                ...
                "selected": None
                if selection.selected is None
                else selection.selected.identity.producer_id,
...
def producer_message_payload(signal: HedgeSignalEnvelope) -> dict[str, Any]:
    payload = {
        ...
        if signal.target_net_ratio is None
        else str(signal.target_net_ratio),
        "target_gross_ratio": None
        if signal.target_gross_ratio is None
        else str(signal.target_gross_ratio),
