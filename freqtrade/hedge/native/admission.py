...
def planner_intent_to_native(intent: object) -> NativeOrderIntent:
    typed_intent: Any = cast(Any, intent)
    action = HedgeAction(
        str(getattr(getattr(intent, "action", ""), "value", getattr(intent, "action", "")))
    )
    bucket = HedgeBucket(
        str(getattr(getattr(intent, "bucket", ""), "value", getattr(intent, "bucket", "")))
    )
    side = HedgeSide.parse(typed_intent.position_side)
    return NativeOrderIntent(
        pair=str(typed_intent.symbol),
        side=side,
        action=action,
        quantity=Decimal(str(typed_intent.quantity)),
        price=Decimal(str(typed_intent.price)),
        bucket=bucket,
        intent_id=str(getattr(typed_intent, "intent_id", "")),
        metadata={"reason": str(getattr(typed_intent, "reason", ""))},
    )
...
def apply_planning_admission_gate(
    ...
    *,
    ...
    typed_planning: Any = cast(Any, planning)
    blocked: list[tuple[str, AdmissionDecision]] = []
    allowed: list[object] = []
    blocked_sides: set[str] = set()
    for item in tuple(typed_planning.submit_orders):
    ...
    if not blocked:
        return typed_planning, ()
    diagnostics = tuple(getattr(typed_planning, "diagnostics", ())) + tuple(
    ...
    return replace(typed_planning, **replacement), tuple(blocked)
