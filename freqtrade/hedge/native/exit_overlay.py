...
from .models import ZERO, HedgeBucket, HedgeSide, LegSnapshot, utc_datetime
...
def policies_from_config(
...
) -> dict[tuple[HedgeSide, HedgeBucket], HedgeExitPolicy]:
    ...
    default = {
        ...
        "trailing_only_offset_reached": config.get("trailing_only_offset_is_reached", True),
    ...
    for side in HedgeSide:
        ...
        for bucket in HedgeBucket:
            ...
            result[(side, bucket)] = HedgeExitPolicy(
                ...
                trailing_only_offset_reached=bool(values.get("trailing_only_offset_reached", True)),
...
class NativeExitOverlay:
    ...
    def apply(self, planning: Any, *, app: Any, market: Any) -> tuple[Any, tuple[str, ...]]:
        ...
        for leg in self._leg_snapshots(app, market):
            ...
            if quantity <= ZERO:
                diagnostics.append(f"NATIVE_EXIT_BELOW_STEP:{leg.side.value}:{leg.bucket.value}")
                continue
            ...
            diagnostics.append(f"NATIVE_EXIT:{leg.side.value}:{leg.bucket.value}:{decision.reason}")
        if not exits:
            return planning, tuple(diagnostics)
        submit = tuple(planning.submit_orders) + tuple(exits)
        ideal = tuple(planning.ideal_orders) + tuple(exits)
