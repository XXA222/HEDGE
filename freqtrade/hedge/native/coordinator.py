...
class NativeConvergenceCoordinator:
    ...
    def __init__(
        ...
        *,
        ...
        convergence = dict(convergence_raw) if isinstance(convergence_raw, Mapping) else {}
        self.callbacks = HedgeStrategyCallbackAdapter(
            strategy,
            mode=CallbackCompatibilityMode(str(convergence.get("callback_mode", "native_only"))),
            fail_closed_confirmations=bool(convergence.get("fail_closed_confirmations", True)),
        )
        self.model_gate: object | None = None
        self.producer_gate: object | None = None
        self.universe_gate: object | None = None
    ...
    def transform_planner_intent(self, planner_intent: object) -> object:
        ...
        transformed = (
            ...
            else replace(cast(Any, planner_intent), price=adjusted_price)
        )
        ...
        if quantity <= Decimal(0):
            return replace(cast(Any, transformed), quantity=Decimal("0.000000000000000001"))
        if quantity == native.quantity:
            return transformed
        return replace(cast(Any, transformed), quantity=quantity)
    ...
    def notify_order_filled(
        ...
        self.callbacks.order_filled(
            ...
            current_time=cast(datetime | None, at),
        )
