...
def finite_decimal(value: object, *, field_name: str) -> Decimal:
    ...
    if isinstance(value, bool):
        raise HedgeDataError(f"{field_name} must be a finite decimal")
    try:
        result = value if isinstance(value, Decimal) else Decimal(str(value))
    except (InvalidOperation, TypeError, ValueError) as exc:
        raise HedgeDataError(f"{field_name} must be a finite decimal") from exc
...
class NativeOrderIntent:
    ...
    def __post_init__(self) -> None:
        ...
        if not pair:
            raise HedgeDataError("pair is required")
        ...
        if quantity <= ZERO or price <= ZERO:
            raise HedgeDataError("quantity and price must be positive")
...
class AdmissionDecision:
    ...
    def __post_init__(self) -> None:
        ...
        if self.allowed and self.code is not AdmissionCode.ALLOWED:
            raise HedgeDataError("allowed decisions must use ALLOWED code")
    ...
    def block(
        ...
        *,
        ...
        if code is AdmissionCode.ALLOWED:
            raise HedgeDataError("blocking decision cannot use ALLOWED")
...
class CapitalSnapshot:
    ...
    def __post_init__(self) -> None:
        ...
        for name in names:
            ...
            if value < ZERO:
                raise HedgeDataError(f"{name} cannot be negative")
...
class LegSnapshot:
    ...
    def __post_init__(self) -> None:
        ...
        if self.quantity < ZERO:
            raise HedgeDataError("quantity cannot be negative")
        if self.quantity > ZERO and (self.average_price <= ZERO or self.mark_price <= ZERO):
            raise HedgeDataError("open leg prices must be positive")
...
class ExitDecision:
    ...
    def __post_init__(self) -> None:
        ...
        if fraction < ZERO or fraction > ONE:
            raise HedgeDataError("exit fraction must be between zero and one")
        if self.should_exit and fraction <= ZERO:
            raise HedgeDataError("an exit decision requires a positive fraction")
        if not self.should_exit and fraction != ZERO:
            raise HedgeDataError("non-exit decisions must use zero fraction")
...
class HedgeEvent:
    ...
    def __post_init__(self) -> None:
        ...
        if not event_type:
            raise HedgeDataError("event_type is required")
