...
class DeterministicTrial:
    """Small reproducible TrialLike source used by native Hedge searches."""

    def __init__(self, seed: int) -> None:
        from random import Random

        self._random = Random(seed)  # noqa: S311 - deterministic search, not security
...
class DecimalSpace:
    ...
    def __post_init__(self) -> None:
        ...
        step = (
            None if self.step is None else finite_decimal(self.step, field_name=f"{self.name}.step")
        )
    ...
    def suggest(self, trial: TrialLike) -> Decimal:
        ...
        return Decimal(
            str(trial.suggest_float(self.name, float(self.low), float(self.high), **kwargs))
        )
...
class HedgeHyperoptSpace:
    ...
    spaces: tuple[Space, ...] = field(
        default_factory=lambda: (
            ...
            DecimalSpace(
                "core_wallet_exposure_long", Decimal("0.02"), Decimal("0.50"), Decimal("0.01")
            ),
            DecimalSpace(
                "core_wallet_exposure_short", Decimal("0.02"), Decimal("0.50"), Decimal("0.01")
            ),
            DecimalSpace(
                "tactical_wallet_exposure_long", Decimal("0.01"), Decimal("0.30"), Decimal("0.01")
            ),
            DecimalSpace(
                "tactical_wallet_exposure_short", Decimal("0.01"), Decimal("0.30"), Decimal("0.01")
            ),
            DecimalSpace(
                "max_gross_wallet_exposure", Decimal("0.10"), Decimal("1.50"), Decimal("0.05")
            ),
            DecimalSpace(
                "take_profit_spacing", Decimal("0.001"), Decimal("0.05"), Decimal("0.001")
            ),
            DecimalSpace(
        ...
        rows = [
            {
                ...
                "loss": item.loss,
                "params": {
                    k: str(v) if isinstance(v, Decimal) else v for k, v in item.params.items()
                },
