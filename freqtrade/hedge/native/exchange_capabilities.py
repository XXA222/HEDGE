...
class HedgeExchangeCapabilities:
    ...
    def safe_for_live_write(self) -> bool:
        ...
        return self.level in {CapabilityLevel.TESTNET_WRITE, CapabilityLevel.MAINNET_WRITE} and all(
            required
        )
    ...
    def missing_for_live(self) -> tuple[str, ...]:
        fields = (
            ...
            "independent_position_side_orders",
            "reduce_only",
            "client_order_id",
            "user_stream",
            "rest_reconciliation",
        ...
        *,
        ...
        if order.index(entry.level) < order.index(CapabilityLevel(minimum)):
            raise PermissionError(
                f"{entry.exchange}/{entry.market} capability {entry.level.value} "
                f"is below {minimum.value}"
            )
        if (
            minimum in {CapabilityLevel.TESTNET_WRITE, CapabilityLevel.MAINNET_WRITE}
            and not entry.safe_for_live_write
        ):
            raise PermissionError(
