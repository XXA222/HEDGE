...
class HedgeStrategyCallbackAdapter:
        ...
        if self.native_enabled:
            ...
            candidates.append(
                self._finite_or_default(
                    value, field_name="hedge_custom_stake_amount", default=proposed
                )
            )
        if self.legacy_enabled and self._legacy_callback_available("custom_stake_amount"):
            value = self._call_optional(
                ...
                kwargs={
                    ...
                    "entry_tag": str(
                        intent.metadata.get("tag") or intent.metadata.get("reason") or ""
                    )
                    or None,
                    "side": intent.side.pairlock_side,
                },
            ...
            candidates.append(
                self._finite_or_default(value, field_name="custom_stake_amount", default=proposed)
            )
    ...
    def leverage(
        ...
        *,
        ...
        if self.native_enabled:
            ...
            candidates.append(
                self._finite_or_default(value, field_name="hedge_leverage", default=proposed)
            )
        if self.legacy_enabled and self._legacy_callback_available("leverage"):
            ...
            candidates.append(
                self._finite_or_default(value, field_name="leverage", default=proposed)
            )
        result = min(candidates)
        if result <= ZERO:
    ...
    def custom_order_price(
        ...
        *,
        ...
        if self.native_enabled and callable(native_callback):
            ...
            candidates.append(
                self._finite_or_default(
                    value, field_name="hedge_custom_order_price", default=proposed
                )
            )
        ...
        if self.legacy_enabled and self._legacy_callback_available(callback_name):
            ...
            candidates.append(
                self._finite_or_default(legacy, field_name=callback_name, default=proposed)
            )
        if not candidates:
            return proposed
        ...
        if self.native_enabled:
            ...
            if value is not None:
                try:
                    ...
                    logger.warning(
                        "Ignoring invalid hedge_custom_stoploss callback result",
                        exc_info=True,
                    )
        if (
            self.legacy_enabled
            ...
            if value is not None:
                try:
                    ...
                    logger.warning(
                        "Ignoring invalid legacy custom_stoploss callback result",
                        exc_info=True,
                    )
    ...
    def custom_roi(
        ...
        *,
        ...
        duration = (
            0 if leg.opened_at is None else max(0, int((now - leg.opened_at).total_seconds() // 60))
        )
