...
_VOLATILE_KEYS = {"generated_at", "run_id", "elapsed", "elapsed_seconds", "process_id", "hostname"}
...
def deep_differences(  # noqa: C901 - recursive diff boundary
    baseline: Any,
    candidate: Any,
    *,
    path: str = "$",
    ...
    if isinstance(left, Mapping) and isinstance(right, Mapping):
        ...
        for key in keys:
            ...
            if key not in left:
                ...
                rows.extend(
                    deep_differences(left[key], right[key], path=child, tolerance=tolerance)
                )
        return tuple(rows)
    if (
        ...
        for index in range(max(len(left), len(right))):
            ...
            if index >= len(left):
                ...
                rows.extend(
                    deep_differences(left[index], right[index], path=child, tolerance=tolerance)
                )
        return tuple(rows)
    try:
...
class HedgeLookaheadAnalyzer:
    ...
    def __init__(
        self, *, fields: Iterable[str] = ("signals", "orders", "events", "snapshots")
    ) -> None:
