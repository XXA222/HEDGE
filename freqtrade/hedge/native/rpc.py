...
def _serial(value: Any) -> Any:
    ...
    if is_dataclass(value):
        return _serial(asdict(cast(Any, value)))
...
class HedgePositionProjection:
    ...
    def from_legs(cls, legs: Iterable[LegSnapshot], **kwargs: Any) -> HedgePositionProjection:
        ...
        return cls(
            ...
            core_quantity=sum(
                (item.quantity for item in rows if item.bucket is HedgeBucket.CORE), Decimal(0)
            ),
            tactical_quantity=sum(
                (item.quantity for item in rows if item.bucket is HedgeBucket.TACTICAL), Decimal(0)
            ),
    ...
    def publish(self, event: HedgeEvent) -> None:
        with self._lock:
            ...
            if len(self._events) > self.event_capacity:
                del self._events[: len(self._events) - self.event_capacity]
