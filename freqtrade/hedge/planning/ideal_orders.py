...
def _same_enough(context: PlanningContext, desired: OrderIntent, active: ActiveOrder) -> bool:
    ...
    qty_tolerance = context.market.qty_step * Decimal(context.config.replace_qty_tolerance_steps)
    return (
...
def _enforce_exposure(
    ...
    for order in sorted(
        ...
        elif context.collect_diagnostics:
            diagnostics.append(
...
class PureHedgePlanner(StrategyPlanningPort):
    ...
    def plan(self, context: PlanningContext) -> PlanningResult:
        ...
        for order in desired:
            ...
            compatible = next(
                (
                    active
                    for active in managed_active
                    if active.order_id not in matched_managed and _compatible_family(order, active)
                ),
                None,
            )
            if compatible is not None and _replacement_debounced(
                ...
                if context.collect_diagnostics:
                    debounce_diag.append(
                        f"{order.position_side.value}:replacement_debounced:{compatible.order_id}"
                    )
                continue
        ...
        unstuck_sides = {
            order.position_side for order in desired if order.action is IntentAction.UNSTUCK
        }
        ...
        for active in managed_active:
            ...
            opposing_net_repair = net_repair_active and (
                (long_target.net_gap_quantity > ZERO and active.position_side is PositionSide.SHORT)
                or (
        ...
        return PlanningResult(
            ...
            diagnostics=tuple(long_diag + short_diag + risk_diag + tuple(debounce_diag)),
