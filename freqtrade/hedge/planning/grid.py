...
def build_entry_grid(  # noqa: C901 - deterministic grid construction boundary
    context: PlanningContext,
    side: PositionSide,
    target: TargetPosition,
    *,
    ...
    for layer, qty, price, reason in allocations:
        ...
        for bucket, bucket_qty in (
            ...
            if len(intents) >= order_slots_left:
                break
            action = (
                IntentAction.OPEN if leg.quantity == ZERO and not intents else IntentAction.INCREASE
            )
            intents.append(
