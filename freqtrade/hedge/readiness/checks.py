...
def _require_bool(value: object, *, field_name: str) -> bool:
    if not isinstance(value, bool):
        raise ValueError(  # noqa: TRY004 - readiness input contract
...
class ReadinessInputs:
    ...
    def __post_init__(self) -> None:  # noqa: C901 - readiness input validation boundary
        for field_name in (
        ...
        if isinstance(self.observed_leverages, (str, bytes)):
            raise ValueError(  # noqa: TRY004 - readiness input contract
                "observed_leverages must be a tuple of leverage values."
            )
        try:
        ...
        normalized_leverages = tuple(
            require_positive(value, field="observed_leverage") for value in observed_leverages
        )
        ...
        if not isinstance(self.halt_reasons, tuple):
            raise ValueError(  # noqa: TRY004 - readiness input contract
                "halt_reasons must be a tuple of non-empty strings."
            )
        ...
        if not isinstance(self.unknown_orders, tuple):
            raise ValueError(  # noqa: TRY004 - readiness input contract
                "unknown_orders must be a tuple."
            )
