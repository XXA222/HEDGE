...
class ReadinessGate:
    ...
    def evaluate(self, inputs: ReadinessInputs) -> ReadinessReport:
        try:
            ...
            with self._lock:
                self._report = self._clock_failure(previous_timestamp=self._report.evaluated_at_ms)
                return self._report
        ...
        non_positional_reasons = tuple(
            code for code in reasons if code is not ReadinessReasonCode.UNKNOWN_ORDER_PRESENT
        )
    ...
    def allows_new_risk(self, position_key: RiskPositionKey | None = None) -> bool:
        ...
        return bool(report.reason_codes) and all(
            code is ReadinessReasonCode.UNKNOWN_ORDER_PRESENT for code in report.reason_codes
        )
