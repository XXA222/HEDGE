...
class AccountRiskFacts:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.source_complete, bool):
            raise ValueError(  # noqa: TRY004 - risk facts contract
                "source_complete must be a boolean."
            )
        if not isinstance(self.reconciliation_converged, bool):
            raise ValueError(  # noqa: TRY004 - risk facts contract
                "reconciliation_converged must be a boolean."
            )
        if not isinstance(self.source_errors, tuple):
            raise ValueError(  # noqa: TRY004 - risk facts contract
                "source_errors must be a tuple."
            )
        normalized = tuple(
            item.strip() for item in self.source_errors if isinstance(item, str) and item.strip()
        )
    ...
    def to_portfolio(self) -> RiskPortfolioSnapshot:
        portfolio = build_risk_portfolio(
            ...
            risk_data_valid=(
                self.source_complete and self.reconciliation_converged and not self.source_errors
            ),
