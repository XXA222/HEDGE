...
class HedgeRiskRuntimeStatus:
    ...
    def as_dict(self) -> dict[str, object]:
        return {
            ...
            "lease_runner": (None if self.lease_runner is None else self.lease_runner.as_dict()),
...
class HedgeRiskRuntime:
    ...
    def _sync_state(self, report: ReadinessReport) -> RiskActionState:
        ...
        only_position_unknown = bool(reasons) and all(
            reason is ReadinessReasonCode.UNKNOWN_ORDER_PRESENT for reason in report.reason_codes
        )
        ...
        if not report.ready:
            ...
            if current.mode is RiskMode.NORMAL or (
                current.mode is RiskMode.REDUCE_ONLY and current.reason_codes != desired_reasons
            ):
                return self._state_machine.transition(
...
def build_hedge_risk_runtime(
    *,
    ...
    if not isinstance(enable_lease_runner, bool):
        raise ValueError(  # noqa: TRY004 - risk runtime config contract
            "enable_lease_runner must be a boolean."
        )
