...
class RiskActionState:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.reason_codes, tuple):
            raise ValueError(  # noqa: TRY004 - risk action contract
                "Risk action reason_codes must be a tuple."
            )
        normalized = tuple(
            reason.strip()
...
class RiskActionStateMachine:
    ...
    def transition(
        ...
        *,
        ...
        normalized_event = event if isinstance(event, RiskEvent) else RiskEvent(str(event).upper())
...
class UnifiedRiskApproval:
    ...
    def __post_init__(self) -> None:  # noqa: C901 - risk approval validation boundary
        if not isinstance(self.allowed, bool):
            raise ValueError(  # noqa: TRY004 - risk approval contract
                "allowed must be a boolean."
            )
        if not isinstance(self.reason_codes, tuple):
            raise ValueError(  # noqa: TRY004 - risk approval contract
                "Approval reason_codes must be a tuple."
            )
        normalized_reasons = tuple(
            reason.strip()
            for reason in self.reason_codes
    ...
    def confirm(
        ...
        *,
        ...
        try:
            ...
            if any(
                commit_port.read_commit(record.decision_id) != record for record in record_tuple
            ):
...
class RiskApprovalCoordinator:
    ...
    def _approve_increase(
        ...
        with self._reservation_lock:
            ...
            account_with_pending = replace(
                ...
                pending_net_notional_delta=(account.pending_net_notional_delta + local_net_delta),
            ...
            request_with_pending = replace(
                request,
                pending_leg_increase_notional=(request.pending_leg_increase_notional + local_leg),
            ...
            reservation_expires_at_ms = reservation_created_at_ms + self._reservation_ttl_ms
    ...
    def pending_increase_notional(self, *, account_id: str) -> Decimal:
        if not isinstance(account_id, str):
            raise ValueError(  # noqa: TRY004 - reservation API contract
        ...
        with self._batch_lock:
            ...
            for index, item in enumerate(normalized_items):
                try:
                    ...
                    for approval in approvals:
                        if approval.reservation is not None:
                            approval.reservation.release()
                    denied = (
                        tuple(self._deny("BATCH_ROLLED_BACK") for _ in range(index))
                        + (self._deny("INVALID_BATCH_ITEM"),)
                        + tuple(
                ...
                if not approval.allowed:
                    ...
                    denied = (
                        tuple(self._deny("BATCH_ROLLED_BACK") for _ in approvals)
                        + (approval,)
                        + tuple(
                            self._deny("BATCH_NOT_EVALUATED")
                            for _ in range(len(normalized_items) - index - 1)
