...
class HedgeRiskEngine:
    ...
    def _evaluate_increase(  # noqa: C901 - risk increase decision boundary
        self,
        request: RiskRequest,
        account: AccountRiskSnapshot,
    ) -> RiskDecision:
        if not account.effective_risk_data_valid:
            invalid_reasons = tuple(account.risk_data_errors) or ("RISK_DATA_INVALID",)
            return RiskDecision(
                False,
                Decimal(0),
                invalid_reasons,
