...
class AccountRiskSnapshot:
    ...
    def __post_init__(self) -> None:  # noqa: C901 - risk model validation boundary
        ...
        errors = tuple(
            item.strip() for item in self.risk_data_errors if isinstance(item, str) and item.strip()
        )
    ...
    def unattributed_pending_notional(self) -> Decimal:
        return max(
            self.pending_order_notional - self.pending_long_notional - self.pending_short_notional,
            Decimal(0),
        )
...
class RiskDecision:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.allowed and (self.approved_quantity != 0 or self.approved_notional != 0):
            raise HedgeConfigurationError(
