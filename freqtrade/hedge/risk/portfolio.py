...
def build_risk_portfolio(
    *,
    ...
    if any(
        item.liquidation_price is None or item.liquidation_price <= 0 for item in open_positions
    ):
    ...
    effective_valid = risk_data_valid and (not strict_completeness or not completeness_errors)
    ...
    account = AccountRiskSnapshot(
        ...
        liquidation_data_complete=("LIQUIDATION_DATA_INCOMPLETE" not in completeness_errors),
        maintenance_margin_complete=("MAINTENANCE_MARGIN_INCOMPLETE" not in completeness_errors),
        exchange=exchange,
