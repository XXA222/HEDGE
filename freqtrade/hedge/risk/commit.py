...
class SqlRiskApprovalCommitStore:
    ...
    def _from_row(row: object) -> ApprovalCommitRecord:
        ...
        return ApprovalCommitRecord(
            decision_id=str(typed_row.decision_id),
            intent_id=str(typed_row.intent_id),
            idempotency_key=str(typed_row.idempotency_key),
            correlation_id=str(typed_row.correlation_id),
            risk_snapshot_id=str(typed_row.risk_snapshot_id),
            request_json=str(typed_row.request_json),
            risk_snapshot_json=str(typed_row.risk_snapshot_json),
            rules_version=str(typed_row.rules_version),
            fencing_token=int(typed_row.fencing_token),
            approved_quantity=Decimal(str(typed_row.approved_quantity)),
            approved_notional=Decimal(str(typed_row.approved_notional)),
            evaluated_at_ms=int(typed_row.evaluated_at_ms),
            committed_at_ms=int(typed_row.committed_at_ms),
            durable_reference=str(typed_row.durable_reference),
            target_snapshot_version=typed_row.target_snapshot_version,
            intent_expires_at_ms=typed_row.intent_expires_at_ms,
        )
            ...
            with self._session_factory.begin() as session:
                for record in records:
                    ...
                    conflict = session.scalar(
                        select(RiskApprovalCommitRow).where(
                            RiskApprovalCommitRow.idempotency_key == record.idempotency_key
                        )
                    )
    ...
    def read_commit(self, decision_id: str) -> ApprovalCommitRecord | None:
        ...
        with self._session_factory() as session:
            row = session.scalar(
                select(RiskApprovalCommitRow).where(RiskApprovalCommitRow.decision_id == normalized)
            )
