...
class RiskLimits:
    ...
    def __post_init__(self) -> None:
        ...
        if self.max_gross_notional is None and self.max_gross_exposure_ratio is None:
            raise HedgeConfigurationError("Set max_gross_notional or max_gross_exposure_ratio.")
    ...
    def from_mapping(cls, values: Mapping[str, object]) -> RiskLimits:
        ...
        if unknown:
            raise HedgeConfigurationError(f"Unknown risk limit fields: {sorted(unknown)!r}.")
        missing = {
            "max_margin_utilization",
            "min_liquidation_buffer_ratio",
        } - set(values)
        ...
        return cls(**cast(Any, dict(values)))

    def as_dict(self) -> dict[str, str | None]:
        return {
            field_name: (
                None if getattr(self, field_name) is None else str(getattr(self, field_name))
            )
