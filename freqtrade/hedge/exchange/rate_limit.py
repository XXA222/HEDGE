...
class BinanceTransportError(HedgeIntegrationError):
    default_reason_code = "BINANCE_TRANSPORT_ERROR"
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(message, reason_code=self.default_reason_code)
        self.status = status
        self.code = code
...
class BinanceRateLimitError(BinanceTransportError, HedgeRetryableError):
    default_reason_code = HedgeRetryableError.default_reason_code


class BinanceRetryableError(BinanceTransportError, HedgeRetryableError):
    default_reason_code = HedgeRetryableError.default_reason_code


class BinancePermissionError(BinanceTransportError, HedgeDefinitiveError):
    default_reason_code = HedgeDefinitiveError.default_reason_code


class BinanceDataError(BinanceTransportError, HedgeDefinitiveError):
    default_reason_code = HedgeDefinitiveError.default_reason_code
    ...
    def __init__(
        ...
        *,
        ...
        if shared_budget is not None and (
            shared_budget.limit != limit_per_minute or shared_budget.reserve != reserve_weight
        ):
    ...
    async def acquire(self, weight: int = 1) -> None:
        ...
        while True:
            if self._shared_budget is not None:
                decision = await asyncio.to_thread(self._shared_budget.reserve_weight, weight)
                if decision.granted:
    ...
    *,
    ...
    for attempt in range(1, effective_policy.max_attempts + 1):
        try:
            ...
            last_error = exc
            should_retry = retry_if(exc) if retry_if is not None else is_retryable_error(exc)
            if not should_retry or attempt >= effective_policy.max_attempts:
                raise
            retry_after = exc.retry_after if isinstance(exc, BinanceTransportError) else None
