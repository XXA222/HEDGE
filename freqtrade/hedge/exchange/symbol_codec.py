...
def parse_canonical_pair(value: str) -> BinanceSymbol:
    ...
    if settle != quote:
        raise ValueError("Binance USD-M canonical pair must settle in the quote asset")
...
def to_canonical_pair(
    ...
    *,
    ...
    raise ValueError(f"cannot infer quote asset for Binance exchange symbol {exchange_symbol!r}")


def normalize_exchange_symbols(values: tuple[str, ...] | list[str]) -> tuple[str, ...]:
    if isinstance(values, (str, bytes)):
        raise ValueError(  # noqa: TRY004 - malformed symbol input preserves API contract
