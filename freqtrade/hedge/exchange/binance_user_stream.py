    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(trust_env_proxy, bool):
            raise ValueError(  # noqa: TRY004 - public adapter configuration contract
                "trust_env_proxy must be a boolean"
            )
...
def _reconnect_delays(minimum: float, maximum: float) -> tuple[float, float]:
    ...
    if maximum < minimum:
        raise ValueError("max_reconnect_delay_seconds must be >= min_reconnect_delay_seconds")
...
def _websocket_base_url(value: str) -> str:
    ...
    if parsed.scheme != "wss" and not (parsed.scheme == "ws" and parsed.hostname in local_hosts):
        raise ValueError("websocket_base_url must use WSS except for a local test server")
    return normalized
...
class BinanceEventSequencer:
    ...
    def inspect_account_position(
        ...
        *,
        ...
        return failure or SequencingDecision(EventDisposition.APPLY, "OK", entity_key)
        ...
        *,
        ...
        if entity_key in self._rest_baseline_entities and previous_fill is not None:
            ...
            if cumulative_fill < previous_fill or (
                cumulative_fill == previous_fill and previous_signature == state_signature
            ):
                return SequencingDecision(
                    EventDisposition.DUPLICATE,
                    "EVENT_COVERED_BY_REST_BASELINE",
...
class BinanceUserStream:
    ...
    def __init__(
        ...
        *,
        ...
        if not math.isfinite(reconnect_reset_after_seconds) or reconnect_reset_after_seconds < 0:
            raise ValueError("reconnect_reset_after_seconds must be finite and nonnegative")
    ...
    def current_balances(self) -> tuple[BalanceFact, ...]:
        return tuple(sorted(self._rest_balance_context.values(), key=lambda item: item.asset))
    ...
    def set_execution_order_event_callback(self, callback: AsyncCallback | None) -> None:
    ...
    def _rest_position_rows(
        ...
        for item in positions:
            if item.symbol not in self._managed_symbols:
                if item.quantity != 0:
                    raise BinanceDataError(f"REST seed contains unmanaged position: {item.symbol}")
                continue
            key = (item.symbol, item.position_side)
            if key in rows:
                raise BinanceDataError(
                    f"REST seed contains duplicate position: {item.symbol}:{item.position_side}"
                )
            rows[key] = item
        return rows
    ...
    def _validate_rest_orders(self, orders: Sequence[OrderFact]) -> None:
        ...
        for item in orders:
            ...
            if key in rows:
                raise BinanceDataError(
                    f"REST seed contains duplicate order: {item.symbol}:{item.exchange_order_id}"
                )
            rows.add(key)

    def _rest_balance_rows(self, balances: Sequence[BalanceFact]) -> dict[str, BalanceFact]:
        ...
        for item in balances:
            ...
            if item.asset in rows:
                raise BinanceDataError(f"REST seed contains duplicate balance asset: {item.asset}")
    ...
    def _seed_from_rest_unlocked(
        ...
        self._rest_order_context = {
            (item.symbol, item.exchange_order_id): item for item in orders if item.active
        }
        for position_item in position_rows.values():
            self._sequencer.seed_position(
                f"POSITION:{position_item.symbol}:{position_item.position_side}",
                position_item.update_time_ms,
                state_signature=_position_state_signature(position_item),
            )
        for order_item in orders:
            self._sequencer.seed_order(
                f"ORDER:{order_item.symbol}:{order_item.exchange_order_id}",
                order_item.update_time_ms,
                order_item.cumulative_filled_quantity,
                state_signature=_order_state_signature(order_item),
            )
        self._touch_state()
    ...
    async def _notify_callback_safely(self, callback: AsyncCallback | None, *args: Any) -> None:
        if callback is None:
            return
        try:
    ...
    async def run(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                connection = await self._connector.connect(f"{self._base_url}/{lease.listen_key}")
                connected_started = self._clock.monotonic()
                await self._consume_connection(connection, generation=lease.generation)
            ...
            if connected_started is not None:
                connected_duration = max(0.0, self._clock.monotonic() - connected_started)
            stable_connection = (
                self._current_connection_message_count > 0
    ...
    async def _request_recalibration(self, reason: str, payload: Mapping[str, Any]) -> None:
        try:
            if self._on_recalibration_required is not None:
                await maybe_await(self._on_recalibration_required(reason, payload))
        finally:
            await self._close_current_connection()

    async def _publish_execution_order_event(self, payload: Mapping[str, Any]) -> None:
        ...
        try:
            ...
            raise BinanceDataError("Execution order event bridge failed") from exc

    async def _process_payload(self, payload: Mapping[str, Any]) -> EventDisposition:
        ...
        if event_type == "ORDER_TRADE_UPDATE" and disposition is EventDisposition.APPLY:
    ...
    async def _parse_message(
        ...
        if isinstance(raw_message, bytes):
            ...
            try:
                ...
                await self._fault("INVALID_UTF8", {"raw_hex": raw_bytes[:256].hex()})
    ...
    def _prepare_account_balances(
        ...
        for item in raw_balances:
            if not isinstance(item, Mapping):
                raise BinanceDataError("ACCOUNT_UPDATE balance row must be an object")
    ...
    def _prepare_account_position(
        ...
        *,
        ...
        raw_symbol = str(raw_position.get("s") or raw_position.get("symbol") or "").strip().upper()
        if not raw_symbol:
    ...
    def _single_balance_change(
        ...
        for item in raw_balances:
            ...
            amount = optional_decimal(
                item.get("bc", item.get("balanceChange")),
                field=f"{asset}.balanceChange",
                when_missing=MissingPolicy.ZERO,
            )
            if amount is None:  # pragma: no cover - ZERO policy always returns a value
                raise BinanceDataError(f"{asset}.balanceChange is required")
            if amount != 0:
    ...
    def _last_fill_quantity(order_payload: Mapping[str, Any]) -> Decimal:
        ...
        if quantity < 0:
            raise BinanceDataError("ORDER_TRADE_UPDATE last fill quantity is invalid")
        return quantity
    ...
    async def _optional_order_fill(
        ...
        exists = await self._repository.has_fill(self._account_id, order.symbol, trade_id)
            ...
            await self._repository.append_account_events(
                (
                    AccountEventFact(
                        ...
                        event_key=stable_fingerprint({"reason": reason, "payload": payload}),
