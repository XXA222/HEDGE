...
def _deduplicate_fills(facts: Sequence[FillFact]) -> tuple[FillFact, ...]:
    ...
    for item in facts:
        ...
        if previous is not None and _fill_core(previous) != _fill_core(item):
            raise BinanceDataError(f"Conflicting duplicate fill: {item.exchange_trade_id}")
...
def _deduplicate_orders(facts: Sequence[OrderFact]) -> tuple[OrderFact, ...]:
    ...
    for item in facts:
        ...
        if previous is None or item.update_time_ms > previous.update_time_ms:
            deduplicated[key] = item
        elif item.update_time_ms == previous.update_time_ms and _order_core(
            previous
        ) != _order_core(item):
            raise BinanceDataError(f"Conflicting duplicate order: {item.exchange_order_id}")
    return tuple(
        sorted(
            deduplicated.values(),
...
class AiohttpBinanceRestTransport:
    ...
    def __init__(
        ...
        *,
        ...
        timeout_value = _positive_finite_float(timeout_seconds, field="timeout_seconds")
        recv_window_value = _positive_int(recv_window_ms, field="recv_window_ms")
        ...
        *,
        ...
        async def operation() -> BinanceHttpResponse:
            ...
            try:
                ...
                self._last_latency_ms = max(0.0, (self._clock.monotonic() - started) * 1000.0)
    ...
    def _request_parameters(
        ...
        try:
            timestamp = _nonnegative_int(self._timestamp_provider(), field="timestamp")
    ...
    async def _request_once(
        ...
        *,
        ...
        url = f"{base}{path}"
        requires_api_key = signed or path.startswith("/sapi/") or path.endswith("/listenKey")
        ...
        try:
            ...
            raise BinanceRetryableError(str(exc), retryable=True) from exc
...
class BinanceReadonlyClient:
    ...
    async def _fetch_api_restrictions(
        ...
        try:
            ...
            if policy.allow_testnet_restriction_endpoint_unavailable and exc.status in {400, 404}:
                return {}, ("API_RESTRICTIONS_UNAVAILABLE",)
            raise
    ...
    @staticmethod
    def _restriction_flag(restrictions: Mapping[str, Any], name: str) -> bool | None:
    ...
    async def preflight_permissions(
        ...
        internal_transfer = self._restriction_flag(restrictions, "enableInternalTransfer")
        universal_transfer = self._restriction_flag(restrictions, "permitsUniversalTransfer")
        spot_margin = self._restriction_flag(restrictions, "enableSpotAndMarginTrading")
        ...
        if report.strict_readonly_verified:
            return report
        raise BinancePermissionError(
            "Binance API key failed read-only permission preflight: " + ",".join(report.reasons),
    ...
    async def fetch_order_status(self, symbol: str, *, order_id: str) -> OrderFact:
    ...
    @staticmethod
    def _next_numeric_id(values: Sequence[str], *, endpoint: str, symbol: str) -> int:
        numeric_ids = [int(value) for value in values if value.lstrip("-").isdigit()]
        if not numeric_ids:
            raise BinanceDataError(f"Cannot advance {endpoint} pagination for {symbol}")
    ...
    async def _fetch_fill_window(
        ...
        *,
        ...
        while requests < request_budget:
            ...
            facts.extend(item for item in page if window_start <= item.event_time_ms <= window_end)
            ...
            if from_id is not None and next_from_id <= from_id:
                raise BinanceDataError(f"userTrades pagination did not advance for {symbol}")
            from_id = next_from_id
        raise BinanceDataError(f"userTrades exceeded max_pages={request_budget} for {symbol}")
    ...
    async def fetch_fills(
        ...
        *,
        ...
        windows = self._history_windows(start_time_ms=start_time_ms, end_time_ms=end_time_ms)
        facts: list[FillFact] = []
        ...
        *,
        ...
        while requests < request_budget:
            ...
            if next_order_id is not None and candidate <= next_order_id:
                raise BinanceDataError(f"allOrders pagination did not advance for {symbol}")
            next_order_id = candidate
        raise BinanceDataError(f"allOrders exceeded max_pages={request_budget} for {symbol}")
    ...
    def _income_parameters(
        *,
        ...
        if start_time_ms is not None:
            params["startTime"] = _nonnegative_int(start_time_ms, field="start_time_ms")
        if end_time_ms is not None:
    ...
    def _deduplicate_income(
        ...
        for item in items:
            ...
            if previous is not None and dict(previous[1]) != dict(item):
                raise BinanceDataError(f"Conflicting duplicate income record: {key!r}")
    ...
    async def confirm_configuration(
        ...
        invalid_leverage = sorted(
            key for key, value in configuration.leverage_by_symbol_side.items() if value <= 0
        )
        if invalid_leverage:
            raise BinancePermissionError(
                "Managed symbol leverage could not be confirmed: " + ",".join(invalid_leverage)
            )
    ...
    def _contains_unmanaged_facts(
        ...
        *,
        ...
        unmanaged_position = any(
            item.quantity > 0 and item.symbol not in managed_symbols for item in positions
        )
        unmanaged_order = any(item.active and item.symbol not in managed_symbols for item in orders)
