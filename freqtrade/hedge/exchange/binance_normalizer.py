...
class MissingPolicy(StrEnum):
    """Explicit policy for missing numeric fields at the exchange boundary."""
...
def finite_decimal(value: Any, *, field: str) -> Decimal:
    ...
    if isinstance(value, bool):
        raise BinanceDataError(f"{field} must not be a boolean")
    try:
...
def optional_decimal(
    ...
    *,
    field: str,
    when_missing: MissingPolicy,
) -> Decimal | None:
    """Parse an exchange decimal with an explicit missing-value decision."""

    missing = value is None or (isinstance(value, str) and not value.strip())
    if missing:
        if when_missing is MissingPolicy.NONE:
            return None
        if when_missing is MissingPolicy.ZERO:
            return ZERO
        raise BinanceDataError(f"{field} is required")
    return finite_decimal(value, field=field)


def _decimal_with_policy(value: Any, *, field: str, when_missing: MissingPolicy) -> Decimal:
    parsed = optional_decimal(value, field=field, when_missing=when_missing)
    if parsed is None:
        raise BinanceDataError(f"{field} must be present for this boundary")
    return parsed


def nonnegative_decimal(value: Any, *, field: str) -> Decimal:
    parsed = finite_decimal(value, field=field)
    if parsed < 0:
...
def _optional_liquidation_price(payload: Mapping[str, Any], *, symbol: str) -> Decimal | None:
    ...
    parsed = optional_decimal(
        raw_value,
        field=f"{symbol}.liquidationPrice",
        when_missing=MissingPolicy.NONE,
    )
    if parsed is None or parsed == 0:
        return None
    if parsed < 0:
        raise BinanceDataError(f"{symbol}.liquidationPrice must be non-negative")
    return parsed
...
def _validate_symbol_configuration_rows(
    ...
    *,
    ...
    non_cross = sorted(
        symbol for symbol, (margin_mode, _leverage, _raw) in rows.items() if margin_mode != "cross"
    )
    if non_cross:
        raise BinancePermissionError(
...
def normalize_account_snapshot(
    ...
    *,
    ...
    snapshot = AccountSnapshotFact(
        ...
        total_wallet_balance=_decimal_with_policy(
            payload.get("totalWalletBalance"),
            field="totalWalletBalance",
            when_missing=MissingPolicy.ZERO,
        ),
        total_available_balance=_decimal_with_policy(
            payload.get("availableBalance"),
            field="availableBalance",
            when_missing=MissingPolicy.ZERO,
        ),
        total_margin_balance=_decimal_with_policy(
            payload.get("totalMarginBalance"),
            field="totalMarginBalance",
            when_missing=MissingPolicy.ZERO,
        ),
        total_initial_margin=_decimal_with_policy(
            payload.get("totalInitialMargin"),
            field="totalInitialMargin",
            when_missing=MissingPolicy.ZERO,
        ),
        total_maintenance_margin=_decimal_with_policy(
            payload.get("totalMaintMargin"),
            field="totalMaintMargin",
            when_missing=MissingPolicy.ZERO,
        ),
        total_unrealized_pnl=_decimal_with_policy(
            payload.get("totalUnrealizedProfit"),
            field="totalUnrealizedProfit",
            when_missing=MissingPolicy.ZERO,
        ),
...
def normalize_account_update_balance(
    ...
    *,
    ...
    available_balance = previous.available_balance if previous is not None else cross_wallet_balance
    unrealized_pnl = previous.unrealized_pnl if previous is not None else Decimal(0)
    return BalanceFact(
        account_id=account_id,
        asset=asset,
...
def normalize_account_update_balances(
    ...
    *,
    ...
    for index, item in enumerate(payload):
        if not isinstance(item, Mapping):
            raise BinanceDataError(f"ACCOUNT_UPDATE balance row {index} must be an object")
        ...
        if fact.asset in seen:
            raise BinanceDataError(f"ACCOUNT_UPDATE contains duplicate balance asset: {fact.asset}")
        seen.add(fact.asset)
        result.append(fact)
    return tuple(result)
...
def normalize_position(
    ...
    *,
    ...
    leverage_value = leverage_override if leverage_override is not None else payload.get("leverage")
    ...
    return PositionFact(
        ...
        leverage=nonnegative_int(leverage_value, field=f"{symbol}.leverage", default=0),
    ...
    *,
    ...
    for index, item in enumerate(payload):
        ...
        try:
            ...
            if raw_amount != 0:
                raise BinanceDataError(
                    f"Unsupported non-perpetual position exposure: {raw_symbol or '<missing>'}"
                ) from exc
            continue
    ...
    for item, symbol in normalized_rows:
        ...
        margin_source_missing = (
            item.get("marginType", item.get("mt")) in (None, "") and "isolated" not in item
        )
        ...
        fact = normalize_position(
            ...
            source=source,
            margin_mode_override=(
                fallback[0] if fallback is not None and margin_source_missing else None
            ),
            leverage_override=(
                fallback[1] if fallback is not None and leverage_source_missing else None
            ),
        )
        key = (fact.symbol, fact.position_side)
        if key in seen:
            raise BinanceDataError(f"duplicate position row: {fact.symbol}:{fact.position_side}")
        seen.add(key)
        result.append(fact)
    return tuple(result)
...
def normalize_order(
    ...
    *,
    ...
    client_order_id = str(payload.get("clientOrderId", payload.get("c", ""))).strip()
    ...
    return OrderFact(
        account_id=account_id,
        symbol=symbol,
        position_side=normalize_position_side(payload.get("positionSide") or payload.get("ps")),
...
def normalize_orders(
    ...
    *,
    ...
    for item in payload:
        ...
        if key in seen:
            raise BinanceDataError(f"duplicate order row: {fact.symbol}:{fact.exchange_order_id}")
        seen.add(key)
        result.append(fact)
    return tuple(result)
...
def normalize_configuration(
    *,
    ...
    hedge_mode = strict_bool(dual_side_payload.get("dualSidePosition"), field="dualSidePosition")
