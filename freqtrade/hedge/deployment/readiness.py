...
def validate_security_readiness_report(config: HedgeDeploymentConfig) -> DeploymentReadiness:
    ...
    checks: dict[str, str] = {
        "status": "PASS"
        if payload.get("status") == "SECURITY_AND_DEPLOYMENT_HARDENING_COMPLETE_MAINNET_LOCKED"
        else "FAIL",
        "complete": "PASS"
        if payload.get("security_and_deployment_hardening_complete") is True
        else "FAIL",
        "mainnet_locked": "PASS"
        if payload.get("mainnet_live_exchange_write") == "LOCKED"
        else "FAIL",
        "mainnet_requests_zero": "PASS"
        if int(payload.get("real_mainnet_order_requests_sent", -1)) == 0
        else "FAIL",
        "installer_network_none": "PASS"
        if payload.get("installer_network_access") == "NONE"
        else "FAIL",
    }
