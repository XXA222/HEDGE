...
class HedgeDeploymentConfig:
    ...
    @classmethod
    def from_mapping(  # noqa: C901 - deployment config normalization boundary
        cls,
        ...
        *,
        ...
        if project_root is None:
            raise DeploymentConfigError("project_root is required")
        ...
        if freqtrade_config is None:
            raise DeploymentConfigError("freqtrade_config is required")
        ...
        if isinstance(python_raw, str) and python_raw.strip().upper() == "AUTO":
            ...
            if python_executable is None:
                raise DeploymentConfigError(
                    "python_executable=AUTO did not find a project virtual environment"
                )
        else:
            python_executable = resolve_path("python_executable")
            if python_executable is None:
                raise DeploymentConfigError("python_executable is required")
        ...
        security_readiness_report: Path | None
        if isinstance(readiness_raw, str) and readiness_raw.strip().upper() == "AUTO":
            ...
            report_candidates: list[Path] = sorted(
                report_root.glob("**/SECURITY-DEPLOYMENT-READINESS.json"),
                key=lambda candidate: candidate.stat().st_mtime_ns,
                reverse=True,
            )
            security_readiness_report = (
                report_candidates[0].resolve() if report_candidates else None
            )
            ...
            if security_readiness_report is None:
                raise DeploymentConfigError("security_readiness_report is required")
        ...
        try:
            ...
            raise DeploymentConfigError(
                "mode must be HEDGE_SIMULATED, HEDGE_PRODUCTION_LOCKED, or HEDGE_TESTNET_VALIDATION"
            ) from exc
