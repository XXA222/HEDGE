...
class HedgeProcessSupervisor:
    ...
    def run(self) -> SupervisorResult:
        with self.lock:
            ...
            if identity is None:
                raise RuntimeError("supervisor lock identity is unavailable")
            ...
            while True:
                ...
                self.state_store.write(state)
                self.events.write(
                    "CHILD_STARTED", child_pid=self._child.pid, restart_count=state.restart_count
                )
                ...
                if self._should_stop():
                    ...
                    self.state_store.write(state)
                    self.events.write(
                        "SUPERVISOR_STOPPED", requested=True, child_exit_code=last_exit
                    )
                    return SupervisorResult(
                        phase=state.phase,
                        exit_code=last_exit,
                ...
                while (
                    restarts and now_tick - restarts[0] > self.config.restart_policy.window_seconds
                ):
                    restarts.popleft()
    ...
    def _monitor_child(self, state: RuntimeState) -> int:
        if self._child is None:
            raise RuntimeError("supervisor child process is unavailable")
        current = state
        while True:
    ...
    def _install_signal_handlers(self) -> None:
        ...
        for candidate in (signal.SIGINT, signal.SIGTERM):
            try:
                ...
                logger.debug(
                    "Unable to install deployment supervisor signal handler for %s",
                    candidate,
                    exc_info=True,
                )
