...
from freqtrade.hedge.errors import HedgeContractViolation
...
class HedgeContractError(HedgeContractViolation):
    ...
    def __init__(self, reason_code: ReasonCode | str, message: str = "") -> None:
        try:
            ...
            text = message or getattr(normalized, "value", str(normalized))
        except (TypeError, ValueError):
            normalized = str(reason_code)
            text = message or normalized
        super().__init__(text, reason_code=str(normalized))
        # Keep the public contract's enum identity while the common error base
        # still receives a stable wire-compatible string representation.
