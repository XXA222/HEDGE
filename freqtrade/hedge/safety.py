...
def assert_supported_operation_mode(value: object) -> str:
    ...
    if normalized not in {"paper", "readonly", "shadow"}:
        raise OperationalException("hedge.operation_mode must be paper, readonly, or shadow.")
    return normalized
...
def install_native_exchange_write_barrier(exchange: object, config: Any) -> tuple[str, ...]:
    ...
    exchange_obj = cast(Any, exchange)
    exchange_obj._hedge_native_write_barrier_installed = True
    exchange_obj._hedge_native_write_barrier_methods = tuple(blocked)
