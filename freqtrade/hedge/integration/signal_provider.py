...
def _required_aware_datetime(value: object, *, field: str) -> datetime:
    if hasattr(value, "to_pydatetime"):
        value = value.to_pydatetime()
    if not isinstance(value, datetime):
        raise ValueError(  # noqa: TRY004 - signal input contract
            f"analyzed candle {field} must be a datetime"
        )
    if value.tzinfo is None:
        return value.replace(tzinfo=UTC)
    return value.astimezone(UTC)
...
def _candle_from_row(
    ...
    if not isinstance(row, Mapping) and not callable(getattr(row, "get", None)):
        return None
    ...
    open_value = _exact_decimal(row_mapping.get("open"), field="open")
    high_value = _exact_decimal(row_mapping.get("high"), field="high")
    low_value = _exact_decimal(row_mapping.get("low"), field="low")
    close_value = _exact_decimal(row_mapping.get("close"), field="close")
    if any(value is None for value in (open_value, high_value, low_value, close_value)):
        raise ValueError("analyzed candle OHLC values are required")
    return AnalyzedCandle(
        symbol=pair,
        timeframe=timeframe,
        open_time=open_time,
        close_time=close_time,
        open=cast(Decimal, open_value),
        high=cast(Decimal, high_value),
        low=cast(Decimal, low_value),
        close=cast(Decimal, close_value),
        volume=volume,
    )
...
def signal_from_analyzed_row(
    *,
    ...
    if candle is None:
        raise ValueError("analyzed dataframe must contain date/open/high/low/close for Hedge")
...
class FreqtradeStrategySignalProvider:
    ...
    def signals_since(  # noqa: C901 - signal history aggregation boundary
        self,
        pair: str,
        timeframe: str,
        *,
        ...
        for raw_date in frame["date"]:
            ...
            if close_times and close_time <= close_times[-1]:
                raise ValueError("analyzed DataProvider candles must be strictly ordered")
        ...
        closed_indices = tuple(index for index, value in enumerate(close_times) if value <= as_of)
        ...
        if after is None:
            ...
            selected_indices = tuple(
                index for index in closed_indices if close_times[index] > after
            )
            ...
            for index in selected_indices:
                missing += missing_candle_count(previous_close, close_times[index], timeframe)
