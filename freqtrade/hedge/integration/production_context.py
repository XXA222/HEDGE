...
class ReadonlyPlanningContextBuilder:
    ...
    def build(
        ...
        *,
        ...
        leverage = max(
            (Decimal(max(int(item.leverage), 1)) for item in matching_positions),
            default=self.default_leverage,
        )
        raw_active_orders = tuple(
        ...
        active_orders: tuple[ActiveOrder, ...] = tuple(
            cast(ActiveOrder, item) for item in raw_active_orders if item is not None
        )
        ignored = sum(
            1
            for item in (*account_view.positions, *account_view.active_orders)
            if raw_symbol(str(getattr(item, "symbol", ""))) != symbol
        )
        ...
        effective_config = planner_config_for_directive(self.planner_config, cast(Any, directive))
    ...
    def _active_order(
        ...
        *,
        ...
        order_type = (
            OrderType.MARKET if str(order.order_type).upper() == "MARKET" else OrderType.LIMIT
        )
        return ActiveOrder(
