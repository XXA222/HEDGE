...
def _valid_perpetual_symbol(value: str) -> bool:
    return (
        ...
        and value.isalnum()
        and any(
            value.endswith(suffix) and len(value) > len(suffix)
            for suffix in _SUPPORTED_SETTLE_SUFFIXES
        )
    )
...
class ProductionEquivalentHedgeMainLoop:
    ...
    def __init__(
        ...
        *,
        ...
        if not normalized or not all(_valid_perpetual_symbol(item) for item in normalized):
            raise ValueError("allowed_symbols must contain valid USDT/USDC/FDUSD perpetual symbols")
    ...
    def bind_order_admission_provider(self, provider: Callable[[object], object]) -> None:
        ...
        self._admission_policy.add(cast(Any, provider))
    ...
    def run_cycle(  # noqa: C901 - production cycle orchestration boundary
        self,
        ...
        *,
        ...
        for original_planner_intent in submit_orders:
            planner_intent = cast(Any, self._transform_intent(original_planner_intent))
            if cancel_phase_incomplete and not planner_intent.reduce_only:
                blocked.append(planner_intent.intent_id)
                continue
            if (
                not self.new_risk_enabled or not strategy_allows_new_risk
...
def _adapt_stable(planner_intent: object, *, account_id: str, strategy_id: str, cycle_id: str):
    ...
    planner_id = str(cast(Any, planner_intent).intent_id)
    digest = sha256(f"{account_id}|{strategy_id}|{planner_id}".encode()).hexdigest()[:32]
