...
def _reference_price(order: OrderFact, positions: tuple[PositionRiskLeg, ...]) -> Decimal | None:
    ...
    for leg in positions:
        if (
            leg.symbol == canonicalize_symbol(order.symbol)
            and leg.position_side is _position_side(order.position_side)
            and leg.mark_price > 0
        ):
...
def _risk_from_view(
    ...
    position_facts = tuple(
        position_item for position_item in view.positions if position_item.quantity != 0
    )
    total_notional = sum(
        (
            abs(position_item.quantity) * position_item.mark_price
            for position_item in position_facts
        ),
        Decimal(0),
    )
    positions: list[PositionRiskLeg] = []
    for position_item in position_facts:
        quantity = abs(position_item.quantity)
        mark = (
            position_item.mark_price if position_item.mark_price > 0 else position_item.entry_price
        )
        ...
        positions.append(
            PositionRiskLeg(
                account_id=position_item.account_id,
                symbol=canonicalize_symbol(position_item.symbol),
                position_side=_position_side(position_item.position_side),
                quantity=quantity,
                mark_price=mark,
                leverage=Decimal(max(int(position_item.leverage), 1)),
                maintenance_margin=maintenance,
                liquidation_price=(
                    position_item.liquidation_price
                    if position_item.liquidation_price and position_item.liquidation_price > 0
                    else None
                ),
            )
        )
    ...
    for order_item in view.active_orders:
        remaining = max(
            order_item.original_quantity - order_item.cumulative_filled_quantity, Decimal(0)
        )
        if remaining <= 0:
            continue
        reference = _reference_price(order_item, normalized_positions)
        ...
        leverage = next(
            (
                ...
                if leg.symbol == canonicalize_symbol(order_item.symbol)
                and leg.position_side is _position_side(order_item.position_side)
            ),
            Decimal(1),
        )
        pending.append(
            PendingOrderRisk(
                ...
                symbol=canonicalize_symbol(order_item.symbol),
                position_side=_position_side(order_item.position_side),
                action=_order_action(order_item),
...
def build_central_projection(
    ...
    reasons = tuple(
        dict.fromkeys(
            str(item) for item in health.reason_codes if str(item) and str(item) != "CONSISTENT"
        )
    )
    if risk is None:
    ...
    return CentralRuntimeProjection(
        ...
        reconciliation_status=(
            "HEALTHY" if reconciliation_ok else "DRIFT" if latest_calibration else "UNKNOWN"
        ),
