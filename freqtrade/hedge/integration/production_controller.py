...
class ProductionHedgeController:
    ...
    def after_strategy_analyze(  # noqa: C901 - production cycle orchestration boundary
        self,
        ...
        if (
            self.loop_config.require_reconciliation_consistent
            and not health.reconciliation_consistent
        ):
        ...
        if (
            self._last_candle_close is not None
            and signal.candle_close_time <= self._last_candle_close
        ):
            return ProductionControllerCycle(
                False,
