...
from freqtrade.hedge.native.coordinator import NativeConvergenceCoordinator
from freqtrade.hedge.native.freqai import HedgeModelReadinessGate
from freqtrade.hedge.native.producer import HedgeProducerConsumerGate
...
class HedgeController:
    ...
    def __init__(
        ...
        *,
        ...
        self.native_convergence: NativeConvergenceCoordinator | None = None
        self.model_gate: HedgeModelReadinessGate | None = None
        self.producer_gate: HedgeProducerConsumerGate | None = None
        ...
        if (
            ...
            self._funding_provider = ExchangeFundingEventProvider(
                exchange,
                symbol=runtime.config.managed_pair or str(config.get("managed_pair", "")),
                ...
                initial_since_ms=(
                    None if last_funding_time is None else int(last_funding_time.timestamp() * 1000)
                ),
            )
    ...
    def bind_native_convergence(self, coordinator: NativeConvergenceCoordinator) -> None:
        ...
        if self.paper_application is not None:
            ...
            self.paper_application.bind_order_admission_provider(cast(Any, coordinator.admit))
        ...
        if assembly is not None:
            assembly.loop.bind_order_admission_provider(cast(Any, coordinator.admit))
            transformer = getattr(coordinator, "transform_planner_intent", None)
            if callable(transformer):
                assembly.loop.bind_order_transformer(transformer)

    def bind_model_gate(self, gate: HedgeModelReadinessGate) -> None:
        if not callable(getattr(gate, "observe", None)):
            raise TypeError("model gate must provide observe(signal)")
        self.model_gate = gate

    def bind_producer_gate(self, gate: HedgeProducerConsumerGate) -> None:
        if not callable(getattr(gate, "observe", None)):
    ...
    def _observe_native_signal(self, signal: SignalSnapshot) -> None:
        ...
        feature_schema = getattr(manifest, "feature_schema", None) or str(
    ...
    def after_strategy_analyze(  # noqa: C901 - controller cycle orchestration boundary
        self,
    ) -> HedgeControllerCycle:
        ...
        try:
            for index, signal in enumerate(signals):
                try:
                    ...
                    self.runtime.halt(f"HEDGE_NATIVE_SIGNAL_EVIDENCE_FAILED:{type(exc).__name__}")
            ...
            if market_input is None or result is None:
                raise RuntimeError("controller cycle completed without market or planning result")
