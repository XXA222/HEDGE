...
class SqlPaperAccountEventSink:
    ...
    def record(self, event: AccountEvent) -> bool:
        if event.event_type not in {AccountEventType.FEE, AccountEventType.FUNDING}:
            raise ValueError("SQL Paper account-event sink only accepts fee and funding events")
        _, created = self._persistence.record_account_event(
            ...
            position_side=(
                None
                if event.position_side is None
                else str(getattr(event.position_side, "value", event.position_side))
            ),
            raw_payload={
...
class ExchangeFundingEventProvider:
    ...
    def __init__(
        ...
        *,
        ...
        if not callable(self._history_fetch) and not callable(self._current_fetch):
            raise RuntimeError(  # noqa: TRY004 - runtime capability failure is fail-closed
        ...
        timestamp_ms = cast(
            ...
            next(
                (
                    ...
                    if (timestamp_value := _timestamp_ms(payload.get(key))) is not None
                ),
                None,
            ),
        )
        rate: Decimal | None = next(
            (
                rate_value
                for key in rate_keys
                if (rate_value := _decimal(payload.get(key))) is not None
            ),
            None,
        )
    ...
    def _history(self, bar: BarEvent) -> tuple[list[FundingEvent], bool]:
        ...
        if not isinstance(rows, Sequence) or isinstance(rows, (str, bytes)):
            raise RuntimeError("exchange funding history must be a sequence")  # noqa: TRY004
    ...
    def _current(self, bar: BarEvent) -> tuple[list[FundingEvent], bool]:
        ...
        if not isinstance(payload, Mapping):
            raise RuntimeError("exchange funding-rate response must be a mapping")  # noqa: TRY004
        result: list[FundingEvent] = []
    ...
    def collect(self, bar: BarEvent) -> FundingCollection:
        ...
        should_poll = (
            ...
            or (observed_at - self._last_poll_at).total_seconds() >= self._poll_interval_seconds
        )
        ...
        return FundingCollection(
            events=events,
            source=(
                "EXCHANGE_FUNDING_HISTORY_OR_SCHEDULE" if should_poll else "EXCHANGE_FUNDING_CACHE"
            ),
...
def fee_account_event(
    *,
    ...
    return AccountEvent(
        ...
        amount=-amount,
        position_side=cast(Any, position_side),
