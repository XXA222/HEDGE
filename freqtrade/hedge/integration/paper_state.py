    ...
    def load(self) -> Mapping[str, object] | None:
        with self._lock:
            ...
            if not isinstance(payload, dict):
                raise TypeError("paper state document must be a JSON object")
        ...
        with self._lock:
            ...
            try:
                ...
                try:
                    ...
                    logger.debug(
                        "Unable to remove temporary paper-state file %s",
                        temporary,
                        exc_info=True,
                    )
...
class SqlPaperStateStore:
    ...
    def __init__(
        ...
        *,
        ...
        self._session_factory = cast(_SessionFactory, session_factory)
    ...
    def _decode(encoded: str) -> Mapping[str, object]:
        ...
        if not isinstance(payload, dict):
            raise TypeError("SQL paper state must decode to a JSON object")
        version = payload.get("schema_version")
        if version not in {1, PAPER_STATE_SCHEMA_VERSION}:
            raise ValueError(
