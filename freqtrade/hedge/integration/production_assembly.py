...
def build_production_main_loop_assembly(
    *,
    ...
    account_id = (
        str(hedge.get("account_id", "default")).strip() if isinstance(hedge, Mapping) else "default"
    )
    ...
    if loop_config.mode is HedgeExecutionMode.HEDGE_SIMULATED:
        if paper_application is None or paper_application.execution is None:
            raise ValueError("HEDGE_SIMULATED requires the durable IntegratedPaperHedgeApplication")
        ...
        if production_runtime is not None:
            ...
            if not all(hasattr(runtime, name) for name in ("engine", "store", "kill_switch")):
                raise TypeError("production_runtime must expose engine, store, and kill_switch")
            ...
            if loop_config.mode is HedgeExecutionMode.HEDGE_PRODUCTION_ARMED:
                raise ValueError("HEDGE_PRODUCTION_ARMED requires the authoritative R5 runtime")
    ...
    for name, value in raw.items():
        ...
        if isinstance(default, Decimal):
            if isinstance(value, bool):
                raise ValueError(  # noqa: TRY004 - planner config contract
                    f"hedge.planner.{name} must be an exact decimal"
                )
            converted[name] = Decimal(str(value))
        elif isinstance(default, int) and not isinstance(default, bool):
            if isinstance(value, bool) or not isinstance(value, int):
                raise ValueError(  # noqa: TRY004 - planner config contract
                    f"hedge.planner.{name} must be an integer"
                )
            converted[name] = value
        elif isinstance(default, bool):
            if not isinstance(value, bool):
                raise ValueError(  # noqa: TRY004 - planner config contract
                    f"hedge.planner.{name} must be a boolean"
                )
            converted[name] = value
        else:
            converted[name] = value
