"""SimbaV2-inspired scalable Soft Actor-Critic for HPRL."""
...
from ..networks import CategoricalTwinCritic, SimbaGaussianActor
...
class SimbaSACAgent:
    def __init__(
        ...
        self.actor = SimbaGaussianActor(obs_dim, action_dim, **kwargs).to(self.device)
        self.critic = CategoricalTwinCritic(
            obs_dim, action_dim, **kwargs, bins=101, value_min=-5.0, value_max=5.0
        ).to(self.device)
    ...
    def _critic_loss_surface(
        ...
        with torch.no_grad(), self.precision.autocast():
            ...
            next_action, next_log_prob, _, next_mean, next_log_std = sampled
            next_action = action_for_critic(self, next_action, straight_through=False)
            if level_count >= 2:
                next_log_prob = self._selected_tier_log_prob_fn(
                    next_mean, next_log_std, next_action, boundaries
                ).sum(dim=-1, keepdim=True)
            nq1, nq2 = self.critic_target(next_obs, next_action)
            target = reward + cfg.gamma * (1.0 - done) * (
                torch.minimum(nq1, nq2) - alpha_detached * next_log_prob
            )
        with self.precision.autocast():
    ...
    def _actor_loss_surface(self, obs, alpha_detached, boundaries):
        ...
        with self.precision.autocast():
            ...
            new_action, log_prob, _, policy_mean, policy_log_std = sampled
            ...
            if level_count >= 2:
                ...
                entropy_term = -alpha_detached * tier_entropy
            else:
                tier_entropy = torch.empty((0,), device=obs.device, dtype=obs.dtype)
                entropy_term = alpha_detached * log_prob
            actor_loss = (entropy_term - torch.minimum(q1, q2)).mean()
        return actor_loss, tier_entropy, log_prob
    ...
    def update(self, batch, *, collect_metrics: bool = True) -> UpdateMetrics:
        ...
        if level_count >= 2 and tier_entropy is not None:
            target_entropy = (
                cfg.tier_entropy_target_fraction
                * float(self.action_dim)
                * math.log(float(level_count))
            )
            alpha_loss = (
