...
def _event_directories(root: Path) -> list[Path]:
    event_files = sorted(path for path in root.rglob("events.out.tfevents*") if path.is_file())
    ...
    *,
    ...
    try:
        accumulator = event_accumulator_type(
            str(directory),
            size_guidance={"scalars": max(1, int(max_points_per_tag) * 4)},
        )
        accumulator.Reload()
...
def _append_scalar_events(
    ...
    *,
    ...
    source_name = directory.relative_to(root).as_posix() if directory != root else "."
