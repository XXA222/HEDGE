...
def _load_config(config_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        load_from_files = loader.load_from_files
        payload = load_from_files([str(path)])
    except (ImportError, AttributeError):
        payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError(  # noqa: TRY004 - artifact contract preserves loader API
            "Freqtrade configuration must be an object"
        )
    return payload
...
def _load_object(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError(  # noqa: TRY004 - artifact contract preserves loader API
            f"optimization artifact must contain an object: {path.name}"
        )
    return payload
...
def materialize_best_parameter_overlay(
    ...
    *,
    ...
    if not isinstance(raw_values, Mapping):
        raise ValueError(  # noqa: TRY004 - artifact contract preserves loader API
            "best-parameters artifact does not contain parameters"
        )
...
def ranked_candidates(
    ...
    *,
    ...
    if not isinstance(raw_trials, Sequence) or isinstance(raw_trials, (str, bytes)):
        raise ValueError(  # noqa: TRY004 - artifact contract preserves loader API
            "optimization summary trials must be an array"
        )
