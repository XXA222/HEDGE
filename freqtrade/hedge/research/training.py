...
def _safe_identifier(value: str) -> str:
    ...
    if not _IDENTIFIER.fullmatch(normalized):
        raise ValueError(
            "freqai identifier must contain only letters, digits, dot, dash, underscore"
        )
    return normalized
...
def normalize_cpu_threads(value: object, *, default: int = 1) -> int:
    try:
        threads = int(str(default if value in (None, "") else value))
...
def _validate_training_kind(kind: ResearchKind) -> None:
    if kind not in {
        ...
        raise ValueError("FreqAI override is only valid for ML/RL research jobs")
...
def _training_identifier(
    ...
    *,
    ...
    resume_identifier = str(parameters.get("resume_identifier", "")).strip()
    requested_identifier = str(parameters.get("freqai_identifier", "")).strip()
    identifier = _safe_identifier(
        resume_identifier or requested_identifier or default_identifier(job_id, request.kind)
    )
    continual = bool(parameters.get("continual_learning", False) or resume_identifier)
...
def _base_freqai_override(
    ...
    *,
    ...
    freqai: dict[str, Any] = {
        ...
        "activate_tensorboard": bool(parameters.get("activate_tensorboard", True)),
...
def _apply_training_device(
    ...
    *,
    ...
    model_training = dict(freqai.get("model_training_parameters", {}))
    ...
    if request.kind in {
        ...
        if cpu_threads is not None:
            model_training["cpu_threads"] = normalize_cpu_threads(cpu_threads)
    else:
