...
class HedgeResearchService:
    ...
    def _job_from_payloads(
        ...
        return ResearchJob(
            ...
            artifacts=[
                self._artifact_from_payload(item) for item in state_payload.get("artifacts", ())
            ],
            revision=revision,
        )
    ...
    def submit(self, request: ResearchRequest) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Unable to discard failed queued research job %s",
                    job.job_id,
                    exc_info=True,
                )
            raise
    ...
    def _walk_forward_shared_identifier(
        ...
        *,
        ...
        shared_identifier = str(request.parameters.get("freqai_identifier", "")).strip()
    ...
    def _walk_forward_fold_parameters(
        ...
        *,
        ...
        if base_identifier:
            parameters["freqai_identifier"] = f"{base_identifier[:72]}-wf-{fold.index + 1:03d}"
        return parameters
    ...
    def _submit_walk_forward_fold(
        ...
        *,
        ...
        child = ResearchRequest(
            ...
            parameters=parameters,
            tags=tuple(dict.fromkeys((*request.tags, "walk-forward", group_id))),
            priority=request.priority,
    ...
    def _walk_forward_group_payload(
        ...
        *,
        ...
        return {
            ...
            "shared_freqai_identifier": (shared_identifier if continual else ""),
    ...
    def submit_walk_forward(
        ...
        *,
        ...
        continual = bool(request.parameters.get("walk_forward_continual", False))
    ...
    def walk_forward_group(self, group_id: str) -> dict[str, Any]:
        ...
        if not isinstance(payload, dict):
            raise ValueError(  # noqa: TRY004 - persisted research contract
                "walk-forward group is invalid"
            )
        jobs = []
        ...
        result["success_ratio"] = states.get("SUCCEEDED", 0) / len(jobs) if jobs else 0.0
    ...
    def _submit_optimization_replay_child(
        ...
        *,
        ...
        try:
            ...
            logger.debug(
                "Optimization replay overlay already attached to parent job %s",
                job_id,
                exc_info=True,
            )
        ...
        try:
            ...
            logger.debug(
                "Optimization replay overlay already attached to child job %s",
                child_id,
                exc_info=True,
            )
    ...
    def replay_best_optimization(
        ...
        *,
        ...
        if parent.state is not ResearchState.SUCCEEDED:
            raise ValueError(
                "optimization job must succeed before its best candidate can be replayed"
            )
        ...
        selected_timerange = (
            timerange.strip() or str(parent.request.parameters.get("replay_timerange", "")).strip()
        )
    ...
    def _append_optimization_replay(
        ...
        *,
        ...
        if not isinstance(payload, dict):
            raise ValueError(  # noqa: TRY004 - persisted research contract
                "optimization replay registry is invalid"
            )
        rows = payload.setdefault("replays", [])
        if not isinstance(rows, list):
            raise ValueError(  # noqa: TRY004 - persisted research contract
                "optimization replay registry rows are invalid"
            )
        rows.append(
            {
                "job_id": child_id,
    ...
    def _replay_leaderboard(
        ...
        ranked = [row for row in rows if ranking_metric in row.get("metrics", {})]
        ranked.sort(
    ...
    def optimization_replays(
        ...
        result["oos_leaderboard"] = (
            self._replay_leaderboard(rows, ranking_metric) if ranking_metric else []
        )
        return result
