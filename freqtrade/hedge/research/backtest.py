...
def _risk_statistics(returns: Sequence[float], periods_per_year: int) -> dict[str, float | None]:
    ...
    return {
        ...
        "sortino": (None if downside_deviation == 0 else average / downside_deviation * annualizer),
    }
...
def summarize_equity(
    ...
    *,
    ...
    return EquitySummary(
        ...
        volatility=float(risk["volatility"] or 0.0),
        sharpe=risk["sharpe"],
        downside_deviation=float(risk["downside_deviation"] or 0.0),
...
def rolling_returns(equity: Sequence[float], *, window: int) -> tuple[float, ...]:
    ...
    return tuple(
        values[index] / values[index - window] - 1.0 for index in range(window, len(values))
    )
...
def fee_sensitivity(
    ...
    return {f"{fee:g}bps": float(base_return) - total_turnover * fee / 10_000.0 for fee in fees}
