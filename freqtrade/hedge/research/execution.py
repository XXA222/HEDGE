...
class ResearchExecutionManager:
    ...
    def _resolve_budget_limits(
        ...
        trial_count = None if trials is None else min(int(trials), request.budget.max_trials)
        worker_count = None if workers is None else min(int(workers), request.budget.max_workers)
        if request.kind is ResearchKind.OPTIMIZATION:
            trial_count = request.budget.max_trials if trial_count is None else trial_count
            worker_count = request.budget.max_workers if worker_count is None else worker_count
    ...
    def _resolve_replay_overlay(
        ...
        replay_overlay = str(request.parameters.get("replay_overlay_relative_path", "")).strip()
    ...
    def _materialize_training_config(
        ...
        try:
            ...
            logger.debug(
                "Training materialization attachment was not registered for job %s",
                job_id,
                exc_info=True,
            )
    ...
    def _assert_safe_plan(plan: ResearchCommandPlan) -> None:
        if plan.exchange_write_enabled:
            raise RuntimeError("research command plan unexpectedly enables exchange writes")
        if "trade" in plan.argv:
            raise RuntimeError("live trade command is forbidden in research executor")
    ...
    def _active_cpu_threads(self) -> int:
        return sum(
            item.cpu_threads for item in self._active.values() if item.training_device == "cpu"
        )
    ...
    def _system_block_reason(self) -> str:
        ...
        if memory_percent is not None and float(memory_percent) >= self.config.max_memory_percent:
            return (
                f"system memory {float(memory_percent):.1f}% >= limit "
                f"{self.config.max_memory_percent:.1f}%"
            )
        cpu_percent = self._system.get("cpu_percent")
        if cpu_percent is not None and float(cpu_percent) >= self.config.max_cpu_percent:
            return (
                f"system cpu {float(cpu_percent):.1f}% >= limit {self.config.max_cpu_percent:.1f}%"
            )
        return ""
    ...
    def _gpu_training_block_reason(self, job_id: str) -> str:
        ...
        if free_mb < required_mb:
            return f"gpu {device} free memory {free_mb:.0f} MB < required {required_mb} MB"
        return ""
    ...
    def _observe_output_line(self, running: _RunningProcess, line: str) -> None:
        ...
        if (
            ...
            and observation.progress >= running.last_progress
            and (
            ...
            try:
                ...
                logger.debug(
                    "Research progress observation rejected for job %s",
                    running.job_id,
                    exc_info=True,
                )
    ...
    def _register_experiment_best_effort(self, job_id: str) -> None:
        try:
            ...
            logger.debug(
                "Best-effort experiment registration failed for job %s",
                job_id,
                exc_info=True,
            )
    ...
    def _write_replay_error(
        ...
        try:
            ...
            logger.warning(
                "Unable to persist optimization replay error for job %s",
                job_id,
                exc_info=True,
            )
    ...
    def _auto_replay_optimization(self, running: _RunningProcess) -> None:
        ...
        try:
            ...
            replay_timerange = str(request.parameters.get("replay_timerange", ""))
            if replay_top_n > 1:
    ...
    def _complete_successful_process(
        ...
        *,
        ...
        output_root = self.service.workspace.resolve(f"jobs/{running.job_id}/outputs")
    ...
    def _record_executor_failure(
        ...
        try:
            ...
            logger.exception(
                "Unable to record research executor failure for job %s",
                running.job_id,
            )
            ...
            if suspended:
                for process in reversed(changed):
                    try:
                        ...
                        logger.debug(
                            "Unable to roll back a partially suspended research process",
                            exc_info=True,
                        )
            action = "pause" if suspended else "resume"
            raise RuntimeError(f"unable to {action} research process tree") from exc
    ...
    def cancel(self, job_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                "Research cancel note rejected for job %s",
                job_id,
                exc_info=True,
            )
        self._terminate_process_tree(
    ...
    def _monitor_loop(self) -> None:
        while True:
            ...
            for running in active:
                ...
                if (
                    active_elapsed > job.request.budget.max_seconds
                    and not running.timed_out.is_set()
                ):
    ...
    def _runtime_row(self, running: _RunningProcess, *, state: str) -> dict[str, Any]:
        return {
            ...
            "paused_seconds": round(
                running.paused_total_seconds
                + (
                    0.0
                    if running.paused_at is None
                    else max(0.0, time.monotonic() - running.paused_at)
                ),
                3,
            ),
