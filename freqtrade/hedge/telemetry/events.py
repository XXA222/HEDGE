...
def _freeze_json(  # noqa: C901 - recursive telemetry payload boundary
    value: object,
    *,
    depth: int = 0,
    ...
    raise TypeError(f"telemetry payload contains non-serializable type: {type(value).__name__}")
...
class HedgeTelemetryEvent:
    ...
    def __post_init__(self) -> None:  # noqa: C901 - telemetry event validation boundary
        try:
            event_type = (
                self.event_type
