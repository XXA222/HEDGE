...
class HedgeMetrics:
    ...
    def __init__(self) -> None:
        self._counters: dict[tuple[str, tuple[tuple[str, str], ...]], Decimal] = defaultdict(
            Decimal
        )
        self._gauges: dict[tuple[str, tuple[tuple[str, str], ...]], Decimal] = {}
        self._lock = RLock()
    ...
    def _amount(value: Decimal | int) -> Decimal:
        if isinstance(value, (bool, float)):
            raise ValueError(  # noqa: TRY004 - metric value contract
                "metric amount must use an exact numeric value"
            )
        try:
    ...
    def error(self, reason_code: str, *, disposition: str = "unknown") -> None:
        """Record a bounded, stable domain error reason for observability."""

        self._inc(
            "error_total",
            reason_code=self._bounded_reason(reason_code),
            disposition=self._bounded_reason(disposition),
        )
    ...
    def account_risk(
        ...
        *,
        ...
        self._set("gross_long_notional", gross_long, account_id=account_id)
        self._set("gross_short_notional", gross_short, account_id=account_id)
        self._set("net_notional", net, signed=True, account_id=account_id)
        self._set("gross_exposure_ratio", gross_ratio, account_id=account_id)
        self._set("margin_utilization", margin_utilization, account_id=account_id)
        self._set("pending_risk_notional", pending_risk, account_id=account_id)
    ...
    def snapshot(self) -> dict[str, dict[str, str]]:
        with self._lock:
            result: dict[str, dict[str, str]] = {name: {} for name in self.REQUIRED_FAMILIES}
