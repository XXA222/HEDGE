...
_INLINE_AUTH = re.compile(r"(?i)(authorization\s*[:=]\s*)(?:bearer\s+)?([\"']?)[^\s,;\"']+\2")
...
def _redact(  # noqa: C901 - recursive telemetry redaction boundary
    value: Any,
    *,
    depth: int,
    budget: list[int],
    ...
    if isinstance(value, Mapping):
        ...
        try:
            ...
            for raw_key, item in value.items():
                ...
                if not key or len(key) > 256 or any(ord(ch) < 32 or ord(ch) == 127 for ch in key):
    ...
    if isinstance(value, float):
        ...
        return value
    raise TypeError(f"structured log contains unsupported type: {type(value).__name__}")
