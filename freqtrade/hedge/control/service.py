...
class HedgeControlService:
    ...
    def execute(
        ...
        *,
        ...
        if request.action in _CONFIRMATION_REQUIRED and (
            confirmation_token is None
            or not self.confirmations.consume(
    ...
    def _dispatch(
        ...
        *,
        ...
        if action is ControlAction.CANCEL_MANAGED_ORDERS:
            ...
            cancel_report = self.loop.cancel_managed_orders(symbol=request.symbol)
            return self._success(
                principal,
                request,
                operation_id,
                ...
                writes_attempted=len(cancel_report.canceled),
                planned=plan,
                executed=tuple(item.order.client_order_id for item in cancel_report.canceled),
                errors=tuple(item.message for item in cancel_report.errors),
            )
        if action in {
            ControlAction.CLOSE_LONG,
            ControlAction.CLOSE_SHORT,
    ...
    def _authorize(principal: HedgePrincipal, action: ControlAction) -> None:
        ...
        if principal.role < required:
            raise ControlPermissionError(f"{action.value} requires {required.name} role")
