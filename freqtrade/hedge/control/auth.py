...
def _scope_text(value: object, *, field_name: str) -> str:
    ...
    result = value.strip()
    if not result or len(result) > 128 or any(ord(ch) < 32 or ord(ch) == 127 for ch in result):
...
def _confirmation_scope(
    *,
    ...
    idempotency_text = _optional_scope_text(idempotency_key, field_name="idempotency_key")
    scope = "\x1f".join(
        (
            subject_text,
            action_text,
...
class ConfirmationService:
    ...
    def _purge_locked(self, now: datetime) -> None:
        expired = [
            (token, scope) for token, (scope, deadline) in self._issued.items() if now >= deadline
        ]
