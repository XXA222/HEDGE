...
def build_hedge_control_service(
    *,
    ...
    if production_assembly is None:
        raise OperationalException("hedge.control_plane requires hedge.main_loop.enabled=true")
    ...
    if len(jwt_secret) < 16:
        raise OperationalException("hedge.control_plane requires api_server.jwt_secret_key")
    ...
    if loop.mode is HedgeExecutionMode.HEDGE_SIMULATED:
        ...
        account_view = PaperAccountViewProvider(cast(Any, paper_application))
    ...
    secret = sha256(
        ("freqtrade-hedge-control-v1\x1f" + loop.account_id + "\x1f" + jwt_secret).encode("utf-8")
    ).digest()
    service = HedgeControlService(
        ...
        exchange_write_surface=str(getattr(production_assembly, "exchange_write_surface", "NONE")),
