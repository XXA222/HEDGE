...
class SqlControlOperationStore:
                ...
                try:
                    ...
                    with self._session_factory.begin() as retry_session:
                        ...
                        return self._claim_from_row(
                            existing, request, acquire_stale=True, actor=actor
                        )
                return ControlOperationClaim(
    ...
    def _claim_from_row(
        ...
        *,
        ...
        request_hash = str(typed_row.request_hash)
        if request_hash != request.request_hash:
            raise ControlOperationConflict("idempotency key payload mismatch")
        raw_result = typed_row.result_json
        result = (
            None
            if not raw_result
            else ControlOperationResult.from_dict(json.loads(str(raw_result)))
        )
        operation_id = UUID(str(typed_row.operation_id))
        created_at = typed_row.created_at.replace(tzinfo=UTC)
        ...
        expires = getattr(typed_row, "lease_expires_at", None)
        ...
        if actor is None or str(typed_row.actor) != actor:
            raise ControlOperationConflict("stale operation actor mismatch")
        typed_row.lease_owner = self._owner
        typed_row.lease_expires_at = (now + timedelta(seconds=self._lease_seconds)).replace(
            tzinfo=None
        )
        typed_row.revision = int(getattr(typed_row, "revision", 0)) + 1
        return ControlOperationClaim(operation_id, created_at, recovered_stale=True)
