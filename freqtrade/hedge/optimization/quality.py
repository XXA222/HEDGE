...
def validate_worker_budget(*, workers: int, trials: int) -> None:
    ...
    if workers < -1 or trials < 1:
        raise ValueError("workers must be -1, 0, or positive; trials must be positive")
    if workers > 1 and workers > trials:
        raise ValueError("explicit workers cannot exceed trial count")
...
def grid_cardinality(specs: Sequence[ParameterSpec]) -> int:
    ...
    for spec in specs:
        if spec.kind is ParameterKind.BOOLEAN:
            ...
            integer_step = _integer_step(spec)
            width = ((integer_high - integer_low) // integer_step) + 1
        else:
            ...
            if spec.step is None:
                ...
                decimal_step = _decimal_step(spec)
                width = int((decimal_high - decimal_low) / decimal_step) + 1
        total *= width
    return total
...
def validate_step_alignment(specs: Sequence[ParameterSpec]) -> None:
    for spec in specs:
        if spec.kind is ParameterKind.DECIMAL and spec.step is not None:
            decimal_low, decimal_high = _decimal_bounds(spec)
            decimal_step = _decimal_step(spec)
            quotient = (decimal_high - decimal_low) / decimal_step
            ...
            integer_low, integer_high = _integer_bounds(spec)
            integer_step = _integer_step(spec)
            if (integer_high - integer_low) % integer_step:
    ...
    for spec in specs:
        ...
        for name, value in (("low", spec.low), ("high", spec.high), ("step", spec.step)):
            ...
            if not isinstance(exponent, int):
                raise TypeError(f"{spec.name}.{name} must be finite")
...
def validate_optimization_definition(config: HedgeOptimizationConfig) -> None:
