...
def spearman_rank_correlation(left: Sequence[object], right: Sequence[object]) -> Decimal:
    ...
    left_rank = {value: index for index, value in enumerate(sorted(left, key=str))}
    right_rank = {value: index for index, value in enumerate(sorted(right, key=str))}
...
def selection_entropy(selection_counts: Mapping[str, int]) -> Decimal:
    ...
    if total <= 0:
        return ZERO
    entropy = -sum(
        (count / total) * log(count / total) for count in selection_counts.values() if count
    )
...
def knee_point(points: Sequence[tuple[object, object]]) -> int:
    ...
    normalized = [(float(str(x)), float(str(y))) for x, y in points]
...
def epsilon_pareto(
    ...
    *,
    ...
    vectors = {
        name: tuple(Decimal(str(value)) for value in vector) for name, vector in points.items()
    }
    width = len(eps)
    if (
        width == 0
