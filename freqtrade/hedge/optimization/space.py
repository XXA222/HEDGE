...
def _decimal_grid(spec: ParameterSpec) -> tuple[Decimal, ...]:
    ...
    if not values or values[-1] != high:
        raise ValueError(f"decimal range for {spec.name} is not exactly divisible by step {step}")
    return tuple(values)
...
def _integer_grid(spec: ParameterSpec) -> tuple[int, ...]:
    ...
    if not values or values[-1] != high:
        raise ValueError(f"integer range for {spec.name} is not exactly divisible by step {step}")
    return values
...
class ParameterSpace:
    ...
    def _sample_one(spec: ParameterSpec, rng: Random) -> object:
        ...
        if spec.kind is ParameterKind.INTEGER:
            integer_values = _integer_grid(spec) if spec.step is not None else None
            if integer_values is not None:
                return integer_values[rng.randrange(len(integer_values))]
            integer_low, integer_high = _integer_bounds(spec)
            return rng.randint(integer_low, integer_high)
        decimal_low, decimal_high = _decimal_bounds(spec)
        if spec.step is not None:
            decimal_values = _decimal_grid(spec)
            return decimal_values[rng.randrange(len(decimal_values))]
        if decimal_low == decimal_high:
            return decimal_low
        unit = Decimal(str(rng.random()))
        if spec.log:
            sampled_float = exp(
                log(float(decimal_low))
                + float(unit) * (log(float(decimal_high)) - log(float(decimal_low)))
            )
            sampled = Decimal(str(sampled_float))
        else:
            sampled = decimal_low + (decimal_high - decimal_low) * unit
        low_exponent = decimal_low.as_tuple().exponent
        high_exponent = decimal_high.as_tuple().exponent
        if not isinstance(low_exponent, int) or not isinstance(high_exponent, int):
            raise TypeError("decimal sampling requires finite bounds")
        exponent = min(low_exponent, high_exponent, -12)
