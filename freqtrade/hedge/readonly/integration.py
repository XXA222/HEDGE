...
def _mapping(value: Any, *, field: str) -> Mapping[str, Any]:
    ...
    if not isinstance(value, Mapping):
        raise ValueError(  # noqa: TRY004 - readonly config contract
...
def _sequence(value: Any, *, field: str) -> tuple[str, ...]:
    ...
    if not isinstance(value, Sequence) or isinstance(value, (bytes, bytearray)):
        raise ValueError(  # noqa: TRY004 - readonly config contract
            f"{field} must be a string or sequence"
        )
...
def _credentials(
    ...
    *,
    ...
    resolved_secret = api_secret if api_secret is not None else exchange.get("secret", "")
