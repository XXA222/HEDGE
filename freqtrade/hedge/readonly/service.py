...
class BinanceReadonlyService:
    ...
    def __init__(
        ...
        *,
        ...
        self._status = ServiceStatus(ReadonlyState.STARTING, "NOT_STARTED", self.clock.now(), None)
        ...
        self._last_calibration_by_kind: dict[CalibrationKind, CalibrationResult] = {}
    ...
    def _set_state(self, state: ReadonlyState, reason: str, run_id: str | None = None) -> None:
    ...
    def _record_calibration(self, result: CalibrationResult) -> CalibrationResult:
        self._last_calibration_by_kind[result.kind] = result
        return result

    async def _run_calibration(self, kind: CalibrationKind) -> CalibrationResult:
        result = await self.calibration.run(kind)
        return self._record_calibration(result)

    def latest_calibration(self, kind: CalibrationKind) -> CalibrationResult | None:
    ...
    def _bundle_health_values(self) -> tuple[int, bool]:
        ...
        if bundle is None:
            return 0, False
        external_orders = sum(1 for item in bundle.open_orders if item.active and item.quarantined)
        configuration = getattr(bundle, "configuration", None)
        leverage = (
            {} if configuration is None else getattr(configuration, "leverage_by_symbol_side", {})
        )
    ...
    def _clock_sync_health(self) -> tuple[bool, list[str]]:
        ...
        synchronized = bool(
            clock_status is not None and getattr(clock_status, "synchronized", False)
        )
        reasons: list[str] = []
    ...
    def _freshness_health(self, freshness: FreshnessAssessment) -> tuple[bool, bool, list[str]]:
    ...
    def _direction2_health(self, freshness: FreshnessAssessment) -> Direction2HealthFact:
        ...
        return Direction2HealthFact(
            ...
            unmanaged_position_count=(0 if latest is None else len(latest.unmanaged_positions)),
            unmanaged_order_count=(0 if latest is None else len(latest.unmanaged_orders)),
    ...
    def account_view(self) -> ReadonlyAccountView:
        ...
        return ReadonlyAccountView(
            ...
            positions=tuple(sorted(positions, key=lambda item: (item.symbol, item.position_side))),
            active_orders=tuple(
                sorted(
    ...
    def runtime_snapshot(self) -> ReadonlyRuntimeSnapshot:
        freshness = self.freshness.assess(self.stream.health, now=self.clock.now())
        ...
        return ReadonlyRuntimeSnapshot(
            ...
            stream_health=self.stream.health,
            freshness=freshness,
            last_fast_calibration=self.latest_calibration(CalibrationKind.FAST),
            last_full_calibration=self.latest_calibration(CalibrationKind.FULL),
            last_reconnect_calibration=self.latest_calibration(CalibrationKind.RECONNECT),
    ...
    async def preflight_and_bootstrap(self) -> None:
        ...
        try:
            ...
            self._set_state(ReadonlyState.HALT, f"PREFLIGHT_FAILED:{type(exc).__name__}")
            raise
    ...
    async def on_stream_event(self, _observed_at: datetime) -> None:
        ...
        if self._status.state in {ReadonlyState.DEGRADED, ReadonlyState.READY}:
            assessment = self.freshness.assess(self.stream.health, now=self.clock.now())
            latest = self._latest_health_calibration()
    ...
    async def on_recalibration_required(self, reason: str, _payload: Any) -> None:
        if self._status.state is ReadonlyState.HALT:
            return
        self._set_state(
            ReadonlyState.RECOVERING,
    ...
    async def _verify_repaired_drift(self, result: CalibrationResult) -> CalibrationResult:
    ...
    async def on_scheduled_calibration(self, result: CalibrationResult) -> None:
        if self._status.state is ReadonlyState.HALT:
            return
        ...
        if result.consistent:
            ...
            if self.stream.health.connected and self._status.state not in {
                ReadonlyState.RECOVERING,
    ...
    async def calibrate_now(
        ...
        async with self._recovery_lock:
            if self._status.state in {ReadonlyState.HALT, ReadonlyState.STOPPED}:
                raise RuntimeError("Cannot calibrate a HALT or STOPPED read-only service")
            self._set_state(
                ReadonlyState.CALIBRATING,
    ...
    async def _handle_freshness_failure(self, assessment: FreshnessAssessment) -> None:
        ...
        if assessment.reason == "USER_STREAM_STALE":
            ...
            try:
                async with self._recovery_lock:
                    ...
                    await self._reseed_stream_from_last_bundle()
                    self.stream.mark_calibrated(result.completed_at)
                    state = ReadonlyState.READY if result.consistent else ReadonlyState.DEGRADED
                    self._set_state(
                        state,
