...
class SoakObservation:
    ...
    def __post_init__(self) -> None:
        ...
        if self.stream_age_seconds is not None and (
            not math.isfinite(self.stream_age_seconds) or self.stream_age_seconds < 0
        ):
        ...
        if not isinstance(self.funding_period_observed, bool):
            raise ValueError(  # noqa: TRY004 - soak monitor state contract
                "funding_period_observed must be a boolean"
            )
    ...
    def expected_wallet_delta(self) -> Decimal:
        return self.funding_income + self.realized_pnl - self.commissions + self.transfers
...
class SoakMonitor:
        ...
        for line_number, line in enumerate(
            ...
            try:
                ...
                observation = SoakObservation(
                    ...
                    funding_period_observed=raw.get("funding_period_observed", False),
                    funding_event_count_delta=int(raw.get("funding_event_count_delta", 0)),
                ...
                raise ValueError(f"Invalid soak observation at line {line_number}: {exc}") from exc
    ...
    def _timeline_metrics(
        ...
        coverage_ratio = 0.0 if expected_count == 0 else min(1.0, len(ordered) / expected_count)
    ...
    def _quality_metrics(
        ...
        non_ready_count = sum(1 for item in ordered if item.service_state != "READY")
        diff_count = sum(
            1 for item in ordered if item.fast_diff_count != 0 or item.full_diff_count != 0
        )
    ...
    def summarize(
        ...
        clean = continuous and drift_count == 0 and non_ready_count == 0 and diff_count == 0
...
class SoakAccountingSource(Protocol):
    async def read_soak_accounting_totals(self, account_id: str) -> SoakAccountingTotals: ...
...
class ReadonlyServiceSoakProvider:
    ...
    async def read_observation(self) -> SoakObservation:
        totals = await self.accounting_source.read_soak_accounting_totals(self.account_id)
...
def main() -> int:
    ...
    monitor = SoakMonitor(args.path, expected_interval_seconds=args.expected_interval_seconds)
    summary = monitor.summarize()
    print(
        json.dumps(
            {
                **asdict(summary),
                "max_abs_unexplained_drift": str(summary.max_abs_unexplained_drift),
            },
            indent=2,
            sort_keys=True,
        )
