...
class ReadonlyCalibration:
    def __init__(
        ...
        *,
        ...
        if isinstance(managed_symbols, (str, bytes)):
            raise ValueError(  # noqa: TRY004 - calibration config contract
                "managed_symbols must be a sequence, not a string"
            )
    ...
    async def _load_history_cursor(self) -> None:
        ...
        if not callable(loader):
            return
        value = await maybe_await(loader(self.client.account_id, HISTORY_CURSOR_NAME))
        if value is None:
            return
        parsed = int(value)
        if parsed < 0:
    ...
    async def _history_start_time_ms(self, started_at: datetime) -> int:
        ...
        lookback_start = max(0, int((started_at - self.fill_lookback).timestamp() * 1000))
    ...
    async def _run_once(self, kind: CalibrationKind) -> CalibrationResult:
        ...
        start_time_ms = await self._history_start_time_ms(started_at) if include_history else None
        try:
            ...
            result, diffs = await self._apply_bundle(run_id, kind, started_at, bundle)
    ...
    def _external_order_diffs(self, bundle: BinanceAccountBundle) -> list[ReconciliationDiffFact]:
        return [
            ReconciliationDiffFact(
                account_id=self.client.account_id,
                entity_type="ORDER",
            ...
            for item in bundle.open_orders
            if item.active and item.symbol in self.managed_symbols and item.quarantined
        ]
    ...
    def _blocking_diffs(
        ...
        return tuple(item for item in diffs if item.severity.upper() in {"ERROR", "CRITICAL"})
    ...
    def _income_events(
        ...
        for item in bundle.income_events:
            ...
            transaction_identity = str(item.get("tranId") or item.get("tradeId") or "")
    ...
    async def _persist_bundle(
        ...
        *,
        ...
        if bundle.fills:
            await self.repository.append_fill_events(bundle.fills, reconciliation_run_id=run_id)
    ...
    async def _apply_bundle(
        ...
        local_positions = tuple(await self.repository.load_active_positions(self.client.account_id))
        local_orders = tuple(await self.repository.load_active_orders(self.client.account_id))
        ...
        await self._persist_bundle(run_id, bundle, completed_at=completed_at, diffs=resolved_diffs)
        ...
        result = CalibrationResult(
            ...
            reason=self._result_reason(unmanaged_positions, unmanaged_orders, blocking_diffs),
        )
