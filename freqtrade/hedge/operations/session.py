...
class RunSession:
    ...
    def __post_init__(self):
        ...
        if self.last_cycle_at is not None:
            ensure_aware(self.last_cycle_at)
        if not self.session_id or not self.account_id or not self.symbols:
            raise ValueError("session identity fields are required")
        if self.cycle_sequence < 0:
            raise ValueError("cycle_sequence must be nonnegative")
    ...
    def create(
        ...
        *,
        ...
        ts = ensure_aware(started_at or utc_now())
        normalized = tuple(dict.fromkeys(x.strip().upper() for x in symbols if x.strip()))
        if not normalized:
            raise ValueError("at least one symbol is required")
        config_hash = sha256_value(config)
        sid = sha256_value(
            {"account": account_id, "symbols": normalized, "config": config_hash, "started": ts}
        )[:24]
    ...
    def next_cycle(self, at: datetime) -> tuple[RunSession, str]:
        ...
        if self.last_cycle_at is not None and ts <= self.last_cycle_at:
            raise ValueError("cycle time must be strictly monotonic")
        sequence = self.cycle_sequence + 1
        cycle_id = f"{self.session_id}:{sequence:012d}"
