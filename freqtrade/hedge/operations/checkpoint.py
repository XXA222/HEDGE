...
class RuntimeCheckpointManager:
    ...
    def create(self, payload: dict[str, object], *, at: datetime) -> RuntimeCheckpoint:
        ts = ensure_aware(at)
        body = {"created_at": ts, "payload": payload}
        digest = sha256_value(body)
        checkpoint = RuntimeCheckpoint(digest[:20], ts, payload, digest)
        self.directory.mkdir(parents=True, exist_ok=True)
        atomic_write_text(
            self.directory / f"{checkpoint.checkpoint_id}.json",
            canonical_json({"created_at": ts, "payload": payload, "sha256": digest}),
        )
        self._prune()
        return checkpoint
    ...
    def restore(self, checkpoint_id: str) -> RuntimeCheckpoint:
        ...
        body = {"created_at": datetime.fromisoformat(raw["created_at"]), "payload": raw["payload"]}
        if raw.get("sha256") != sha256_value(body):
