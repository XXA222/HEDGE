...
class AtomicRunStateStore:
    ...
    def save(self, session: RunSession, extras: dict[str, object] | None = None) -> None:
        ...
        with self._lock:
            atomic_write_text(self.path, canonical_json(envelope))
    ...
    def load(self) -> tuple[RunSession, dict[str, object]] | None:
        with self._lock:
            ...
            try:
                ...
                raise StateCorruptionError("runtime state is unreadable") from exc
            if envelope.get("sha256") != sha256_value(body):
                raise StateCorruptionError("runtime state checksum mismatch")
            if int(body.get("schema_version", 0)) != self.schema_version:
                raise StateCorruptionError("unsupported runtime state schema")
            raw = body["session"]
            try:
                session = RunSession(
                    ...
                    cycle_sequence=int(raw["cycle_sequence"]),
                    last_cycle_at=None
                    if raw.get("last_cycle_at") is None
                    else datetime.fromisoformat(raw["last_cycle_at"]),
                    reason=str(raw.get("reason", "")),
                )
            except Exception as exc:
                raise StateCorruptionError("runtime state session is invalid") from exc
            extras = body.get("extras", {})
            return session, extras if isinstance(extras, dict) else {}
    ...
    def load_fail_closed(self) -> tuple[RunSession | None, dict[str, object], tuple[str, ...]]:
        try:
            ...
            return None, {}, (f"STATE_RECOVERY_FAILED:{exc}",)
