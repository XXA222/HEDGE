...
def _validate_safety(
    config: Mapping[str, Any], hedge: Mapping[str, Any], raw: Mapping[str, Any]
) -> list[str]:
    errors: list[str] = []
...
def _validate_integer_rules(raw: Mapping[str, Any]) -> list[str]:
    errors: list[str] = []
    integer_rules = (
...
def _validate_informative_warmup(raw: Mapping[str, Any]) -> list[str]:
    informative = raw.get("informative_warmup", {})
    if not isinstance(informative, Mapping):
        return ["OPERATIONS_INFORMATIVE_WARMUP_INVALID"]
    errors: list[str] = []
...
def _validate_risk_ratios(raw: Mapping[str, Any]) -> list[str]:
    try:
        values = tuple(
            _decimal(raw.get(key, default), key)
            for key, default in (
                ("max_gross_ratio", "0.80"),
                ("max_margin_ratio", "0.55"),
                ("max_net_ratio", "0.50"),
            )
        )
        ...
        return ["OPERATIONS_RISK_RATIOS_INVALID"]
    return []


def _validate_drawdown_thresholds(raw: Mapping[str, Any]) -> list[str]:
    try:
        recovery, warning, pause, kill = tuple(
            _decimal(raw.get(key, default), key)
            for key, default in (
        ...
        return ["OPERATIONS_DRAWDOWN_THRESHOLDS_INVALID"]
    return []
...
def validate_operations_config(config: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    errors = ["OPERATIONS_UNKNOWN_KEYS:" + ",".join(unknown)] if unknown else []
    errors.extend(_validate_safety(config, hedge, raw))
    errors.extend(_validate_integer_rules(raw))
    errors.extend(_validate_informative_warmup(raw))
    errors.extend(_validate_risk_ratios(raw))
    errors.extend(_validate_drawdown_thresholds(raw))
