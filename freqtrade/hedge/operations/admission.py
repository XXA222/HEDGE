...
class IntentAdmissionGate:
    def __init__(self, *, min_notional: Decimal, max_notional: Decimal, max_open_orders: int = 20):
        if (
            min_notional < ZERO
            or max_notional <= ZERO
            or min_notional > max_notional
    ...
    def evaluate(
        ...
        *,
        ...
        if not intent.intent_id or intent.quantity <= ZERO or intent.price <= ZERO:
        ...
        if not intent.reduce_only:
            if not new_risk_enabled:
                reasons.append("NEW_RISK_PAUSED")
            if cooldown_until is not None and ts < ensure_aware(cooldown_until):
