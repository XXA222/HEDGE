...
class StrategyParameterRegistry:
    ...
    def create(self, values: dict[str, Decimal], *, actor: str, at: datetime) -> ParameterVersion:
        ts = ensure_aware(at)
        unknown = set(values) - set(self.bounds)
        if unknown:
            raise ValueError("unknown parameter(s): " + ",".join(sorted(unknown)))
        for name, bound in self.bounds.items():
            if name not in values:
                raise ValueError(f"missing parameter: {name}")
            if not bound.minimum <= values[name] <= bound.maximum:
                raise ValueError(f"parameter out of bounds: {name}")
        ordered = tuple(sorted(values.items()))
        digest = sha256_value({"at": ts, "actor": actor, "values": ordered})
        version = ParameterVersion(digest[:16], ts, actor[:128], ordered, digest)
    ...
    def active(self) -> ParameterVersion | None:
        return None if self._active is None else self._versions[self._active]

    def rollback(self) -> ParameterVersion:
        if self._active is None or self._active < 1:
            raise RuntimeError("no previous parameter version")
        self._active -= 1
        return self._versions[self._active]

    def versions(self) -> tuple[ParameterVersion, ...]:
