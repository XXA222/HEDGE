...
class FeeFundingLedger:
    ...
    def append(self, event: LedgerEvent) -> bool:
        ...
        if not event.event_id or not event.currency:
            raise ValueError("event identity is required")
        existing = self._events.get(event.event_id)
        if existing is not None:
            if existing != event:
                raise ValueError("ledger event id collision")
            return False
    ...
    def balance(self, currency: str = "USDT") -> LedgerBalance:
        ...
        net = (
            totals[LedgerEventType.REALIZED_PNL]
            + totals[LedgerEventType.FUNDING]
            + totals[LedgerEventType.ADJUSTMENT]
            - totals[LedgerEventType.FEE]
        )
    ...
    def events(self) -> tuple[LedgerEvent, ...]:
        return tuple(sorted(self._events.values(), key=lambda x: (x.timestamp, x.event_id)))
