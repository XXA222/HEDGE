    ...
    def __init__(
        ...
        *,
        ...
        if self.state_store is not None:
            ...
            if recovered is not None:
                ...
                if identity_matches and recovered.status is SessionStatus.RUNNING:
                    ...
                    try:
                        self.total_orders = max(0, int(str(extras.get("total_orders", 0))))
                        self.total_fills = max(0, int(str(extras.get("total_fills", 0))))
                    except (TypeError, ValueError):
                        self.recovery_reasons = ("STATE_COUNTERS_INVALID",)
                elif not identity_matches:
                    self.recovery_reasons = ("STATE_IDENTITY_MISMATCH",)
                else:
                    self.recovery_reasons = (f"STATE_NOT_RESUMABLE:{recovered.status.value}",)
        ...
        self.market = MarketDataHealthGate(
            ...
            max_divergence_bps=Decimal(str(operations.get("mark_index_max_divergence_bps", "50"))),
        )
