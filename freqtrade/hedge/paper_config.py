...
def _choice(value: object, *, field: str, default: str, choices: set[str]) -> str:
    ...
    if normalized not in choices:
        raise OperationalException(f"{field} must be one of: {', '.join(sorted(choices))}")
    return normalized
...
class PaperSimulationConfig:
    ...
    def from_hedge_mapping(
        ...
        if raw_simulation:
            ...
            logger.warning("hedge.simulation is deprecated; values were merged into hedge.paper")
        ...
        if not account_events_enabled and not ephemeral:
            raise OperationalException(
                "hedge.paper.account_events_enabled=false is test-only and requires ephemeral=true"
            )
        ...
        if result.max_missing_candles >= result.max_catchup_candles:
            raise OperationalException(
                "hedge.paper.max_missing_candles must be lower than max_catchup_candles"
            )
