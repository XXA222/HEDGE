...
class OptimizationConfig(TypedDict):
    ...
    max_candidates: int
    workers: int
    walk_forward: dict[str, int | bool]
...
def load_optimization_config(path: Path) -> OptimizationConfig:
    ...
    walk_forward = {
        ...
        "anchored": _strict_bool(walk_raw.get("anchored", False), field="walk_forward.anchored"),
    }
    return {
        "method": method,
        ...
        "workers": _strict_int(raw.get("workers", 0), field="workers", minimum=-1),
