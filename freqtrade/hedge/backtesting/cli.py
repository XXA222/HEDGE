...
from .spaces import ParameterSpec, grid_candidates, random_candidates
...
def _candidates(config: Mapping[str, object]):
    method = cast(SearchMethod, config["method"])
    space = cast(Mapping[str, ParameterSpec], config["space"])
    if method is SearchMethod.GRID:
        return grid_candidates(space, max_candidates=int(str(config["max_candidates"])))
    if method is SearchMethod.RANDOM:
        return random_candidates(
            space,
            count=int(str(config["random_count"])),
            seed=int(str(config["seed"])),
        )
    raise ValueError(
        "CLI Optuna mode requires the optional bridge and is not used by the dependency-free CLI"
    )
...
def _self_test() -> dict[str, object]:
    ...
    events: list[SignalEvent | BarEvent | FundingEvent] = []
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "self-test":
            print(
                json.dumps(json_value(_self_test()), ensure_ascii=False, indent=2, sort_keys=True)
            )
            return 0
        ...
        if args.command == "optimize":
            runner = HedgeBacktestRunner(
                dataset=dataset,
                engine_config=cast(EngineConfig, config["engine_config"]),
                planner_config=cast(Any, config["planner_config"]),
                objective_config=cast(ObjectiveConfig, config["objective_config"]),
            )
            parallel = evaluate_parallel(
                runner=runner, candidates=candidates, workers=int(str(config["workers"]))
            )
            summary = _summary_from_evaluations(
                cast(SearchMethod, config["method"]), parallel.evaluations
            )
        ...
        folds = walk_forward_splits(
            dataset,
            train_bars=int(str(walk_raw["train_bars"])),
            test_bars=int(str(walk_raw["test_bars"])),
            step_bars=int(str(walk_raw["step_bars"])),
            gap_bars=int(str(walk_raw["gap_bars"])),
            anchored=bool(walk_raw["anchored"]),
        )
        ...
        print(
            json.dumps(
                json_value({"status": "PASS", "folds": len(result.folds), "output": args.output}),
                ensure_ascii=False,
                indent=2,
                sort_keys=True,
            )
