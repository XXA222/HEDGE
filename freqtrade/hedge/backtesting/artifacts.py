...
def write_optimization_artifacts(
    ...
    *,
    ...
    metric_names = sorted({name for item in summary.evaluations for name in item.metrics})
    ...
    with temporary.open("w", newline="", encoding="utf-8-sig") as handle:
        ...
        for rank, item in enumerate(ranked, 1):
            ...
            row.update(
                {f"parameter.{key}": value for key, value in item.candidate.parameters.items()}
            )
            row.update({f"metric.{key}": value for key, value in item.metrics.items()})
            writer.writerow(json_value(row))  # type: ignore[arg-type]
