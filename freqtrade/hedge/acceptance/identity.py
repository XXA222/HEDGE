    ...
    *,
    ...
    for symbol in sorted({str(value).strip().upper() for value in managed_symbols}):
        for side in VALID_SIDES:
            ...
            matched_position: PositionFact | None = by_key.get((symbol, side))
            identities.append(
                LegIdentity(
                    account_id=account_id,
                    symbol=symbol,
                    position_side=side,
                    present_in_rest=matched_position is not None,
                    quantity=(
                        Decimal(0) if matched_position is None else matched_position.quantity
                    ),
...
def leverage_mismatches(
    ...
    if target_leverage is None:
        return ()
    return tuple(
        sorted(key for key, value in leverage_by_side.items() if int(value) != int(target_leverage))
    )
