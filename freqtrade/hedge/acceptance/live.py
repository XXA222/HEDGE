...
def _synthetic_schema_event(event_type: str, *, now_ms: int, trade_id: str = "") -> EventEnvelope:
...
def _fresh_ledger_recovery(ledger_path: Path, *, account_id: str) -> tuple[Any, int]:
    ...
    recovery_sessions = sessionmaker(bind=recovery_engine, autoflush=False, expire_on_commit=False)
    try:
    ...
    *,
    ...
    try:
        ...
        engine.round14_out_of_order(
            _synthetic_schema_event("ORDER_TRADE_UPDATE", now_ms=now_ms + 20, trade_id="newer"),
            _synthetic_schema_event("ORDER_TRADE_UPDATE", now_ms=now_ms + 10, trade_id="older"),
        )
        _require_round_passed(engine)
