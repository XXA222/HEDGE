...
class ProductionExecutionGate:
    ...
    def assert_order_allowed(self, approved: ApprovedOrderIntent) -> ExecutionPermit:
        ...
        expected_account = f"{self.evidence.account_id_prefix}:{self.evidence.account_fingerprint}"
    ...
    def _evidence_reasons(self) -> tuple[str, ...]:  # noqa: C901 - evidence aggregation boundary
        value = self.evidence
        reasons: list[str] = []
        if value.environment is ExecutionEnvironment.DISABLED:
            reasons.append("EXECUTION_ENVIRONMENT_DISABLED")
...
def _decimal(value: object, field_name: str) -> Decimal:
    if isinstance(value, (bool, float)):
        raise ValueError(  # noqa: TRY004 - decimal domain contract
            f"{field_name} must use an exact decimal"
        )
    try:
