...
class InMemoryExecutionLedger:
    ...
    def record(
        ...
        *,
        ...
        with self._lock:
            if fill is not None:
                if fill.position_key is None:
                    raise ValueError("fill must expose position_key")
                fill_key = (
    ...
    def fills(self) -> tuple[FillEvent, ...]:
        with self._lock:
            return tuple(
                sorted(self._fills.values(), key=lambda item: (item.observed_time, item.trade_id))
            )
    ...
    def positions(
        self, *, account_id: str | None = None, symbol: str | None = None
    ) -> tuple[PositionProjection, ...]:
        with self._lock:
