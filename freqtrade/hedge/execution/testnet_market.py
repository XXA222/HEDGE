...
class BinanceTestnetMarketProbe:
    ...
    def rules(self, symbol: str) -> TestnetSymbolRules:
        ...
        if not isinstance(exchange_info, Mapping):
            raise RuntimeError(  # noqa: TRY004 - remote payload failure contract
                "Testnet exchangeInfo returned invalid payload"
            )
        rows = exchange_info.get("symbols")
        if not isinstance(rows, list):
            raise RuntimeError(  # noqa: TRY004 - remote payload failure contract
                "Testnet exchangeInfo symbols are missing"
            )
        row = next(
            (
                item
        ...
        if not isinstance(premium, Mapping):
            raise RuntimeError(  # noqa: TRY004 - remote payload failure contract
                "Testnet premiumIndex returned invalid payload"
            )
...
def _positive_decimal(value: object, field_name: str) -> Decimal:
    if isinstance(value, (bool, float)):
        raise ValueError(  # noqa: TRY004 - decimal domain contract
            f"{field_name} must use exact decimal text"
        )
    try:
