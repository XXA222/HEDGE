...
class FakeHedgeAccount:
    ...
    def restore(self, rows: object) -> None:
        ...
        if isinstance(rows, (str, bytes)):
            raise ValueError(  # noqa: TRY004 - persisted snapshot contract
                "paper account rows must be an iterable of mappings"
            )
        try:
            items = tuple(cast(Iterable[object], rows))
        ...
        for raw in items:
            if not isinstance(raw, dict):
                raise ValueError(  # noqa: TRY004 - persisted snapshot contract
                    "paper account row must be a mapping"
                )
