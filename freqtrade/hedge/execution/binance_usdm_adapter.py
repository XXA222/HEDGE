...
class UrlLibHttpTransport:
    ...
    def request(
        ...
        if parsed_url.scheme not in {"http", "https"} or not parsed_url.netloc:
            raise ValueError("Binance HTTP endpoint must use http or https")
...
class BinanceUSDMExecutionAdapter(ExchangeExecutionPort):
        ...
        if intent.order_type is OrderType.LIMIT:
            ...
            if stp is not None:
                ...
                if stp_value not in {"EXPIRE_TAKER", "EXPIRE_MAKER", "EXPIRE_BOTH"}:
                    raise DefinitiveSubmissionError("unsupported Binance selfTradePreventionMode")
    ...
    def _perform(
        ...
        *,
        ...
        for attempt in range(attempts):
            ...
            if (
                not write
                and (response.status >= 500 or response.status in {418, 429})
    ...
    def _assert_account(self, account_id: str) -> None:
        expected = (
            f"{self._gate.evidence.account_id_prefix}:{self._gate.evidence.account_fingerprint}"
        )
    ...
    def _exchange_order_id_map(self, symbol: str) -> dict[str, str]:
        result: dict[str, str] = {}
        for order in self._store.list_orders(symbol=symbol):
...
def _decimal(value: object, field_name: str) -> Decimal:
    if isinstance(value, (bool, float)):
        raise TypeError(f"{field_name} must be exact")
    try:
