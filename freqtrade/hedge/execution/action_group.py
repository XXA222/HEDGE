...
class ActionGroupReport:
    ...
    def fully_successful(self) -> bool:
        return (
            bool(self.results or self.skipped)
            and not self.errors
            and all(
    ...
    def terminal(self) -> bool:
        return (
            bool(self.results or self.skipped)
            and not self.errors
            and all(result.order.lifecycle.terminal for result in self.results)
        )
    ...
    def rejected_count(self) -> int:
        return sum(result.order.lifecycle.status is OrderState.REJECTED for result in self.results)
...
def build_close_both_intents(
    *,
    ...
    _, intents = build_close_both_plan(
    ...
    *,
    ...
    for side in (PositionSide.LONG, PositionSide.SHORT):
        ...
        if quantity == 0:
            members.append(
                ActionGroupMember(
                    ContractPositionSide(side.value),
        ...
        for intent in source:
            try:
                ...
                self._update_member(
                    group_id,
                    ActionGroupMember(
                        ContractPositionSide(intent.position_side.value),
        ...
        for result in report.results:
            try:
                if callable(refresh_order):
                    ...
                    latest = cast(Any, get_order)(result.order.client_order_id)
                    results.append(replace(result, order=latest))
            except Exception as exc:
                errors.append(
                    f"{result.order.intent.position_side.value}:{type(exc).__name__}:{exc}"
                )
