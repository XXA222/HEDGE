...
def _decimal(value: object, *, field_name: str) -> Decimal:
    ...
    try:
        result = to_decimal(value, field=field_name)
    except HedgeDataError as exc:
        raise ValueError(str(exc)) from exc
    if result is None:  # pragma: no cover - to_decimal rejects missing values
...
def _reason_codes(value: object, *, field_name: str) -> tuple[str, ...]:
    ...
    try:
        source = tuple(cast(Iterable[Any], value))
...
def _freeze_value(  # noqa: C901 - recursive execution payload boundary
...
class ExternalOrderSnapshot:
    ...
    def __post_init__(self) -> None:  # noqa: C901 - external order validation boundary
        client_id = _text(
...
class ExecutionOrder:
    ...
    def __post_init__(self) -> None:
        ...
        if self.lifecycle.status is OrderState.FILLED and (
            approved <= 0 or self.lifecycle.filled_quantity != approved
        ):
            raise ValueError("FILLED lifecycle must equal approved_quantity")
        if self.lifecycle.status is OrderState.PARTIAL and not (
            Decimal(0) < self.lifecycle.filled_quantity < approved
        ):
...
class ExecutionBatchReport:
    ...
    def unresolved(self) -> int:
        return sum(result.order.lifecycle.status is OrderState.UNKNOWN for result in self.results)
...
class InMemoryExecutionStore:
    """Bounded current-state projection with O(active) hot-path queries."""

    def __init__(self, *, terminal_retention: int = 2000) -> None:
        if not isinstance(terminal_retention, int) or isinstance(terminal_retention, bool):
            raise TypeError("terminal_retention must be an integer")
        ...
        self._unknown: dict[tuple[str, str, PositionSide], set[str]] = {}
        self._terminal_fifo: deque[str] = deque()
        # Python/Windows clock resolution can produce identical created_at values
        # for multiple submissions in the same tick.  Keep an explicit insertion
        # ordinal so newest_first remains true submission order even on ties.
        self._insertion_sequence: dict[str, int] = {}
        self._next_insertion_sequence = 0
        self._terminal_retention = terminal_retention
        self._lock = RLock()

    def _retire_terminal_locked(self, client_order_id: str) -> None:
        self._terminal_fifo.append(client_order_id)
        while len(self._terminal_fifo) > self._terminal_retention:
            stale = self._terminal_fifo.popleft()
            stale_order = self._orders.get(stale)
            if stale_order is not None and stale_order.lifecycle.terminal:
                self._orders.pop(stale, None)
                self._open.pop(stale, None)
                self._insertion_sequence.pop(stale, None)

    def _update_unknown_index_locked(
        self,
        order: ExecutionOrder,
        previous: ExecutionOrder | None,
    ) -> None:
        if previous is not None and previous.lifecycle.status is OrderState.UNKNOWN:
            previous_bucket = self._unknown.get(previous.leg_key)
            if previous_bucket is not None:
                previous_bucket.discard(previous.client_order_id)
                if not previous_bucket:
                    self._unknown.pop(previous.leg_key, None)
        if order.lifecycle.status is OrderState.UNKNOWN:
            self._unknown.setdefault(order.leg_key, set()).add(order.client_order_id)
    ...
    def put(self, order: ExecutionOrder) -> None:
        ...
        with self._lock:
            ...
            if current is not None:
                ...
                if order.lifecycle.version == current.lifecycle.version and order != current:
                    raise ValueError("same lifecycle version contains conflicting data")
            if current is None:
                self._insertion_sequence[order.client_order_id] = self._next_insertion_sequence
                self._next_insertion_sequence += 1
            self._orders[order.client_order_id] = order
            self._update_unknown_index_locked(order, current)
            became_terminal = order.lifecycle.terminal and (
                current is None or not current.lifecycle.terminal
            )
            if order.lifecycle.terminal:
                self._open.pop(order.client_order_id, None)
                if became_terminal:
                    self._retire_terminal_locked(order.client_order_id)
            else:
                self._open.setdefault(order.client_order_id, None)
    ...
    def has_unresolved_unknown(
        ...
        with self._lock:
            return bool(self._unknown.get(leg_key))
    ...
    def _matches(
        ...
        *,
        ...
        statuses: frozenset[OrderState] | None,
        ...
        if symbol is not None and order.intent.symbol != symbol:
            return False
        if position_side is not None and order.intent.position_side is not position_side:
            return False
        if statuses is not None and order.lifecycle.status not in statuses:
            return False
        if action_group_id is not None and order.intent.action_group_id != action_group_id:
            return False
        return include_terminal or not order.lifecycle.terminal
    ...
    def list_orders(
        ...
        *,
        ...
        normalized_statuses = None if statuses is None else frozenset(statuses)
        with self._lock:
            source = (
                self._orders.values()
                if include_terminal
                else (self._orders[client_id] for client_id in self._open)
            )
            rows = [
                order
                for order in source
            ...
            rows.sort(
                key=lambda order: (
                    order.created_at,
                    self._insertion_sequence.get(order.client_order_id, -1),
                ),
    ...
    def collection_gauges(self) -> dict[str, int]:
        with self._lock:
            return {
                ...
                "unknown_orders": sum(len(bucket) for bucket in self._unknown.values()),
                "retained_terminal_orders": len(self._terminal_fifo),
            }
...
class ExecutionService:
    ...
    def leg_guard(
        ...
        elif isinstance(value, tuple) and len(value) == 3 and isinstance(value[2], PositionSide):
            leg_key = value
        else:
    ...
    def _submit_locked(  # noqa: C901 - locked submission transaction boundary
        self,
        intent: OrderIntent,
        leg_key: tuple[str, str, PositionSide],
    ) -> ExecutionResult:
        ...
        if reservation.state is ReservationState.COMPLETED:
            ...
            latest = self._store.get_by_client_order_id(reservation.value.order.client_order_id)
            return replace(
                reservation.value,
        ...
        try:
            ...
            if not intent.reduces_risk and self._store.has_unresolved_unknown(leg_key):
                ...
                raise ExecutionBlockedError("unresolved UNKNOWN order blocks new risk on this leg")
    ...
    def cancel(self, client_order_id: str) -> ExecutionResult:
        ...
        with self.leg_guard(order):
            ...
            snapshot: ExternalOrderSnapshot | object
            try:
                snapshot = self._exchange.cancel_order(client_order_id=client_order_id)
    ...
    def list_orders(
        ...
        *,
        ...
        if statuses is not None:
            ...
            try:
                normalized_statuses = frozenset(
                    item if isinstance(item, OrderState) else OrderState(item) for item in statuses
                )
            except (TypeError, ValueError) as exc:
        ...
        return tuple(
            self._store.list_orders(
                account_id=account,
                symbol=normalized_symbol,
                position_side=side,
                statuses=(None if normalized_statuses is None else tuple(normalized_statuses)),
    ...
    def refresh_order(self, client_order_id: str) -> ExecutionResult:
        ...
        with self.leg_guard(order):
            ...
            try:
                snapshot = self._exchange.query_order(client_order_id=client_order_id)
            ...
            if snapshot is None or snapshot.status is OrderState.UNKNOWN:
                ...
                if direct_error is not None:
                    message = "direct refresh failed; fallback recovery remained unresolved"
                return ExecutionResult(order, message=message)
            try:
                order = self._apply_snapshot_locked(order, snapshot)
            except Exception as exc:
    ...
    def refresh_orders(
        ...
        *,
        ...
        if not include_unknown:
            selected = tuple(
                order for order in selected if order.lifecycle.status is not OrderState.UNKNOWN
            )
        ...
        for order in selected:
            try:
                ...
                errors.append(f"{order.client_order_id}:{type(exc).__name__}:{exc}")
    ...
    def _apply_snapshot_locked(
        ...
        same_fact = (
            ...
            and filled == current.filled_quantity
            and (snapshot.average_price is None or snapshot.average_price == current.average_price)
            and (
    ...
    def _leg_lock(self, leg_key: tuple[str, str, PositionSide]) -> RLock:
        material = "\x1f".join((leg_key[0], leg_key[1], leg_key[2].value)).encode("utf-8")
