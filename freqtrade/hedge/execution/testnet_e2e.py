...
_ALLOWED_TRANSITIONS: dict[TestnetScenarioState, frozenset[TestnetScenarioState]] = {
    TestnetScenarioState.NEW: frozenset(
        {TestnetScenarioState.PREFLIGHT, TestnetScenarioState.FAILED}
    ),
...
class TestnetE2EConfig:
    ...
    def __post_init__(self) -> None:
        mode = (
            self.mode
            if isinstance(self.mode, TestnetScenarioMode)
            else TestnetScenarioMode(self.mode)
        )
...
class TestnetRunJournal:
        ...
        with self._connect() as connection:
            ...
            if row is not None:
                ...
                if record.request_hash != request_hash or record.account_id != account_id:
                    raise RuntimeError(
                        "testnet E2E run_id is already bound to different input or account"
                    )
                if record.actor != actor:
                    ...
                    if record.actor == "":
                        raise RuntimeError(
                            "testnet E2E active run has no actor binding; "
    ...
    def transition(
        ...
        *,
        ...
        target_state = (
            target if isinstance(target, TestnetScenarioState) else TestnetScenarioState(target)
        )
        with self._connect() as connection:
            connection.execute("BEGIN IMMEDIATE")
            row = connection.execute(
            ...
            now = datetime.now(UTC).isoformat()
            next_client = (
                client_order_id if client_order_id is not None else current.client_order_id
            )
...
class TestnetE2EOrchestrator:
    ...
    async def run(  # noqa: C901 - testnet scenario transaction boundary
        self,
        *,
        ...
        if record.state in {TestnetScenarioState.FAILED, TestnetScenarioState.RECOVERY_REQUIRED}:
            raise RuntimeError(
                f"testnet E2E run is terminal {record.state.value}; "
                "inspect evidence and use a new run_id"
            )
        ...
        try:
            ...
            if config.mode is TestnetScenarioMode.VALIDATION_ONLY:
                ...
                if record.state is TestnetScenarioState.SUBMITTING:
                    if submit_in_this_process:
                        ...
                        if (
                            recovered is None
                            or recovered.order.lifecycle.status is OrderState.UNKNOWN
                        ):
            ...
            failed_record = self.journal.require(selected_run_id)
            if (
                config.mode is TestnetScenarioMode.SUBMIT_CANCEL
                and guarded is not None
                and failed_record.client_order_id
                and failed_record.state
                in {
                    TestnetScenarioState.SUBMITTED,
                    TestnetScenarioState.CANCELING,
                    TestnetScenarioState.VERIFYING,
                ...
                try:
                    if failed_record.state is not TestnetScenarioState.COMPENSATING:
                        failed_record = self.journal.transition(
                            run_id=selected_run_id,
                            target=TestnetScenarioState.COMPENSATING,
                            details={"reason": error},
                        )
                    cleanup_order_id = failed_record.client_order_id
                    if cleanup_order_id is None:
                        raise RuntimeError("TESTNET_CLEANUP_ORDER_ID_MISSING")
                    refreshed = guarded.runtime.engine.refresh_order(cleanup_order_id)
                    if not refreshed.order.lifecycle.terminal:
                        guarded.cancel(cleanup_order_id)
                    verified = guarded.runtime.engine.refresh_order(cleanup_order_id)
                    manual_cleanup_required = not verified.order.lifecycle.terminal
                except Exception:
                    manual_cleanup_required = True
            failed_record = self.journal.require(selected_run_id)
            if failed_record.state not in _TERMINAL_STATES:
                target = (
                    TestnetScenarioState.RECOVERY_REQUIRED
                    if manual_cleanup_required
                    and failed_record.client_order_id is not None
                    and failed_record.state
                    in {
                        TestnetScenarioState.SUBMITTING,
                        TestnetScenarioState.COMPENSATING,
                ...
                self.journal.transition(
                    ...
                    target=target,
                    error=error,
                    last_order_status=failed_record.last_order_status,
                    details={
                        "manual_cleanup_required": manual_cleanup_required,
                        "blind_resubmit": False,
                    },
...
for _public_type in (
    ...
    cast(Any, _public_type).__test__ = False
...
def _notional(value: Decimal, field_name: str) -> Decimal:
    if isinstance(value, (bool, float)):
        raise ValueError(  # noqa: TRY004 - testnet contract
            f"{field_name} must use exact Decimal"
        )
    try:
