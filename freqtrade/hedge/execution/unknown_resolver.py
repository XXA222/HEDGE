...
from .service import ExchangeExecutionPort, ExecutionOrder, ExternalOrderSnapshot
...
class UnknownOrderResolver:
    ...
    def resolve(  # noqa: C901 - unknown-order resolution boundary
        ...
        if self._user_stream_cache is not None:
            try:
                ...
                candidate = self._valid_candidate(order, stream_value, errors, "USER_STREAM")
                if candidate is not None and candidate.status is not OrderState.UNKNOWN:
    ...
    def _valid_candidate(
        ...
        if value.filled_quantity > order.approved_quantity:
            ...
            return None
        if value.status is OrderState.FILLED and value.filled_quantity != order.approved_quantity:
            errors.append(f"{source}:INVALID_FILLED_QUANTITY")
            return None
        return value

    @staticmethod
    def _merge_order_candidates(  # noqa: C901 - candidate reconciliation boundary
        order: ExecutionOrder,
        candidates: list[tuple[ResolutionSource, ExternalOrderSnapshot]],
        ...
        if len(candidates) == 1:
            ...
            if snapshot.filled_quantity == order.approved_quantity and snapshot.status in {
                OrderState.PARTIAL,
                OrderState.CANCELED,
            }:
        ...
        if max_filled > 0:
            priced = [
                ...
                if pair[1].filled_quantity == max_filled and pair[1].average_price is not None
            ]
            if priced:
                average = max(
                    priced,
        ...
        if exchange_order_id is None:
            identified = [pair for pair in candidates if pair[1].exchange_order_id is not None]
            if identified:
    ...
    def _aggregate_fills(
        ...
        status = OrderState.FILLED if total == order.approved_quantity else OrderState.PARTIAL
    ...
    def _merge(
        ...
        if filled == order.approved_quantity:
            ...
            status = (
                OrderState.CANCELED if base.status is OrderState.CANCELED else OrderState.PARTIAL
            )
        else:
            status = base.status
        ...
        return (
            ExternalOrderSnapshot(
                ...
                average_price=average,
                exchange_order_id=(base.exchange_order_id or fills.exchange_order_id),
