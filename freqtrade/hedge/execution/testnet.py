...
def evidence_from_ready_testnet_readonly(  # noqa: C901 - readiness evidence boundary
    *,
    ...
    offset = round(float(getattr(clock_status, "offset_ms", 0.0)))
    ...
    return ProductionGateEvidence(
        ...
        user_stream_status=(
            "FULL_PASS"
            if snapshot.stream_health.last_event_at is not None
...
def build_guarded_testnet_runtime(
    *,
    ...
    ledger = cast(InMemoryExecutionLedger, transaction or InMemoryExecutionLedger())
...
def _notional(value: Decimal) -> Decimal:
    if isinstance(value, (bool, float)):
        raise TypeError("max_order_notional must use exact Decimal")
    try:
