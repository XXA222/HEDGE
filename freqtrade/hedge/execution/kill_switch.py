...
class KillSwitch:
    ...
    def deactivate(self, *, actor: str, confirmed: bool) -> KillSwitchSnapshot:
        ...
        if not confirmed:
            raise PermissionError("kill switch release requires secondary confirmation")
        actor_text = _required_text(actor, field_name="actor", limit=128)
        with self._lock:
            snapshot = KillSwitchSnapshot(
                KillSwitchMode.RUNNING,
    ...
    def _emit(self, snapshot: KillSwitchSnapshot) -> None:
        ...
        if self._metrics is not None:
            try:
                ...
                logger.exception(
                    "Kill switch metric emission failed (halt=%s, reason=%s)",
                    active,
                    snapshot.reason or "",
                )
        if self._audit is not None:
            try:
                ...
                logger.exception(
                    "Kill switch audit emission failed (mode=%s)",
                    snapshot.mode.value,
                )
