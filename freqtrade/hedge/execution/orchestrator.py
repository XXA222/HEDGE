...
class HedgeExecutionEngine:
    ...
    def apply_exchange_event(self, snapshot: ExternalOrderSnapshot) -> ExecutionResult:
        ...
        self._record_result(
            result, previous=before, event_type="ORDER_EVENT_APPLIED", snapshot=snapshot
        )
        return result
    ...
    @contextmanager
    def _position_guard(self, key: PositionKey) -> Iterator[None]:
        acquired = self._position_lock.acquire(key)
        if isinstance(acquired, bool):
            if not acquired:
                raise ExecutionBlockedError("POSITION_LOCK_UNAVAILABLE")
            yield
            return
        with acquired:
            yield
    ...
    def _derive_fill(
        ...
        *,
        ...
        return FillEvent(
            ...
            price=delta_price,
            exchange_time=(
                snapshot.observed_at if snapshot is not None else current.lifecycle.updated_at
            ),
