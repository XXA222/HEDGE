...
def _decimal(value: object, *, field_name: str) -> Decimal:
    ...
    try:
        ...
        raise InvalidOrderTransition(str(exc)) from exc
    if result is None:  # pragma: no cover - to_decimal rejects missing values
        raise InvalidOrderTransition(f"{field_name} must be a valid decimal")
    return result
...
class OrderLifecycle:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "_terminal", status in TERMINAL_STATES)
    ...
    def _build_fact(  # noqa: C901 - immutable transition fact assembly
        self,
