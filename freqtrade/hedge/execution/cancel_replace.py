...
class CancelReplaceCoordinator:
    ...
    def execute(
        ...
        *,
        ...
        with self._service.leg_guard(original):
            ...
            remaining_before = original.approved_quantity - original.lifecycle.filled_quantity
            ...
            if replacement_intent.quantity > remaining_before:
                raise ValueError("replacement quantity exceeds original remaining quantity")
            ...
            remaining_after = (
                canceled.order.approved_quantity - canceled.order.lifecycle.filled_quantity
            )
    ...
    def execute_remaining(
        ...
        *,
        ...
        with self._service.leg_guard(original):
            original = self._service.get_order(original_client_order_id)
            remaining = original.approved_quantity - original.lifecycle.filled_quantity
            if remaining <= 0:
                canceled = self._service.cancel(original_client_order_id)
                return CancelReplaceResult(
                    canceled=canceled,
            ...
            replacement_metadata["replaces_client_order_id"] = original_client_order_id
    ...
    def _validate_replacement(
        ...
        if (
            ...
            raise ValueError("replacement intent must target the same account, symbol and side")
