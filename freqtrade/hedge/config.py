...
def _reject_unknown_keys(
    ...
    *,
    ...
    if unknown:
        raise OperationalException(f"Unknown {field} configuration key(s): {', '.join(unknown)}")
...
def normalize_hedge_config(  # noqa: C901 - layered config normalization boundary
    config: MutableMapping[str, Any],
) -> HedgeRuntimeConfig:
...
def validate_hedge_config(
    ...
    if hedge.enabled:
        ...
        if hedge.operation_mode in {"readonly", "shadow"} and (
            not isinstance(exchange, MutableMapping)
            or not str(exchange.get("key", "")).strip()
            or not str(exchange.get("secret", "")).strip()
        ):
            raise OperationalException(
