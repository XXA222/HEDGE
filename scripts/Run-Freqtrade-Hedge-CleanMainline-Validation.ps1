...
try {
    ...
    $DependencyAuditPath = Join-Path $AuditRoot "hedge-dependencies.cyclonedx.json"
    Run-Step "Dependency vulnerability audit and SBOM" {
        & $Python -m pip_audit `
            --skip-editable `
            --format cyclonedx-json `
            --output $DependencyAuditPath
    }
    Run-Step "pip-audit regression test" {
        & $Python -m pytest -q -ra -o addopts= `
            --basetemp (Join-Path $AuditRoot "pytest-pip-audit-basetemp") `
            tests\test_pip_audit.py
    }
