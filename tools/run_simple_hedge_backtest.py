...
def with_funding(
    events: Iterable[SignalEvent | BarEvent],
) -> list[SignalEvent | BarEvent | FundingEvent]:
...
def write_outputs(output_dir: Path, result, signals: list[SignalEvent], source: str) -> None:
    ...
    payload = {
        ...
        "source": source,
        "bar_count": len(result.snapshots)
        - len([event for event in result.events if isinstance(event, FundingEvent)]),
        "report": report,
        "final_snapshot": {name: getattr(final, name) for name in final.__dataclass_fields__},
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("user_data/backtest_results/simple_dual_leg_hedge")
    )
    ...
    console = {
        ...
        "return_pct": str(
            (report["final_equity"] / _decimal(args.initial_balance) - D("1")) * D("100")
        ),
