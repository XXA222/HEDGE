...
LOCAL_SOURCE_COPY_PATTERNS = (
    "freqtrade",
    "freqtrade_client",
    "freqtrade-*.dist-info",
    "freqtrade_client-*.dist-info",
)
...
def check_layout(root: Path, workspace_mode: bool) -> dict[str, Any]:
    ...
    for path in root.rglob("*"):
        ...
        if (
            path.is_file()
            and relative.startswith("tests/hedge/")
            and VERSIONED_TEST_FILE_RE.search(relative)
        ):
...
def check_python(root: Path, workspace_mode: bool) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for path in source_files(root, workspace_mode):
        ...
        if relative not in ALLOWED_LEGACY_CONFIG_ALIAS_FILES and re.search(
            r"[\"']r56[\"']", source
        ):
...
def check_package_hygiene(root: Path, workspace_mode: bool) -> dict[str, Any]:
    if workspace_mode:
        return gate(
            "package-hygiene", True, "workspace mode ignores local environment/runtime state"
        )
    findings: list[str] = []
    for path in root.rglob("*"):
        relative = rel(root, path)
...
def check_local_source_authority(root: Path) -> dict[str, Any]:
    """Reject a second Freqtrade source tree inside the project venv."""

    site_packages = root / ".venv" / "Lib" / "site-packages"
    if not site_packages.is_dir():
        return gate(
            "no-installed-freqtrade-copy",
            True,
            "project-local site-packages directory is absent",
        )
    findings: list[str] = []
    for pattern in LOCAL_SOURCE_COPY_PATTERNS:
        findings.extend(rel(root, path) for path in site_packages.glob(pattern) if path.exists())
    return gate("no-installed-freqtrade-copy", not findings, sorted(findings))
...
def check_manifest(root: Path, workspace_mode: bool) -> dict[str, Any]:
    ...
    for relative, file_path in expected.items():
        ...
        if not row:
            continue
        if row.get("size") != file_path.stat().st_size or row.get("sha256") != sha256_file(
            file_path
        ):
            bad.append(relative)
...
def validate(root: Path, workspace_mode: bool) -> dict[str, Any]:
    ...
    gates = [
        ...
        check_local_source_authority(root),
        python_gate,
        import_gate,
