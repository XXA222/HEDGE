...
class HedgeIndicatorMtfMemoryEfficient(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        base_weight = self._TREND_WEIGHTS.get(self.timeframe, 0.05)
        weighted = base_trend.fillna(0.0) * base_weight
        weight = base_trend.notna().astype("float32") * base_weight
        configured_weight = base_weight
        rsi_15m = base_rsi

        # Fuse each informative merge into the aggregate immediately, then drop the
        # merged columns before loading the next timeframe.  At most one informative
        # pair is therefore resident in the wide main dataframe at a time.
        if self.dp is not None:
            for timeframe in active:
                tf_weight = self._TREND_WEIGHTS.get(timeframe, 0.10)
                configured_weight += tf_weight
                ...
                if timeframe == "15m" and rsi_column in dataframe.columns:
                    # Copy the only informative RSI used by the tactical signal so the
                    # merged column can be released immediately.
                    rsi_15m = dataframe[rsi_column].astype("float32", copy=True)
                drop_now = [
                    name
                    for name in (trend_column, rsi_column, informative_date)
                    if name in dataframe.columns
                ]
                if drop_now:
                    dataframe.drop(columns=drop_now, inplace=True)
                del compact
                del informative
        ...
        dataframe["hedge_long_score"] = long_score.where(enough_history, 0.0).astype("float32")
        dataframe["hedge_short_score"] = short_score.where(enough_history, 0.0).astype("float32")
        dataframe["hedge_target_net_ratio"] = (direction * 0.18).astype("float32")
        dataframe["hedge_confidence"] = (
            (0.55 + 0.45 * direction.abs()).mul(coverage).clip(0.0, 1.0).astype("float32")
        )
        dataframe["hedge_risk_scale"] = (
            (0.60 + 0.40 * coverage).clip(0.0, 1.0).astype("float32")
        )
        ...
        # Drop local Series/DataFrame references eagerly.  This is useful on long
        # 1m runs because pandas temporaries otherwise survive until function return.
        del base, base_trend, base_rsi, weighted, weight, direction, coverage
        del long_core, short_core, oversold, overbought, long_score, short_score
        return dataframe
