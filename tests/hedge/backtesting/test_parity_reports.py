...
def events():
    return [
        ...
        FundingEvent(
            START + timedelta(minutes=7), "ETH/USDT:USDT", Decimal("0.0001"), Decimal(103)
        ),
        BarEvent(
...
def test_required_report_fields_exist():
    ...
    required = {
        "long_pnl",
        "short_pnl",
        "core_cost_change_long",
        "core_cost_change_short",
        "tactical_trading_pnl",
        "gross_peak",
        "net_exposure",
        "funding",
        "fees",
        "dual_leg_duration_seconds",
        "add_count",
        "reduce_count",
        "max_drawdown",
    }
...
def test_failed_incremental_symbol_validation_does_not_corrupt_state():
    ...
    expected = HedgeBacktesting(initial_balance=Decimal(1000)).run(all_events[:2])
    assert first.snapshots + second.snapshots == expected.snapshots
    assert second.report == expected.report
