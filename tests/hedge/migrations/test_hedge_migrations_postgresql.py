...
def test_postgresql_migration_and_internal_backup_schema():
    ...
    try:
        with engine.begin() as connection:
            ...
            connection.execute(
                text("INSERT INTO trades VALUES (1, 'binance', 'ETH/USDT:USDT', TRUE, FALSE, 2)")
            )
        ...
        failed_status = {row["migration_id"]: row for row in migration_status(engine)}
        ...
        with engine.begin() as connection:
            connection.execute(text("UPDATE trades SET pair='BTC/USDT:USDT' WHERE id=1"))
        restored = restore_postgresql_backup(engine, full_backup)
        assert restored["trades"] == 1
        with engine.connect() as connection:
            assert (
                connection.execute(text("SELECT pair FROM trades WHERE id=1")).scalar_one()
                == "ETH/USDT:USDT"
            )
        ...
        with admin_engine.begin() as connection:
            ...
            for candidate in (backup_schema, full_backup_schema):
                if candidate:
                    connection.execute(text(f'DROP SCHEMA IF EXISTS "{candidate}" CASCADE'))
