...
def wallet(long_qty=Decimal(0), short_qty=Decimal(0), active=()):
    return WalletSnapshot(
        ...
        long=LegPosition(
            PositionSide.LONG,
            long_qty,
            Decimal(100) if long_qty else Decimal(0),
            long_qty,
            Decimal(100) if long_qty else Decimal(0),
            Decimal(0),
            Decimal(0),
        ...
        short=LegPosition(
            PositionSide.SHORT,
            short_qty,
            Decimal(100) if short_qty else Decimal(0),
            short_qty,
            Decimal(100) if short_qty else Decimal(0),
            Decimal(0),
            Decimal(0),
...
def test_long_and_short_have_distinct_intents_and_state():
    ...
    assert {item.position_side for item in result.ideal_orders} == {
        PositionSide.LONG,
        PositionSide.SHORT,
    }
        ...
        len(
            {
                ...
                if not item.reduce_only and item.position_side is PositionSide.LONG
            }
        )
        <= 2
    )
        ...
        len(
            {
                item.layer
                for item in result.ideal_orders
                if not item.reduce_only and item.position_side is PositionSide.SHORT
            }
        )
        <= 2
    )
...
def test_projected_gross_never_exceeds_cap():
    cfg = PlannerConfig(
        max_grid_layers=6,
        max_gross_wallet_exposure=Decimal("0.30"),
        max_wallet_exposure_long=Decimal("0.20"),
        max_wallet_exposure_short=Decimal("0.20"),
        cooldown_seconds=0,
    )
...
def test_core_and_tactical_allocations_are_separate():
    cfg = PlannerConfig(
        core_wallet_exposure_long=Decimal("0.05"),
        tactical_wallet_exposure_long=Decimal("0.20"),
        max_wallet_exposure_long=Decimal("0.30"),
        cooldown_seconds=0,
    )
    result = PureHedgePlanner().plan(context(config=cfg))
    long_entries = [
...
def test_disabled_short_side_produces_no_short_orders():
    result = PureHedgePlanner().plan(
        context(config=PlannerConfig(short_enabled=False, cooldown_seconds=0))
    )
...
def test_entry_cooldown_blocks_reentry_plans():
    ...
    result = PureHedgePlanner().plan(
        context(long_state=state, config=cfg, wallet=wallet(Decimal("0.2"), Decimal(0)))
    )
    assert all(
        item.reduce_only for item in result.ideal_orders if item.position_side is PositionSide.LONG
    )
...
def test_close_grid_preserves_core_floor():
    cfg = PlannerConfig(
        ...
        core_min_fraction=Decimal("0.75"),
        cooldown_seconds=0,
    )
    ...
    target_core = calculate_target(
        context(config=cfg, wallet=custom_wallet), PositionSide.LONG
    ).core_quantity
...
def test_replacement_is_debounced_for_young_semantically_matching_order():
    ...
    result = PureHedgePlanner().plan(context(config=cfg, wallet=wallet(active=(active,))))
