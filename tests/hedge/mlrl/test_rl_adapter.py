...
class TestRound16RLAdapter(unittest.TestCase):
    ...
    def test_freqai_bridge_reproduces_flat_shape_and_mask(self):
        ...
        bridge = HedgeFreqAIPolicyBridge(feature_names=("a", "b"), window_size=4, config=cfg)
        ...
        model = FakeModel()
        action = bridge.predict_action(model, observation, context=context, use_masking=True)
