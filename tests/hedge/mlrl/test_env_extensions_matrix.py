...
def test_round85_truncation_reason_distinguishes_dataset_and_step_limit():
    ...
    assert (
        truncation_reason(current_tick=5, end_tick=9, episode_steps=2, max_episode_steps=10) is None
    )
...
def test_round86_invalid_action_fallback_is_fail_closed_to_hold():
    ...
    assert (
        invalid_action_fallback(HedgeActions.LONG_OPEN_SMALL, mask) is HedgeActions.LONG_OPEN_SMALL
    )
