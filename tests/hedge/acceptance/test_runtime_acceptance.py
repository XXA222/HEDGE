...
from freqtrade.hedge.acceptance.facts import (
    build_fact_plane,
    fill_values,
    income_values,
    order_values,
...
def test_round16_fast_reconciliation_detects_position_drift() -> None:
        ...
        balances={
            **rest.balances,
            balance_key: replace(
                ...
                wallet_balance=rest.balances[balance_key].wallet_balance + Decimal("0.0000005"),
            ),
        },
    )
    ...
    assert any(
        item.entity_type == "ACTIVE_ORDER" and item.reason == "CUMULATIVE_FILLED_QUANTITY_MEMORY"
...
def test_round16_exchange_and_canonical_symbols_share_one_identity() -> None:
    ...
    database = replace(
        rest,
        positions={
            key: replace(value, symbol="BTC/USDT:USDT") for key, value in rest.positions.items()
        },
        active_orders={
...
def test_round17_deep_reconciliation_includes_fill_and_income_keysets() -> None:
    now = _now()
    rest = build_fact_plane(account_id="acct", observed_at=now, fills=_fills(now), income=_income())
