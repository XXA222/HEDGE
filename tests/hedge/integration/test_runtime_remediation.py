...
def test_disabled_hedge_bootstrap_is_schema_noop() -> None:
    ...
    with engine.begin() as connection:
        ...
        connection.execute(
            text("CREATE TABLE orders (id INTEGER PRIMARY KEY, ft_pair VARCHAR(64))")
        )
    before = {
...
def test_strategy_adapter_uses_latest_analyzed_row_without_order_submission() -> None:
    ...
    signal = FreqtradeStrategySignalProvider(provider, strategy).signals("ETH/USDT:USDT", "5m")
...
def test_partial_active_order_is_checkpointed_and_restored_without_resubmit(tmp_path) -> None:
    ...
    assert any(item.lifecycle.filled_quantity < item.approved_quantity for item in active_before)
    ...
    assert {(item.client_order_id, item.lifecycle.filled_quantity) for item in active_after} == {
        (item.client_order_id, item.lifecycle.filled_quantity) for item in active_before
    }
