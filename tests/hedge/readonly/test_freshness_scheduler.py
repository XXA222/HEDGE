...
def test_disconnected_and_stale_stream_are_not_ready():
    clock = FakeClock()
    checker = UserStreamFreshness(FreshnessPolicy(event_stale_after=timedelta(seconds=10)))
...
def test_freshness_rejects_naive_timestamps_even_when_both_are_naive():
    checker = UserStreamFreshness()
    naive = datetime(2026, 7, 26, 12, 0, 0, tzinfo=UTC).replace(tzinfo=None)
