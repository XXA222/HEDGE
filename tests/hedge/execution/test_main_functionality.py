...
def test_batch_refresh_and_filtered_listing() -> None:
    ...
    second = service.submit(make_intent("batch-short", side=PositionSide.SHORT))
    ...
    assert {order.lifecycle.status for order in service.list_orders(account_id="main")} == {
        OrderState.FILLED,
        OrderState.PARTIAL,
    }
...
def test_resolve_unknowns_operates_as_a_batch() -> None:
    ...
    exchange.set_order(exchange.acknowledge_order(result.order.client_order_id))
...
def test_cancel_orders_filters_by_position_side() -> None:
    ...
    short_order = service.submit(make_intent("cancel-short", side=PositionSide.SHORT))
    ...
    assert (
        service.get_order(short_order.order.client_order_id).lifecycle.status
...
def test_cancel_replace_remaining_builds_exact_replacement() -> None:
    ...
    assert (
        result.replacement.order.intent.metadata["replaces_client_order_id"]
        == original.order.client_order_id
    )
