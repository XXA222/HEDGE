...
def test_canceled_fact_is_merged_with_later_partial_fills() -> None:
    ...
    resolver = UnknownOrderResolver(MultiSourceExchange(direct=canceled, fills=fills))
    resolved = resolver.resolve(order)
...
def test_rejected_fact_with_real_fill_is_promoted_to_partial() -> None:
    ...
    resolved = UnknownOrderResolver(MultiSourceExchange(direct=rejected, fills=fills)).resolve(
        order
    )
    assert resolved.status is OrderState.PARTIAL
...
def test_stronger_open_fact_can_override_direct_ack() -> None:
    ...
    resolver = UnknownOrderResolver(MultiSourceExchange(direct=direct, opened=opened))
...
def test_last_result_access_is_safe_under_concurrency() -> None:
    order = unknown_order()
    exchange = MultiSourceExchange(direct=snapshot(order, OrderState.ACKNOWLEDGED))
    resolver = UnknownOrderResolver(exchange)
...
def test_more_complete_base_fact_keeps_authoritative_average_price() -> None:
    ...
    resolved = UnknownOrderResolver(MultiSourceExchange(direct=base, fills=fills)).resolve(order)
...
def test_rejected_zero_does_not_erase_partial_fill_from_open_orders() -> None:
    ...
    resolved = UnknownOrderResolver(MultiSourceExchange(direct=direct, opened=opened)).resolve(
        order
    )
