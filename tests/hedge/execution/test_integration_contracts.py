...
def test_planner_adapter_produces_deterministic_execution_intent() -> None:
    one = adapt_planner_intent(PlannerIntent(), account_id="acct", strategy_id="s", cycle_id="c")
    two = adapt_planner_intent(PlannerIntent(), account_id="acct", strategy_id="s", cycle_id="c")
    assert one == two
...
def test_readiness_halt_blocks_new_risk_but_allows_reduce_to_reach_core() -> None:
    ...
    reduced = runtime.engine.submit(market_intent(action=IntentAction.REDUCE, key="reduce"))
