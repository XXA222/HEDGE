...
from freqtrade.hedge.errors import HedgeDefinitiveError
...
class DefinitiveResolver(Resolver):
    def resolve_unknown(self, client_order_id: str) -> ExecutionResult:
        self.calls += 1
        raise HedgeDefinitiveError("venue rejected the recovery query")
...
def test_supervisor_routes_definitive_recovery_failure_to_manual_review():
    client_id = "hx-test-definitive-unknown"
    clock = Clock(datetime(2026, 7, 27, tzinfo=UTC))
    supervisor = UnknownOrderSupervisor(DefinitiveResolver(client_id), clock=clock)

    supervisor.register(client_id)
    result = supervisor.attempt(client_id)

    assert result.state is UnknownRecoveryState.MANUAL_REVIEW
    assert result.next_retry_at is None
    assert "HedgeDefinitiveError" in result.last_message
