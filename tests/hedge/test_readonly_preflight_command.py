...
_MODULE_PATH = (
    Path(__file__).resolve().parents[2] / "freqtrade" / "commands" / "hedge_readonly_commands.py"
)
...
def test_readonly_preflight_sanitizes_proxy_and_halts_on_unmanaged(monkeypatch):
    ...
    class Client:
        def __init__(self):
            self.clock_sync = SimpleNamespace(status=ClockSyncStatus(True, 1.0, 2.0, 5, 1000.0))
