...
def intent(
    side, order_side, qty, price, reduce=False, bucket=PositionBucket.CORE, action=IntentAction.OPEN
):
...
def test_long_short_average_prices_are_independent():
    ...
    wallet.apply_fill(fill("s", short, "2", "110"))
...
def test_reduce_only_cannot_flip_position():
    ...
    wallet.accept_order("a", add)
    wallet.apply_fill(fill("a", add, "1", "100"))
    close = intent(
        PositionSide.LONG, OrderSide.SELL, "2", "110", True, PositionBucket.CORE, IntentAction.CLOSE
    )
    wallet.accept_order("c", close)
    bar = BarEvent(
        NOW, close.symbol, Decimal(100), Decimal(120), Decimal(90), Decimal(110), Decimal(100)
    )
...
def test_funding_offsets_equal_hedged_notional():
    ...
    wallet.apply_fill(fill("l", long, "1", "100"))
    wallet.accept_order("s", short)
    wallet.apply_fill(fill("s", short, "1", "100"))
    amount = FundingEngine().apply(
        wallet, FundingEvent(NOW, long.symbol, Decimal("0.001"), Decimal(100))
    )
...
def test_conservative_same_bar_path_avoids_optimistic_round_trip():
    ...
    bar = BarEvent(
        NOW, entry.symbol, Decimal(100), Decimal(110), Decimal(90), Decimal(100), Decimal(100)
    )
    fills = ConservativeMatcher().match(bar, wallet)
...
def test_all_ioc_orders_expire_when_volume_is_consumed():
    ...
    matcher = ConservativeMatcher(MatchConfig(volume_participation=Decimal("0.1")))
