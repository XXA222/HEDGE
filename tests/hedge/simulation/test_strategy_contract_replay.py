...
def test_replay_carries_strategy_directive_and_checkpoint():
    ...
    engine.replay(
        (
            sig,
            bar(t + timedelta(minutes=1)),
        )
    )
    cp = engine.checkpoint()
    assert (
        cp.signal_directive.target_net_ratio == Decimal("0.2")
        and cp.signal_directive.regime == "BULL"
    )
    clone = EventReplayEngine(initial_balance=Decimal(1000), planner_config=engine.planner_config)
    clone.restore(cp)
    assert clone.checkpoint().signal_directive == cp.signal_directive
