...
def test_require_role_fails_closed_for_wrong_dependency_type() -> None:
    ...
    @app.get("/protected")
    def protected(_=Depends(dependency)):
        return {"ok": True}
...
def test_readonly_audit_limit_is_validated_not_silently_clamped() -> None:
    app = FastAPI()
    app.include_router(create_hedge_readonly_router(QueryPort(), principal_dependency=viewer))
...
def test_async_websocket_validator_is_awaited() -> None:
    ...
    with TestClient(app).websocket_connect(
        ...
        asyncio.run(hub.publish(HedgeTelemetryEvent(HedgeEventType.ORDER, {"status": "ACK"})))
...
def test_websocket_subscription_is_removed_on_disconnect() -> None:
    hub = HedgeEventHub()
    app = FastAPI()
    app.include_router(create_hedge_ws_router(hub, token_validator=lambda token: True))
...
def test_websocket_rejects_oversized_bearer_token_before_validator() -> None:
    ...
    app = FastAPI()
    app.include_router(create_hedge_ws_router(HedgeEventHub(), token_validator=validator))
    with pytest.raises(WebSocketDisconnect) as exc_info:
        with TestClient(app).websocket_connect(
            "/hedge/ws",
