...
def test_dashboard_uses_planner_targets_not_actual_positions():
    ...
    store.append(
        DryRunCycleTelemetry(
            ...
            net_gap_quantity=Decimal(1),
            long_quantity=Decimal(3),
            short_quantity=Decimal(2),
            long_target_quantity=Decimal(4),
            short_target_quantity=Decimal(2),
        )
    )
    ...
    assert (
        o.latest.long_quantity == 3
        and o.latest.long_target_quantity == 4
        and o.latest.target_net_quantity == 2
    )
