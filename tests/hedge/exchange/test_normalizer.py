...
@pytest.mark.parametrize(
    ...
    [
        (None, MissingPolicy.ZERO, Decimal(0)),
        ("", MissingPolicy.ZERO, Decimal(0)),
        (None, MissingPolicy.NONE, None),
    ],
)
def test_optional_decimal_requires_an_explicit_missing_policy(value, policy, expected):
    assert optional_decimal(value, field="x", when_missing=policy) == expected


def test_optional_decimal_rejects_missing_values_with_reject_policy():
    for value in (None, ""):
        with pytest.raises(BinanceDataError, match="x is required"):
            optional_decimal(value, field="x", when_missing=MissingPolicy.REJECT)


def test_finite_decimal_rejects_boolean_and_empty_values():
    for value in (True, ""):
        with pytest.raises(BinanceDataError):
            finite_decimal(value, field="x")
...
def test_stable_fingerprint_preserves_sequence_order():
    assert stable_fingerprint(["alpha", "beta"]) != stable_fingerprint(["beta", "alpha"])
