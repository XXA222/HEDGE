...
def test_duplicate_and_out_of_order_do_not_overwrite():
    ...
    assert asyncio.run(stream.process_message(order_event())) is EventDisposition.APPLY
    assert asyncio.run(stream.process_message(order_event())) is EventDisposition.DUPLICATE
    assert (
        asyncio.run(
...
def test_unmanaged_active_order_forces_fault_before_persistence():
    ...
    assert asyncio.run(stream.process_message(payload)) is EventDisposition.GAP
...
def test_unmanaged_nonzero_position_forces_fault_before_persistence():
    ...
    assert asyncio.run(stream.process_message(account_event("ETHUSDT"))) is EventDisposition.GAP
...
def test_invalid_utf8_fails_closed_and_is_audited():
    ...
    assert any(item.event_type == "STREAM_INTEGRITY_FAULT" for item in repository.events)
...
def test_account_update_without_rest_context_fails_closed():
    ...
    assert asyncio.run(stream.process_message(account_event())) is EventDisposition.GAP
...
def test_same_timestamp_monotonic_fill_update_is_applied():
    ...
    assert asyncio.run(stream.process_message(monotonic_fill)) is EventDisposition.APPLY
    assert faults == []
...
def test_same_timestamp_non_monotonic_state_still_fails_closed():
    ...
    assert asyncio.run(stream.process_message(regressive)) is EventDisposition.OUT_OF_ORDER
    ...
    class FailingEventRepository(FakeRepository):
        ...
        async def append_account_events(self, facts):
            should_fail = self.fail_once and any(
                fact.event_type == "ORDER_TRADE_UPDATE" for fact in facts
            )
            if should_fail:
                self.fail_once = False
...
def test_unknown_event_without_timestamp_fails_closed():
    ...
    assert any(event.event_type == "STREAM_INTEGRITY_FAULT" for event in repository.events)
...
def test_account_update_exposes_single_balance_change_as_accounting_fact():
    ...
    payload["a"]["P"] = []
    payload["a"]["B"] = [{"a": "USDT", "wb": "1000.25", "cw": "1000.25", "bc": "0.25"}]
