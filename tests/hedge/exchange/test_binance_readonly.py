...
def test_permission_preflight_requires_read_and_futures_but_forbids_money_movement():
    ...
    assert any("NOT_SEPARATELY_VERIFIABLE" in warning for warning in report.warnings)
...
def test_permission_preflight_accepts_usdm_read_when_enable_futures_is_false():
    ...
    assert any(
        "AUTHENTICATED_USDM_USER_DATA_READ_CONFIRMED" in warning for warning in report.warnings
    )
...
def test_fills_and_order_history_paginate_without_gaps():
    ...
    assert any(
        params.get("fromId") == 3 for path, params in transport.calls if path.endswith("userTrades")
    )
    assert any(
        params.get("orderId") == 3 for path, params in transport.calls if path.endswith("allOrders")
    )
...
def test_income_history_uses_current_one_based_page_parameter():
    ...
    assert [(item["incomeType"], item["tranId"]) for item in values] == [
        ("FUNDING_FEE", 1),
        ("COMMISSION", 1),
        ("FUNDING_FEE", 2),
    ]
    assert [params["page"] for params in transport.params] == [1, 2]
    assert all(params["startTime"] == 0 and params["endTime"] == 3 for params in transport.params)
...
def test_low_activity_history_walks_every_seven_day_window():
    ...
    assert all(
        end_time - start_time < seven_days for start_time, end_time in fill_windows + order_windows
    )
...
def test_fetch_bundle_reads_all_account_surfaces_and_history():
    ...
    position_call = next(
        params for path, params in transport.calls if path.endswith("positionRisk")
    )
    open_orders_call = next(
        params for path, params in transport.calls if path.endswith("openOrders")
    )
    symbol_config_call = next(
        params for path, params in transport.calls if path.endswith("symbolConfig")
    )
...
def test_same_numeric_ids_on_different_symbols_are_not_collapsed():
    ...
    assert {(item.symbol, item.exchange_trade_id) for item in fills} == {
        ("BTCUSDT", "1"),
        ("ETHUSDT", "1"),
    }
    assert {(item.symbol, item.exchange_order_id) for item in orders} == {
        ("BTCUSDT", "1"),
        ("ETHUSDT", "1"),
    }
