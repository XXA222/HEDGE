...
def _local_import_targets(
    ...
    *,
    ...
    candidates = list(_imported_modules(current_module, node, is_package=is_package))
...
def _forbidden_import_findings(
    ...
    *,
    ...
    return [
        ...
        for module in _imported_modules(current_module, node, is_package=is_package)
        if _is_forbidden_import(module)
    ]


def _dynamic_import_name(node: ast.Call) -> str | None:
    builtins_import = isinstance(node.func, ast.Name) and node.func.id == "__import__"
    importlib_import = (
...
def _call_findings(path: Path, node: ast.Call) -> list[str]:
    ...
    if write_entry is not None:
        findings.append(f"Forbidden exchange write request {write_entry!r} in {path}:{node.lineno}")
    return findings
...
def _node_findings(
    ...
    *,
    ...
    if isinstance(node, (ast.Import, ast.ImportFrom)):
        return _forbidden_import_findings(path, current_module, node, is_package=is_package)
...
def _inspect_module(
    ...
    for node in ast.walk(tree):
        findings.extend(_node_findings(path, current_module, node, is_package=is_package))
        if isinstance(node, (ast.Import, ast.ImportFrom)):
            targets.extend(
...
def main() -> int:
    hedge_root = Path(sys.argv[1] if len(sys.argv) > 1 else "overlay/freqtrade/hedge").resolve()
