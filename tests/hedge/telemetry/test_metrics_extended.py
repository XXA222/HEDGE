...
def test_halt_reason_is_normalized_in_separate_bounded_family():
    ...
    assert snapshot["halt_reason_total"] == {"reason=SINGLE_WRITER_LOST_LEASE_123": "1"}


def test_error_reason_code_is_bounded_and_structured():
    metrics = HedgeMetrics()
    metrics.error("HEDGE/DEFINITIVE:invalid payload", disposition="DEFINITIVE")

    assert metrics.snapshot()["error_total"] == {
        "disposition=DEFINITIVE,reason_code=HEDGE_DEFINITIVE_INVALID_PAYLOAD": "1"
    }
