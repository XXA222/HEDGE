...
def test_control_persists_and_corruption_fails_closed(tmp_path):
    ...
    s = DryRunControlState(p)
    s.pause_new_risk(actor="u", reason="x")
    assert DryRunControlState(p).snapshot().mode is DryRunControlMode.NEW_RISK_PAUSED
    p.write_text("{bad", encoding="utf-8")
    recovered = DryRunControlState(p).snapshot()
    assert not recovered.new_risk_enabled and recovered.reason == "CONTROL_STATE_RECOVERY_FAILED"


def test_jsonl_telemetry_survives_restart_and_old_rows(tmp_path):
    p = tmp_path / "cycles.jsonl"
    s = JsonlDryRunTelemetryStore(p, capacity=10)
    s.append(item())
