...
- `user_data/` is runtime-owned and is preserved during clean-mainline upgrades.
  It is outside the source-tree naming policy and is intentionally excluded from
  source layout, syntax, and manifest checks. Runtime databases and logs must not
  be moved or renamed by clean-mainline remediation.
...
- `freqtrade/hedge/contracts/` — stable domain contracts and execution-subsystem
  ports. The ports are not currently a universal dependency for every workstream;
  architecture tests protect the specific inversion boundaries that use them.
...
- `freqtrade/freqai/hedge_rl/` — current Hedge ML/RL subsystem.
  It contains both the stable 21-action Hedge RL contract and the independent MultiDiscrete([5, 5]) risk-level RL contract; neither bypasses planner/risk/execution boundaries.
...
The configuration authority split is documented in `docs/hedge/config-authority.md`.
The JSON schema definitions live in `config_schema_extension.py`; runtime
normalization remains in `config.py`, and legacy-key migration remains isolated in
`config_migration.py`. `user_data/` is a runtime boundary and is not part of the
source package naming policy.

## Source hygiene

`tools/validate_clean_mainline.py` is the package/workspace authority. It rejects versioned Hedge namespaces, removed compatibility modules, historical release directories, generated Python metadata, source-package runtime artifacts, and installed Freqtrade source copies inside the project virtual environment.
